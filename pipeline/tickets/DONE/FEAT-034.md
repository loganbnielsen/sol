...
All 5 bug scenarios verified against actual current OCaml source (`framework/kafka-eio-service/lib/{kafka_service,kafka_service_intf,kafka_service_schema}.ml`, post-REFAC-071 reorg): registration ordering/fatality, topic provisioning, Confluent wire format, decode/retry/crash routing, and traceparent flags-byte handling all genuinely match, each with real regression tests (not happy-path only) — 17/17 pass, `tsc --noEmit` clean, OCaml side unaffected (zero `.ml` files touched). API surface (`registerTopic`, `wrapEachMessage`, `wireCrashListener`, `traceparentOf`/`extractTraceparent`) is clean and consumer-usable; `tracing.ts`'s traceparent helpers are documented as a temporary home pending `@sol/obs` (FEAT-035) — sound interim call, not a defect. PR #165's actual GitHub Actions run (34294445598) fully green: `test` passed, all 4 dockerfile-smokes passed, `golden-path-smoke` passed (13m14s). Promoting on confirmed real-CI green.
