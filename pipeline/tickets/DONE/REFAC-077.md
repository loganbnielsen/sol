...
- `devtools/soldev/lib/soldev_merge.ml:69` — pr_review_approved uses `List.exists` over ALL comments ever posted on the PR, searching for the SOLDEV-REVIEW: PASS marker anywhere in history — not the most recent review verdict, and not tied to any commit SHA. Two real failure modes: (1) PASS then a later FAIL (a bounce-and-still-failing round) still reads as approved, because the old PASS line still matches `List.exists`; merge would proceed past an unresolved violation. (2) PASS then an unrelated later commit (rebase, manual fixup, human-requested tweak) that nobody re-reviewed is still covered by the stale PASS — merge has no way to know HEAD moved since approval. Fix: only trust the review verdict if it's the temporally-last SOLDEV-REVIEW comment (PASS or FAIL), and ideally include the reviewed commit SHA in the comment body so merge can confirm it still matches HEAD before treating the PR as approved.
