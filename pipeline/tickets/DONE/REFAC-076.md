...
## Re-review — fix confirmed, real CI green (2026-09-08)

Follow-up commit `4ca73fd1` deduped via an order-preserving filter — confirmed the internal accumulator is already reversed to forward order before dedup runs, so first-occurrence order is genuine, not an artifact. Independently re-verified `pipeline check FEAT-037` now prints `depends on: FEAT-034` once with a freshly-built binary. `test_depends_prose` corrected to match. Full `devtools/soldev/` suite (29+9) and full repo suite pass. Fresh look at the whole PR found nothing else. PR #169's complete CI run (34306630332) fully green: `test`, both `demo-ts-dockerfile-smoke` entries, all 4 `example-dockerfile-smoke` entries, and `golden-path-smoke` (17m49s) all passed. Promoting on confirmed real-CI green.
