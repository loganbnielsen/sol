...
All 3 scenarios verified against actual current OCaml source: metric vocabulary matches `worker.ml`/`kafka_service_intf.ml`/`service.ml` exactly (status enum, separate decode-error counter, fixed `"unmatched"` route label), traceparent flags-byte correctly preserves `flags=0`, Loki push shape matches the demo_ts reference. 12/12 tests genuinely exercise failure modes, not just happy paths. `@sol/kafka` (FEAT-034) has since merged — its traceparent helpers still duplicate this package's, documented as a known interim state on both sides; reconciling that (re-export from `@sol/obs` instead of duplicating) is left for the dogfood pass, where the choice of which helper the migrated demo actually calls will settle it naturally. PR #166 rebased against the now-current main (commit 96bd367a) after FEAT-034 merged; real GitHub Actions run (34296620925) fully green: `test` passed, all 4 dockerfile-smokes passed, `golden-path-smoke` passed (16m22s). Promoting on confirmed real-CI green.
