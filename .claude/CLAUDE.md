...
**TypeScript-parity tracking:** Sol is OCaml-only today, but `examples/pluto/app/demo_ts/` and `pipeline/dogfood/2026-09-07_typescript_demo_spike.md` (FEAT-033) already establish that the CLI/deploy contract is language-neutral and specifically identify which OCaml-side conventions (Kafka schema registry protocol, Confluent wire format, W3C trace-header propagation onto Kafka, retry/crash semantics, metric-naming/label vocabulary) a future `@sol/kafka`/`@sol/obs`/`@sol/http` package would need to encode — deliberately not built yet, gated on real TS demand, same discipline as an unbuilt infra ticket. Any ticket that changes one of those conventions, or introduces a new framework-level concept an OCaml app author gets "for free" (a new primitive, a new library like `sol-jobs`, a new retry/backoff/observability contract), must check whether it affects that gap and say so in one line in its completion notes — either "no TS-parity impact" with why, or a reference to a `BACKLOG` tracking ticket recording what a TS port would now also need. This is bookkeeping, not a requirement to build or update the TS spike itself; the point is that the gap is never silently rediscovered wholesale the way FEAT-076 through FEAT-079 and FEAT-077 accumulated against a spike that predates all of them.
