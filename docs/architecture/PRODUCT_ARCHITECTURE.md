...
**Security on Day 1.** Sol's framework types carry security configuration as a first-class concern — transport encryption, SASL authentication, and TLS are all part of the data model from the beginning, read from environment variables. The transport posture is never defaulted: `config_of_env` refuses a workload that does not set `KAFKA_SECURITY_PROTOCOL`, and Sol's rendered manifests set it explicitly (SEC-007). **Today that declared posture is in-cluster plaintext with no SASL, in every profile including production** — the in-cluster Redpanda runs without TLS. Encrypting and authenticating production Kafka is FEAT-093; until then the posture is stated, not hidden.
...
- Hosted infrastructure lives in a single Sol-owned AWS account. Each
  ...
  same `platform/cloud/aws/cluster` Terraform module self-hosted `sol cloud apply`
