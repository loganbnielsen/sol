...
| 7 | §B B3–B7 | `NOT REACHED` — the call-graph defect blocks the payment path, and one `notify-worker` replica is not ready (§7.2) |
...
| `notify-worker` (2 replicas) | one `1/1`, one `0/1 Running` (1 restart) | diagnosed read-only, §7.2 |
| `charge-svc` | never deployed | FND-0016 |
...
| **FND-0014** (release record: prune + pointer) | `OPEN`, severity low | `OPEN`, **severity escalated**, two manifestations | §7 / execution log: the pointer patch is forbidden and the deploy still reports `Done.`; the pointer still names an older release |
| **FND-0016** (scoped deploy needs a closed call graph) | — | **new**, `VERIFIED_DEFECT`, `OPEN` | Execution log: `charge_svc` cannot be deployed; with FND-0012 this leaves the workspace with no route that deploys all its services |
