# Reference: the application contract
...
these pages are the product-level definition both of them target.

| Concept | Where the contract is defined |
| --- | --- |
| Project/workspace declarations (`sol.yml`, `sol.toml`) | `cli/lib/workspace/sol_cli_config.ml`, `cli/lib/sol_cli_manifest*.ml` |
...
file; this page organizes them as one concept and does not keep a second
