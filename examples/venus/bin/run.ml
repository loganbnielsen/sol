...
let kafka_config : Kafka_service.config =
  {
    (Kafka_service.config_of_env () |> require_kafka "kafka config") with
    linger_ms = 5;
  }
...
let new_charge_id () =
  Printf.sprintf "ch_%08x%08x" (Random.bits ()) (Random.bits ())
...
let create_registered_topic ~sw ~net ~clock () =
  say "registering topic %S ..."
    (Kafka_service.topic_name_to_string Charged.topic_name);
  let svc =
    Kafka_service.create kafka_config ~sw |> require_kafka "kafka create"
  in
  let topic =
...
let () =
  ...
  Printf.printf "%s\n" sep;
  Printf.printf
    "  Teams:           payments (charge-svc)  ·  comms (notify-worker)\n";
  ...
  let db_pool =
    optional_db_pool ~sw
      ...
      ~fs:env#fs postgres_url
  in

  (* ── Kafka (shared infrastructure) ─────────────────────────────────────── *)
  let svc, topic =
    create_registered_topic ~sw ~net:env#net ~clock:env#clock ()
  in
  ...
  Eio.Fiber.fork ~sw (fun () ->
      (try
       ...
       with Failure msg -> Printf.eprintf "[notify-worker] error: %s\n%!" msg);
  ...
  let handle_charge req =
      ...
      Charged.
        {
          ...
          currency =
            (let c = s "currency" in
             if c = "" then "USD" else c);
          correlation_id = corr_id;
        }
    in
    let span_ot = Obs_eio.with_context svc_ot [ ("correlation_id", corr_id) ] in
    let trace_ctx =
      Obs_eio.with_span span_ot "receive_charge" (fun span ->
    ...
    in
    Printf.printf
      "[charge-svc]    received   charge=%-20s  customer=%-10s  %5d %s\n%!"
  ...
  in
  List.iter
    (fun (customer_id, amount_cents, currency) ->
      let corr_id = new_corr_id () in
      let body =
        Printf.sprintf {|{"amount_cents":%d,"customer_id":%S,"currency":%S}|}
          amount_cents customer_id currency
      in
      let status =
        http_post env ~sw ~port ~path:"/charges"
          ~headers:[ ("x-correlation-id", corr_id) ]
          ~body ()
      in
      Printf.printf
        "[client]        POST /charges customer=%-10s  %5d %s  -> HTTP %d  \
         corr=%s\n\
         %!"
  ...
  say "waiting for notify-worker to process all %d events (up to 20s) ..."
    charges_count;
  (match
  ...
  | Some pool -> (
      ...
      | Ok rows ->
          List.iter
            (fun (r : Notification.t) ->
              Printf.printf "  %-20s  %-10s  %5d %s\n" r.charge_id r.customer_id
  ...
  | Some url -> (
      match
        Obs_prometheus.push ~net:env#net ~clock:env#clock ~url ~job:"venus"
          render
      with
      | Ok () -> say "metrics pushed to %s" url
      | Error e ->
          Printf.eprintf "[venus] push failed: %s\n%!"
