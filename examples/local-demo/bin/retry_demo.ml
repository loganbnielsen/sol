(** Sol retry-topics demo
    ...
    KAFKA_BROKERS=localhost:9092 dune exec
    examples/local-demo/bin/retry_demo.exe *)
...
let () =
  ...
  let svc =
    ...
    | Ok s -> s
    | Error e ->
        failwith ("kafka_service.create: " ^ Kafka_service.error_to_string e)
  in
  let topic =
    match
      Kafka_service.register svc ~net:env#net ~clock:env#clock (module Job)
    with
    | Ok t -> t
    | Error e ->
        failwith ("kafka_service.register: " ^ Kafka_service.error_to_string e)
  in
  ...
  (match
     Eio.Time.with_timeout env#clock 30.0 (fun () ->
         Ok (Eio.Promise.await all_done_p))
   with
  | Error `Timeout ->
  ...
  | Ok () ->
      ...
      Printf.printf "\n%s\n" sep;
      say "all %d/%d jobs completed in %.1fs." (Atomic.get completed) total_jobs
        elapsed;
      Printf.printf "  reliable jobs: processed immediately on main consumer\n";
      Printf.printf
        "  flakey  jobs:  acked on main, retried via sol-demo-jobs-retry\n";
      Printf.printf "%s\n%!" sep);
