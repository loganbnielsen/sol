...
let parse_request_path path =
  ...
  in
  if has_double_slash 0 then None else Some (split_path path, trailing_slash path)
;;
...
let parse_pattern source =
  ...
  if source = ""
  then fail "empty pattern"
  else if source.[0] <> '/'
  then fail "pattern must start with /"
  else (
    ...
    | Some (raw_segments, trailing_slash) ->
      let rec parse_segments acc = function
        | [] -> Ok (List.rev acc)
        | seg :: rest ->
          if String.length seg > 0 && seg.[0] = ':'
          then (
            let name = String.sub seg 1 (String.length seg - 1) in
            if valid_param_name name
            then parse_segments (Param name :: acc) rest
            else fail (Printf.sprintf "invalid parameter segment %S" seg))
          else if String.contains seg ':'
          then fail (Printf.sprintf "literal segment %S must not contain :" seg)
          else parse_segments (Literal seg :: acc) rest
      in
      Result.map
