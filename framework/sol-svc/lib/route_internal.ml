let match_path (pattern : Route.pattern) request_path =
  ...
  | Some (req_segs, req_ts) ->
    if pattern.Route.trailing_slash <> req_ts
    then None
    else (
      let rec go pats reqs acc =
        match pats, reqs with
        | [], [] -> Some (List.rev acc)
        | Route.Param key :: ps, r :: rs -> go ps rs ((key, Uri.pct_decode r) :: acc)
        | Route.Literal p :: ps, r :: rs -> if p = r then go ps rs acc else None
        | _ -> None
      in
