...
let validate_api_key ~read_api_key headers =
  ...
  | Some provided -> (
      ...
      | None ->
          Error
            (`Server_error
               "API key not configured (set SOL_API_KEY or SOL_API_KEY_FILE)")
      | Some "" ->
          Error
            (`Server_error
               "API key is empty (set SOL_API_KEY or SOL_API_KEY_FILE)")
      | Some _ when provided = "" -> Error (`Unauthorized "Invalid API key")
      | Some expected ->
          if constant_time_equal provided expected then
            let key_id =
              String.sub provided 0 (min 8 (String.length provided))
            in
...
let base64url_decode s =
  ...
  in
  match Base64.decode pad with Ok s -> Some s | Error _ -> None
...
let decode_jwt_payload parts =
  ...
  |> Option.to_result
       ~none:(`Unauthorized "Malformed JWT: cannot decode payload")
...
let token_sub json =
  match Yojson.Safe.Util.member "sub" json with `String s -> s | _ -> ""
...
let fetch_jwks_over_https ~env url =
  match
    Https_eio.request ~net:env#net ~clock:env#clock ~timeout:10.0 ~meth:`GET
      ~url
  ...
  | _ ->
      Mutex.protect jwks_cache_mutex (fun () ->
          ...
          | _ -> (
              ...
              | Ok jwks ->
                  Atomic.set jwks_cache
                    (Some { url; fetched_at = Unix.gettimeofday (); jwks });
                  Ok jwks
              | Error _ as e -> e))
...
let verify_with_key_source ?fetch_jwks ~kid parsed key_source =
  ...
  | Jwks_static doc -> (
      ...
      | Eio.Cancel.Cancelled _ as exn -> raise exn
      | exn ->
          Error (`Server_error ("JWKS parse failed: " ^ Printexc.to_string exn))
      )
  | Jwks_url url -> (
      ...
      | None ->
          Error
            (`Server_error
               "Jwks_url configured but no JWKS fetcher was provided")
      | Some fetch_jwks -> (
...
let validate_verified_jwt ?fetch_jwks vconfig ~scopes headers =
  ...
  let* verified =
    verify_with_key_source ?fetch_jwks ~kid parsed vconfig.key_source
  in
  ...
  Ok
    {
      principal =
        User { sub = token_sub json; scopes = token_scopes; claims = json };
    }
