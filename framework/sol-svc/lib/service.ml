...
let read_body_limited headers (body : Cohttp_eio.Body.t) max_bytes =
  let too_large_from_header =
    ...
    | None -> false
    | Some s -> (
        try int_of_string (String.trim s) > max_bytes with _ -> false)
  in
...
let dispatch ?read_api_key ?fetch_jwks ~routes ~metrics_renderer ~metrics_auth
  ...
  | Some meth -> (
      ...
      if Route.parse_request_path path = None then
        ...
        let observe lbl =
          match route_observer with Some f -> f lbl | None -> ()
        in
        let builtin =
          ...
          | `GET, "/metrics" -> (
              ...
              | Some render ->
                  let result =
                    let* _ =
                      auth_result ?read_api_key ?fetch_jwks metrics_auth headers
                    in
                    Ok
                      (Response.ok
                         ~headers:
        ...
        | None -> (
            ...
            | Found (route, params) -> (
                ...
                let result =
                  let* auth_ctx =
                    auth_result ?read_api_key ?fetch_jwks route.Route.auth
                      headers
                  in
                  ...
                  | (Out_of_memory | Stack_overflow | Sys.Break) as exn ->
                      raise exn
                  | exn ->
                      Printf.eprintf "sol-svc: handler exception: %s\n%!"
...
let auth_uses_api_key = function `Api_key -> true | `Public | `Jwt _ -> false
...
let api_key_reader ~env ~required =
  ...
  | Some path, _ -> (
      ...
      | exn ->
          Error
            (`Config
               ("could not read SOL_API_KEY_FILE " ^ path ^ ": "
              ^ Printexc.to_string exn)))
  ...
  | None, _ when required ->
      Error
        (`Config
           "API key auth configured but SOL_API_KEY/SOL_API_KEY_FILE is not set")
  | None, _ -> Ok (fun () -> None)
      ...
      | Some o ->
          let req_count, req_duration =
            Obs_eio.register_counter_and_histogram o
              ...
              ~counter_help:
                "Total HTTP requests by method, route, and HTTP status class"
    ...
    Ok
      (try
         Eio.Switch.run (fun sw ->
             ...
             let socket =
               Eio.Net.listen ~sw ~reuse_addr:true ~backlog:128 env#net
             ...
             let callback _conn req body =
               ...
               in
               let sol_resp =
                 dispatch ~fetch_jwks ~routes:H.routes ~metrics_renderer
                   ~metrics_auth ~read_api_key ~max_body_bytes ?route_observer
                   req body
               in
               ...
               | Some (req_count, req_duration), Some t0 ->
                   ...
                   let sc =
                     string_of_int (sol_resp.Response.status / 100) ^ "xx"
                   in
                   req_count
                     ~labels:
                       [
                     ...
                     1;
                   req_duration
                     ~labels:[ ("method", meth_str); ("route", route) ]
                     dt
               | _ -> ());
