...
let test_constructor_rejects_malformed_pattern () =
  Alcotest.check_raises "constructor validates pattern"
    (Invalid_argument
       "invalid route pattern \"users\": pattern must start with /") (fun () ->
      ignore (Route.get "users" ~auth:`Public dummy_handler))
...
let test_percent_decode_passthrough () =
  Alcotest.(check string)
    "plain text unchanged" "hello" (Uri.pct_decode "hello")
...
let () =
  Alcotest.run "routing"
    [
      ( "pattern",
        [
          Alcotest.test_case "typed pattern segments" `Quick
            test_pattern_segments;
          Alcotest.test_case "missing leading slash" `Quick
            test_pattern_rejects_missing_leading_slash;
          Alcotest.test_case "double slash pattern" `Quick
            test_pattern_rejects_double_slash;
          Alcotest.test_case "empty param" `Quick
            test_pattern_rejects_empty_param;
          Alcotest.test_case "constructor validates" `Quick
            test_constructor_rejects_malformed_pattern;
        ] );
      ( "parse_request_path",
        [
          Alcotest.test_case "valid path" `Quick test_parse_valid_path;
          Alcotest.test_case "double slash rejected" `Quick
            test_parse_double_slash_rejected;
          Alcotest.test_case "interior // rejected" `Quick
            test_parse_interior_double_slash_rejected;
      ...
      ( "percent_decode",
        [
          ...
          Alcotest.test_case "%2f lowercase → /" `Quick
            test_percent_decode_lowercase;
          Alcotest.test_case "plain text unchanged" `Quick
            test_percent_decode_passthrough;
          Alcotest.test_case "malformed %GG unchanged" `Quick
            test_percent_decode_malformed;
          Alcotest.test_case "+ not decoded as space" `Quick
            test_percent_decode_plus_not_space;
        ] );
    ]
