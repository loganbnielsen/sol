...
let test_api_key_valid () =
  ...
  match
    Test_auth_internal.validate ~read_api_key `Api_key (api_key "secretkey123")
  with
...
let test_api_key_wrong () =
  let read_api_key () = Some "secretkey123" in
  match
    Test_auth_internal.validate ~read_api_key `Api_key (api_key "wrongkey")
  with
...
let test_api_key_empty_secret_fails_closed () =
  ...
  | Error _ ->
      Alcotest.fail "expected Server_error for empty configured API key"
...
let test_jwt_valid () =
  ...
  match
    Test_auth_internal.validate (jwt_cfg [ "read"; "write" ]) (bearer tok)
  with
...
let test_jwt_malformed () =
  match
    Test_auth_internal.validate (jwt_cfg []) (bearer "not.a.jwt.at.all.extra")
  with
  | Error (`Unauthorized _) -> ()
  | _ -> Alcotest.fail "expected Unauthorized"
...
let test_jwt_wrong_bearer_scheme () =
  match
    Test_auth_internal.validate (jwt_cfg [])
...
let test_jwt_payload_not_base64 () =
  match
    Test_auth_internal.validate (jwt_cfg []) (bearer "header.%.signature")
  with
  | Error (`Unauthorized _) -> ()
  | _ -> Alcotest.fail "expected Unauthorized"
...
let sign_hs256 ?(sub = "user1") ?(scopes = [ "read" ]) ?(iss = issuer)
    ?(aud = audience) ?(exp_offset = 3600.0) () =
...
let hs256_verified_cfg ?(scopes = []) ?(algorithms = [ `HS256 ])
          ...
          Verified_signature_required
            {
              ...
              audience;
              algorithms;
              key_source = Hs256_secret hs256_secret;
            };
      }
...
let sign_rs256 ?(sub = "user1") ?(scopes = [ "read" ]) ?(iss = issuer)
    ?(aud = audience) ?(exp_offset = 3600.0) () =
  let payload = claims_json ~sub ~scopes ~iss ~aud ~exp_offset in
...
let rs256_verified_cfg ?(scopes = []) ?(algorithms = [ `RS256 ])
    ?(issuer = issuer) ?(audience = audience) () =
  `Jwt
    Auth.
      {
          ...
          Verified_signature_required
            {
              ...
              audience;
              algorithms;
              key_source = Jwks_static rsa_jwks_doc;
            };
      }
...
let test_jwt_verified_jwks_fetch_failure_fails_closed () =
  ...
  match
    Test_auth_internal.validate ~fetch_jwks:failing_fetch (jwks_url_cfg ())
      (bearer tok)
  with
  | Error (`Server_error _) -> ()
  | Ok _ ->
      Alcotest.fail "must not fall back to unverified on JWKS fetch failure"
...
let test_jwt_verified_jwks_url_without_fetcher_fails_closed () =
  ...
  | Ok _ ->
      Alcotest.fail
        "must not fall back to unverified with no fetch_jwks configured"
  | Error _ -> Alcotest.fail "expected Server_error (fail closed)"
    ...
    [
      ...
      ( "api_key",
        [
          ...
          Alcotest.test_case "injected reader" `Quick
            test_api_key_uses_injected_reader;
          Alcotest.test_case "wrong key → 401" `Quick test_api_key_wrong;
          Alcotest.test_case "missing header → 401" `Quick
            test_api_key_missing_header;
          Alcotest.test_case "no reader fails closed" `Quick
            test_api_key_without_reader_fails_closed;
          Alcotest.test_case "empty secret fails closed" `Quick
            test_api_key_empty_secret_fails_closed;
        ] );
      ( "jwt_unverified",
        [
          Alcotest.test_case "valid token" `Quick test_jwt_valid;
          Alcotest.test_case "superset scopes → ok" `Quick
            test_jwt_superset_scopes;
          ...
          Alcotest.test_case "wrong bearer → 401" `Quick
            test_jwt_wrong_bearer_scheme;
          Alcotest.test_case "bad payload b64 → 401" `Quick
            test_jwt_payload_not_base64;
          Alcotest.test_case "bad payload JSON → 401" `Quick
            test_jwt_payload_not_json;
          Alcotest.test_case "missing header → 401" `Quick
            test_jwt_missing_header;
        ] );
      ( "jwt_verified",
        [
          Alcotest.test_case "HS256 valid → ok" `Quick
            test_jwt_verified_hs256_valid;
          Alcotest.test_case "RS256 valid (JWKS) → ok" `Quick
            test_jwt_verified_rs256_valid;
          Alcotest.test_case "tampered signature → 401" `Quick
            test_jwt_verified_tampered_signature;
          Alcotest.test_case "alg not in allowlist → 401" `Quick
            test_jwt_verified_wrong_alg_rejected;
          Alcotest.test_case "wrong issuer → 401" `Quick
            test_jwt_verified_wrong_issuer;
          Alcotest.test_case "wrong audience → 401" `Quick
            test_jwt_verified_wrong_audience;
          Alcotest.test_case "expired → 401" `Quick test_jwt_verified_expired;
          Alcotest.test_case "missing scope → 403" `Quick
            test_jwt_verified_missing_scope;
          Alcotest.test_case "JWKS fetch failure fails closed → 500" `Quick
          ...
          Alcotest.test_case "malformed static JWKS fails closed → 500" `Quick
