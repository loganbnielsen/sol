  ...
  if nl > sl then false
    ...
    for i = 0 to sl - nl do
      if not !found then begin
        let rec eq j =
          j >= nl
          || String.unsafe_get s (i + j) = String.unsafe_get needle j
             && eq (j + 1)
        in
...
let make_jwt ?(scopes = [ "read" ]) () =
  ...
  let payload =
    Printf.sprintf {|{"sub":"u1","scope":"%s","exp":%d}|} scope exp
  in
  let b64 =
  ...
  let routes =
    [
      ...
      Route.get "/users/:id" ~auth:`Public (fun req ->
          Response.json
            (Printf.sprintf {|{"id":"%s"}|} (Request.param_exn req "id")));
    ]
end
...
let http_call env ~sw ~port ~meth ~path ?(headers = []) ?(body = "") () =
  ...
  let body_val =
    if body = "" then None else Some (Cohttp_eio.Body.of_string body)
  in
  let resp, resp_body =
...
let test_healthz env () =
  Switch.run (fun sw ->
      with_server env ~sw (fun port ->
          let status, body =
            http_call env ~sw ~port ~meth:`GET ~path:"/healthz" ()
          in
...
let test_not_found env () =
  Switch.run (fun sw ->
      with_server env ~sw (fun port ->
          let status, _ =
            http_call env ~sw ~port ~meth:`GET ~path:"/does-not-exist" ()
          in
...
let test_method_not_allowed env () =
  Switch.run (fun sw ->
      with_server env ~sw (fun port ->
          (* /hello is GET only *)
          let status, _ =
            http_call env ~sw ~port ~meth:`DELETE ~path:"/hello" ()
          in
...
let test_public_route env () =
  Switch.run (fun sw ->
      with_server env ~sw (fun port ->
          let status, _ =
            http_call env ~sw ~port ~meth:`GET ~path:"/hello" ()
          in
          Alcotest.(check int) "status 200" 200 status))
...
let test_path_param env () =
  Switch.run (fun sw ->
      with_server env ~sw (fun port ->
          let status, body =
            http_call env ~sw ~port ~meth:`GET ~path:"/users/42" ()
          in
          Alcotest.(check int) "status 200" 200 status;
          Alcotest.(check bool)
            "contains id" true
...
let test_echo_body env () =
  Switch.run (fun sw ->
      with_server env ~sw (fun port ->
          let status, body =
            http_call env ~sw ~port ~meth:`POST ~path:"/echo"
              ~body:"hello world" ()
          in
          Alcotest.(check int) "status 200" 200 status;
          Alcotest.(check bool)
...
let test_jwt_no_token env () =
  Switch.run (fun sw ->
      with_server env ~sw (fun port ->
          let status, _ =
            http_call env ~sw ~port ~meth:`GET ~path:"/protected" ()
          in
...
let test_metrics_no_renderer env () =
  Switch.run (fun sw ->
      with_server env ~sw (fun port ->
          let status, _ =
            http_call env ~sw ~port ~meth:`GET ~path:"/metrics" ()
          in
          Alcotest.(check int) "status 404" 404 status))
...
let test_metrics_counter env () =
  Switch.run (fun sw ->
      with_server_obs env ~sw (fun port render ->
          ...
          Alcotest.(check bool)
            "requests_total counter present" true
            (contains "sol_svc_requests_total" output);
          Alcotest.(check bool)
            "route label in output" true
            (contains {|route="/hello"|} output);
          Alcotest.(check bool)
            "status_class label in output" true
...
let test_metrics_duration env () =
  Switch.run (fun sw ->
      with_server_obs env ~sw (fun port render ->
          ...
          let output = render () in
          Alcotest.(check bool)
            "duration histogram present" true
...
let test_metrics_route_pattern_label env () =
  ...
  Switch.run (fun sw ->
      with_server_obs env ~sw (fun port render ->
          ...
          let output = render () in
          Alcotest.(check bool)
            "pattern label present" true
            (contains {|route="/users/:id"|} output);
          Alcotest.(check bool)
            "concrete value 42 not a label" false
            (contains {|route="/users/42"|} output);
          Alcotest.(check bool)
            "concrete value 999 not a label" false
...
let test_unauth_large_body_gets_401 env () =
  ...
  Switch.run (fun sw ->
      with_small_body_server env ~sw (fun port ->
          ...
          let status, _ =
            http_call env ~sw ~port ~meth:`POST ~path:"/protected-upload"
              ~body:big_body ()
          in
        ...
        [
          ( "built-ins",
            [
              ...
              Alcotest.test_case "GET /metrics, no renderer → 404" `Quick
                (test_metrics_no_renderer env);
            ] );
          ( "routing",
            [
              Alcotest.test_case "unknown path → 404" `Quick
                (test_not_found env);
              Alcotest.test_case "wrong method → 405" `Quick
                (test_method_not_allowed env);
              Alcotest.test_case "public route → 200" `Quick
                (test_public_route env);
              Alcotest.test_case "path param extracted" `Quick
                (test_path_param env);
              Alcotest.test_case "POST body echoed" `Quick (test_echo_body env);
            ] );
          ( "auth",
            [
              Alcotest.test_case "JWT route, no token → 401" `Quick
                (test_jwt_no_token env);
              Alcotest.test_case "JWT route, valid token → 200" `Quick
                (test_jwt_valid_token env);
              Alcotest.test_case "JWT route, wrong scope → 403" `Quick
          ...
          ( "resilience",
            [
              ...
              Alcotest.test_case "external stop on listen" `Quick
                (test_external_stop_on_listen env);
            ] );
          ( "metrics",
            [
              Alcotest.test_case "requests counter with route label" `Quick
                (test_metrics_counter env);
              Alcotest.test_case "duration histogram present" `Quick
                (test_metrics_duration env);
              Alcotest.test_case "route label uses pattern not path" `Quick
                (test_metrics_route_pattern_label env);
            ] );
          ( "auth_before_body",
            [
              Alcotest.test_case "unauth + large body → 401 not 413" `Quick
                (test_unauth_large_body_gets_401 env);
              Alcotest.test_case "auth ok + oversized body → 413" `Quick
                (test_auth_oversized_body_gets_413 env);
              Alcotest.test_case "public route + oversized body → 413" `Quick
