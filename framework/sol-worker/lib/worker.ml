...
let run_error_to_string = function
  ...
  | `Register e ->
      "sol-worker: register failed: " ^ Kafka_service.error_to_string e
  | `Consume e -> (
      match e with
...
module Make_with_test_seam (W : WORKER) = struct
  ...
  let run ~(env : (_, _, _, _) Sol_env.timed) ~config ?ot
      ?(metrics_port = default_metrics_port) ?on_ready ?stop ?max_messages
      ?(retry_strategy = Kafka_service.default_retry_strategy)
      ?test_consume_loop () =
    ...
    in
    let remaining =
      match max_messages with Some n -> Some (ref n) | None -> None
    in
    ...
    let result =
      Eio.Switch.run (fun sw ->
          ...
          let handler msg ~ack ~trace_ctx =
            ...
            in
            if stop_requested () || limit_reached then Kafka.Consumer.Stop
            else begin
              ...
              | Ok () -> (
                  ...
                  let advance () =
                    ...
                    | Some r ->
                        decr r;
                        if !r <= 0 then Kafka.Consumer.Stop
                 ...
                 so it's only escalated to Kafka.Consumer.Error when fatal. *)
                  ...
                  | Error e ->
                      ...
                      | None -> ()
                      | Some o ->
                          Obs_eio.log_standalone o
                            (if Kafka.Error.is_fatal e then Obs_eio.Error
                             else Obs_eio.Warn)
                            ~fields:[ ("error", Kafka.Error.to_string e) ]
                            (if Kafka.Error.is_fatal e then
                               "sol-worker: fatal ack failure, stopping \
                                consumer"
                             else
                               "sol-worker: ack failed, offset not committed; \
                                message eligible for redelivery"));
                      if Kafka.Error.is_fatal e then Kafka.Consumer.Error e
                      else advance ())
            end
          in
          ...
          | None ->
              let* svc =
                Kafka_service.create config ~sw
                |> Result.map_error (fun msg -> `Create msg)
              in
              let* topic =
              ...
              in
              Kafka_service.consume_partitioned svc topic ~group_id:W.group_id
                ~sw ~clock:env#clock ?on_ready ~retry_strategy ~on_retry ?ot
                ~handler ()
            ...
            List.iter
              (fun (partition, e) ->
                Obs_eio.log_standalone o Obs_eio.Error
                  ~fields:
                    [
                      ("partition", Int32.to_string partition);
                      ("error", Kafka.Error.to_string e);
                    ]
