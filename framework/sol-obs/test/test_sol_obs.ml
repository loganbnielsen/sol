...
let contains haystack needle =
  ...
  if nl = 0 then true
    ...
    let rec go i =
      i <= hl - nl && (String.sub haystack i nl = needle || go (i + 1))
    in
    go 0
...
let with_env pairs f =
  ...
  Fun.protect
    ~finally:(fun () -> List.iter (fun (k, v) -> Unix.putenv k v) saved)
    f
...
let test_default_env_logs_and_counts_without_network () =
  ...
  with_env
    ...
    (fun () ->
      ...
      Sol_obs.with_span obs "op" (fun sp ->
          Sol_obs.log sp Sol_obs.Info "inside span");
      let reqs =
        Sol_obs.counter obs ~name:"requests_total" ~help:"total requests"
          ~label_names:[]
      in
      reqs 1;
      let rendered = Sol_obs.metrics_renderer obs () in
      Alcotest.(check bool)
        "rendered output mentions the registered counter" true
...
let test_gauge_and_histogram_round_trip () =
  ...
  with_env
    ...
    (fun () ->
      ...
      in
      let queue_depth =
        Sol_obs.gauge obs ~name:"queue_depth" ~help:"items queued"
          ~label_names:[]
      in
      queue_depth 3.0;
      let latency =
        Sol_obs.histogram obs ~name:"op_seconds" ~help:"op latency"
          ~label_names:[]
      in
      latency 0.05;
      let rendered = Sol_obs.metrics_renderer obs () in
        ...
        (fun () ->
          let obs =
            Sol_obs.of_env ~net:env#net ~clock:env#clock
              ~mono_clock:env#mono_clock ~service:"test-svc" ()
          in
          Sol_obs.log_info obs "pushed to loki";
          let body = Eio.Promise.await body_promise in
          Alcotest.(check bool)
            "push body mentions the log message" true
  ...
  with_mock_server env (fun ~port ~body_promise ->
      with_env
        ...
        (fun () ->
          ...
          let body = Eio.Promise.await body_promise in
          Alcotest.(check bool)
            "push body carries the team stream label" true
  ...
  with_mock_server env (fun ~port ~body_promise ->
      with_env
        ...
        (fun () ->
          ...
          let body = Eio.Promise.await body_promise in
          Alcotest.(check bool)
            "OTLP push body is non-empty" true
...
let test_metrics_renderer_matches_backend_and_renderer () =
  ...
  with_env
    ...
    (fun () ->
      ...
      in
      let reqs =
        Sol_obs.counter obs ~name:"acc_total" ~help:"h" ~label_names:[]
      in
      reqs 1;
...
let test_with_context_does_not_mutate_original () =
  ...
  with_env
    ...
    (fun () ->
      ...
      Alcotest.(check bool)
        "obs_eio handles are distinct values" true
...
let test_current_trace_context_links_child_span () =
  ...
  with_env
    ...
    (fun () ->
      ...
      in
      let parent_ctx =
        Sol_obs.with_span obs "parent" Sol_obs.current_trace_context
      in
...
let test_trace_id_string_is_32_hex_chars () =
  ...
  with_env
    ...
    (fun () ->
      ...
      Alcotest.(check bool)
        "every character is lowercase hex" true
        (String.for_all
    ...
    [
      ( "defaults",
        [
          ...
          test_case "gauge and histogram round trip" `Quick
            test_gauge_and_histogram_round_trip;
        ] );
      ( "env-driven backends",
        [
          test_case "LOKI_URL wires the Loki backend" `Quick
          ...
          test_case "TEMPO_URL wires the Tempo backend" `Quick
            test_tempo_url_wires_tempo_backend;
        ] );
      ( "accessors",
        [
          test_case "metrics_renderer matches backend_and_renderer's renderer"
            `Quick test_metrics_renderer_matches_backend_and_renderer;
          test_case "with_context derives without mutating the original" `Quick
            test_with_context_does_not_mutate_original;
        ] );
      ( "trace context",
        [
          test_case "current_trace_context links a child span to its parent"
            `Quick test_current_trace_context_links_child_span;
