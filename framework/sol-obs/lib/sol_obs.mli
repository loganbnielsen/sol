(** Sol's app-facing observability facade.
    ...
    [Obs_eio] stays the neutral event/backend core; this module owns Sol's
    ...
    handlers call. Generated code should depend on [sol_obs], not directly
...
(** Reads [LOKI_URL] and [TEMPO_URL] from the environment and composes
    ...
    (Sol's local-dev default — see the top-level CLAUDE.md's "dev mirrors
    prod" principle: the same [of_env] call works unmodified against
    `sol dev up`'s real Loki/Tempo instances once those env vars are set by
    the platform). *)
...
(** What [sol-svc]/[sol-worker] register per-request/per-message metrics
    against internally. *)

val metrics_renderer : t -> unit -> string
(** What [sol-svc]/[sol-worker] render their built-in [/metrics] endpoint
...
(** What [sol-fn] builds its own job-scoped [Obs_eio.t] from internally —
