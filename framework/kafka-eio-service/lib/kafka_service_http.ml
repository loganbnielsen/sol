type body_request = { content_type : string; body : string }
...
let http_do net ~clock request =
  ...
  |> Result.map_error (fun e ->
      "kafka_service: " ^ Https_eio.request_error_to_string e)

let http_post net ~clock ~base_url ~path ~content_type ~body =
  http_do net ~clock
    (With_body
       { meth = `POST; base_url; path; body_request = { content_type; body } })

let http_put net ~clock ~base_url ~path ~content_type ~body =
  http_do net ~clock
    (With_body
       { meth = `PUT; base_url; path; body_request = { content_type; body } })
