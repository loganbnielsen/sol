...
module Schema = struct
  ...
  let decode_compatibility_response =
    Kafka_service_schema.decode_compatibility_response

  let decode_registration_response =
...
module Admin = struct
  ...
  let topic_partition_error_to_string =
    Kafka_service_intf.topic_partition_error_to_string
...
let register : type a.
 ...
 fun svc ~net ~clock (module M) ->
  ...
  let partition_guard () =
    match
      Kafka_service_intf.query_topic_partitions net ~clock
        ~admin_url:svc.admin_url ~topic_name:raw_topic_name
    with
    | Error e ->
        Error
          (Topic_metadata
  ...
  let* () =
    Kafka_service_intf.ensure_topic svc.producer ~topic_name:raw_topic_name
  ...
  let* schema_id =
    Kafka_service_schema.register_schema net ~clock
      ~registry_url:svc.schema_registry_url ~topic_name:raw_topic_name
      ~schema:M.schema
    |> Result.map_error (fun msg -> Schema_registry (M.topic_name, msg))
  in
  (match
     Kafka_service_schema.set_subject_compatibility net ~clock
       ~registry_url:svc.schema_registry_url ~topic_name:raw_topic_name
   with
  | Error e ->
...
let consume svc topic ~group_id ~sw ~clock ?(on_ready = ignore)
    ?(on_decode_error = default_on_decode_error) ?ot ~handler () =
  let on_decode_error =
    Kafka_service_intf.wrap_on_decode_error ~ot
  ...
  | Ok consumer ->
      ...
      in
      let result =
        Kafka.Consumer.consume consumer ~handler:decode_and_handle ()
      in
      Kafka.Consumer.close consumer;
      result

let consume_partitioned svc topic ~group_id ~sw ~clock ?(on_ready = ignore)
    ?(on_decode_error = default_on_decode_error)
    ?(retry_strategy = default_retry_strategy)
    ?(on_retry = fun ~partition:_ ~attempt:_ ~delay_s:_ -> ()) ?ot ~handler () =
  let on_decode_error =
    Kafka_service_intf.wrap_on_decode_error ~ot
      ~topic_name:(topic_name_to_string topic.name)
      on_decode_error
  ...
  | In_memory retry -> (
      ...
      | Ok consumer ->
          ...
          in
          let result =
            Kafka.Consumer.consume_partitioned consumer ~sw ~clock ~retry
              ~on_retry ~handler:decode_and_handle ()
