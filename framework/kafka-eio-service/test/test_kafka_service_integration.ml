...
module PaymentEvent = struct
  ...
  let decode = function
    | `Assoc fields -> (
        match
          ( List.assoc_opt "payment_id" fields,
...
let test_schema_check_new_topic () =
  ...
  let module Fresh = struct
    ...
    let topic_name =
      Kafka_service.topic_name_exn
        (Printf.sprintf "sol-svc-fresh-%05d" fresh_id)
  ...
  match
    Kafka_service.Schema.check ~net:env#net ~clock:env#clock ~registry_url
      (module Fresh)
  with
  | Error e ->
      Alcotest.failf "expected Ok for new topic, got Error: %s"
...
let test_schema_check_compatible () =
  ...
  | Error e ->
      Alcotest.failf "create failed: %s" (Kafka_service.error_to_string e)
  | Ok svc -> (
      match
      ...
      with
      | Error e ->
          Alcotest.failf "register failed: %s" (Kafka_service.error_to_string e)
      | Ok _ -> (
          match
            Kafka_service.Schema.check ~net:env#net ~clock:env#clock
              ~registry_url
              (module PaymentEvent)
          with
          | Error e ->
              Alcotest.failf "compatible schema returned Error: %s"
                (Kafka_service.error_to_string e)
          | Ok () -> ()))
...
let test_schema_check_incompatible () =
  ...
  | Ok svc -> (
      ...
      | Ok _ -> (
          ...
          with
          | Ok () ->
              Alcotest.fail "expected Error for incompatible schema, got Ok"
          | Error _ -> ()))
...
let test_schema_check_all_fails_fast () =
  ...
  | Ok svc -> (
      ...
      | Ok _ -> (
          ...
          | Ok () ->
              Alcotest.fail
                "expected Error for list containing incompatible schema"
          | Error _ -> ()))
...
let test_publish_consume_roundtrip () =
  ...
  | Ok svc -> (
      ...
      | Ok topic -> (
          let group_id =
            Printf.sprintf "sol-test-roundtrip-%d-%d" (Unix.getpid ())
              (Random.int 9999)
          in
          ...
          Eio.Fiber.fork ~sw (fun () ->
              ignore
                (Kafka_service.consume svc topic ~group_id ~sw ~clock:env#clock
          ...
          | Error `Timeout ->
              Alcotest.fail
                "timed out waiting for consumer partition assignment (on_ready)"
          | Ok () -> ());
          let expected =
            PaymentEvent.{ payment_id = "pay-e2e-001"; amount_cents = 9900 }
          in
          (match
             Eio.Promise.await (Kafka_service.publish svc topic expected)
           with
          | Error e ->
          ...
          with
          | Error `Timeout ->
              Alcotest.fail "timed out waiting for consumed message"
          | Ok msg ->
              Alcotest.(check string)
                "payment_id" expected.payment_id msg.PaymentEvent.payment_id;
              Alcotest.(check int)
...
let test_consume_partitioned_reports_partition_error () =
  ...
  | Ok svc -> (
      ...
      | Ok topic -> (
          (match
             Eio.Promise.await
               (Kafka_service.publish svc topic PartitionFailEvent.{ n = 1 })
           with
          | Error e ->
              Alcotest.failf "publish failed: %s" (Kafka.Error.to_string e)
          | Ok () -> ());
          let group_id =
            Printf.sprintf "sol-test-partfail-%d-%d" (Unix.getpid ())
              (Random.int 9999)
          in
          let retry_strategy =
          ...
          let result =
            Eio.Time.with_timeout env#clock 20.0 (fun () ->
                Ok
                  (Kafka_service.consume_partitioned svc topic ~group_id ~sw
                     ~clock:env#clock ~retry_strategy
          ...
          | Ok (Ok ()) ->
              Alcotest.fail "expected the failing handler to exhaust retries"
          ...
          | Ok (Error (Kafka_service.Partition_errors errs)) ->
              Alcotest.(check bool)
                "at least one partition error reported" true (errs <> [])))
...
let test_decode_error_callback () =
  ...
  | Ok svc -> (
      ...
      | Ok topic -> (
          let group_id =
            Printf.sprintf "sol-test-decode-err-%d-%d" (Unix.getpid ())
              (Random.int 9999)
          in
          ...
          | Error e ->
              Alcotest.failf "raw producer create failed: %s"
                (Kafka.Error.to_string e)
          | Ok producer ->
              ...
              (match
                 Eio.Promise.await
                   (Kafka.Producer.produce_await producer
                      ~topic:
                        (Kafka_service.topic_name_to_string
                           RawTestEvent.topic_name)
                      ~value:(Some raw) ())
               with
              | Error e ->
                  Alcotest.failf "raw publish failed: %s"
                    (Kafka.Error.to_string e)
              | Ok () -> ());
          ...
          with
          | Error `Timeout ->
              Alcotest.fail "timed out waiting for decode error callback"
          | Ok _ -> ()))
    ...
    [
      ( "schema_check",
        [
          ...
          test_case "compatible schema returns ok" `Slow
            test_schema_check_compatible;
          test_case "incompatible schema returns error" `Slow
            test_schema_check_incompatible;
          test_case "check_all fails fast" `Slow
      ...
      ( "consume_partitioned",
        [
          test_case "reports partition error, not a collapsed single error"
            `Slow test_consume_partitioned_reports_partition_error;
        ] );
      ( "error_handling",
        [
