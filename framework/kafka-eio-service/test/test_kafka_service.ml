...
let test_wire_schema_id_big_endian () =
  ...
  let encoded =
    Kafka_service.Confluent_wire.encode ~schema_id:0x01020304 (`String "x")
  in
...
let check_uri msg ~expected_host ~expected_port ~expected_scheme url =
  ...
  Alcotest.(check (option string))
    (msg ^ " host") (Some expected_host) (Uri.host u);
  Alcotest.(check (option int)) (msg ^ " port") expected_port (Uri.port u);
  Alcotest.(check (option string))
    (msg ^ " scheme") (Some expected_scheme) (Uri.scheme u)

let test_parse_url () =
  check_uri "http://localhost:8081" ~expected_host:"localhost"
    ~expected_port:(Some 8081) ~expected_scheme:"http" "http://localhost:8081";
  check_uri "http://localhost:9644" ~expected_host:"localhost"
    ~expected_port:(Some 9644) ~expected_scheme:"http" "http://localhost:9644";
  check_uri "http no explicit port" ~expected_host:"localhost"
    ~expected_port:None ~expected_scheme:"http" "http://localhost"

let test_parse_url_https () =
  check_uri "https:// with explicit port" ~expected_host:"registry.example.com"
    ~expected_port:(Some 8081) ~expected_scheme:"https"
    "https://registry.example.com:8081";
  check_uri "https:// no explicit port" ~expected_host:"registry.confluent.io"
    ~expected_port:None ~expected_scheme:"https" "https://registry.confluent.io"
...
let test_retry_metadata_rejects_malformed_headers () =
  ...
  in
  let valid =
    [ ("X-Sol-Attempt", Some "2"); ("X-Sol-Retry-At", Some "123.5") ]
  in
...
let test_retry_publish_then_ack_failure_is_error () =
  let acked = ref 0 in
  let publish_raw ~target_topic:_ ~attempt:_ ~raw_bytes:_ ~headers:_ ~delay_s:_
      ~partition:_ =
    Ok ()
  in
  let ack () =
  ...
  let action =
    Kafka_service.Retry_topics.Forward_retry { target; delay_s = 1.0 }
  in
  match
    Kafka_service.Retry_topics.execute_action action ~raw_msg:(raw_retry_msg ())
      ~attempt:1 ~publish_raw ~ack
  with
  | Error Kafka.Error.Application ->
...
let test_topic_name_accepts_kafka_compatible_names () =
  let check name =
    ...
    | Error e ->
        Alcotest.failf "%s should be valid: %s" name
          (Kafka_service.error_to_string e)
  in
  List.iter check
...
let test_decode_compatibility_response () =
  match
    Kafka_service.Schema.decode_compatibility_response
      {|{"is_compatible":true}|}
  with
  | Ok response ->
      Alcotest.(check bool)
        "is compatible" true response.Kafka_service.Schema.is_compatible
  | Error e -> Alcotest.failf "decode failed: %s" e

let test_decode_compatibility_response_errors () =
  Alcotest.(check (result_error ()))
    "missing field" (Error {|unexpected registry response: {"ok":true}|})
...
let test_decode_registration_response () =
  ...
  | Ok response ->
      Alcotest.(check int) "schema id" 42 response.Kafka_service.Schema.id
  | Error e -> Alcotest.failf "decode failed: %s" e

let test_decode_registration_response_errors () =
  Alcotest.(check (result_error ()))
    "missing id" (Error {|schema registry: missing 'id' in: {"schema":{}}|})
    (Kafka_service.Schema.decode_registration_response {|{"schema":{}}|});
  Alcotest.(check (result_error ()))
    "unexpected shape" (Error {|schema registry: unexpected response: []|})
    (Kafka_service.Schema.decode_registration_response {|[]|});
  Alcotest.(check (result_error ()))
    "malformed json" (Error {|schema registry: json parse error in: {"id":|})
...
let () =
  ...
  run "kafka_service"
    [
      ...
      ( "config",
        [
          test_case "unknown security protocol fails clearly" `Quick
            test_config_of_env_rejects_unknown_security_protocol;
        ] );
      ( "retry_topics",
        [
          test_case "malformed retry headers are rejected" `Quick
            test_retry_metadata_rejects_malformed_headers;
          test_case "ack failure after publish is returned" `Quick
            test_retry_publish_then_ack_failure_is_error;
          test_case "publish failure does not ack" `Quick
            test_retry_publish_failure_does_not_ack;
        ] );
      ( "topic_name",
        [
          test_case "accepts Kafka-compatible names" `Quick
            test_topic_name_accepts_kafka_compatible_names;
          test_case "rejects invalid names" `Quick
            test_topic_name_rejects_invalid_names;
        ] );
      ( "schema_registry_decoding",
        [
          test_case "compatibility response" `Quick
            test_decode_compatibility_response;
          test_case "compatibility response errors" `Quick
            test_decode_compatibility_response_errors;
          test_case "registration response" `Quick
            test_decode_registration_response;
          test_case "registration response errors" `Quick
      ...
      ( "admin_topic_metadata",
        [
          ...
          test_case "topic partition errors" `Quick
            test_decode_topic_partitions_errors;
        ] );
    ]
