  ...
  if nl = 0 then true
    ...
    for i = 0 to hl - nl do
      if not !found then begin
        let rec eq j =
          j >= nl
          || String.unsafe_get haystack (i + j) = String.unsafe_get needle j
             && eq (j + 1)
        in
...
let test_lambda_trigger_requires_runtime_api () =
  if Sys.getenv_opt "AWS_LAMBDA_RUNTIME_API" <> None then
    ...
    | Error (`Config msg) ->
        Alcotest.(check bool)
          "reports missing runtime api" true
    ...
    [
      ( "lifecycle",
        [
          ...
          Alcotest.test_case "external stop before cron run" `Quick
            test_external_stop_before_cron_run;
        ] );
      ( "metrics",
        [
          Alcotest.test_case "metrics_ok_counter" `Quick test_metrics_ok_counter;
          Alcotest.test_case "metrics_error_counter" `Quick
            test_metrics_error_counter;
          Alcotest.test_case "metrics_duration" `Quick test_metrics_duration;
        ] );
      ( "push",
        [
          Alcotest.test_case "push_error_no_raise" `Quick
            test_push_error_no_raise;
        ] );
      ( "lambda",
        [
          Alcotest.test_case "requires AWS_LAMBDA_RUNTIME_API" `Quick
