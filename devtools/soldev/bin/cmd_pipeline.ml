...
let merge_ticket_arg =
  Arg.(
    ...
    & info [] ~docv:"TICKET-ID"
        ~doc:
          "Ticket to merge (e.g. EXP-005) — looked up by its open PR, not a \
...
let merge_cmd =
  Cmd.v
    (Cmd.info "merge"
       ~doc:
         "Merge approved, CI-green PRs — a ticket lands in DONE because its \
...
let submit_cmd =
  Cmd.v
    (Cmd.info "submit"
       ~doc:
         "Run from inside the ticket's worktree, after your final commit has \
...
let review_cmd =
  Cmd.v
    (Cmd.info "review"
       ~doc:
         "Process a structured JSON review result by leaving it as a PR \
          comment — marked SOLDEV-REVIEW: PASS on pass (which `merge` checks \
          for), an ordinary violations comment on fail. Not a formal GitHub \
          review: `gh` always runs as the PR's own author here, and GitHub \
          refuses self-approval. No ticket file moves.")
    Term.(const Soldev_merge.run_review $ ticket_arg $ result_file_arg)

let include_done_flag =
  Arg.(
    value & flag
    & info [ "all"; "a" ] ~doc:"Include DONE tickets in the listing")

let ls_cmd =
  Cmd.v
    (Cmd.info "ls"
       ~doc:
         "List tickets grouped by pipeline stage. Pass --all to include DONE.")
...
let check_cmd =
  Cmd.v
    (Cmd.info "check"
       ~doc:
         "Check whether a ticket is actionable, including human-decision gates \
...
let check_reverts_cmd =
  Cmd.v
    (Cmd.info "check-reverts"
       ~doc:
         "Scan git history for a merge that was later reverted whose ticket \
...
let cmd =
  Cmd.group
    (Cmd.info "pipeline"
       ~doc:
         "Deterministic pipeline operations: merge tickets, process review \
          results, list status")
    [
      ls_cmd;
      check_cmd;
