...
let test_extract_reverted_branch_match () =
  check_option_string "extracts branch" (Some "EXP-023/cloud-init-kubeconfig")
...
let test_ticket_id_from_branch_with_slash () =
  check_string "takes prefix before slash" "EXP-023"
...
let test_mentions_id_exact () =
  check_bool "exact token match" true
    (Soldev_merge.mentions_id ~id:"AUDIT-023"
...
let pass_body sha =
  Printf.sprintf "SOLDEV-REVIEW: PASS %s\n\nAutomated review: pass." sha
...
let toy_main path_const =
  Printf.sprintf
    {|let () = let ic = open_in %S in print_string (input_line ic)|} path_const
...
let test_stale_binary_fails_after_rename () =
  in_temp_dir (fun () ->
      ...
      check_bool
        "stale binary fails against the renamed path (reproduces the bug)" true
        (Sys.command "./_build/default/bin/main.exe >/dev/null 2>&1" <> 0);
      ...
      check_bool "rebuild succeeds" true
        (Sys.command "dune build 2>/dev/null" = 0);
  ...
  Alcotest.run "soldev_merge"
    [
      ( "extract_reverted_branch",
        [
          Alcotest.test_case "matches revert-merge subject" `Quick
            test_extract_reverted_branch_match;
          Alcotest.test_case "ignores unrelated subject" `Quick
            test_extract_reverted_branch_no_match;
        ] );
      ( "ticket_id_from_branch",
        [
          Alcotest.test_case "strips slash suffix" `Quick
      ...
      ( "mentions_id",
        [
          ...
          Alcotest.test_case "rejects prefix embedding" `Quick
            test_mentions_id_rejects_prefix_embedding;
          Alcotest.test_case "rejects numeric suffix" `Quick
            test_mentions_id_rejects_numeric_suffix;
        ] );
      ( "stale-binary post-merge race",
        [
          Alcotest.test_case
            "rebuild before invoking avoids the stale-path race" `Quick
            test_stale_binary_fails_after_rename;
        ] );
      ( "pr_review_approved sha-pinning",
        [
          Alcotest.test_case "PASS then FAIL is not approved" `Quick
          ...
          Alcotest.test_case "PASS on current sha is approved" `Quick
