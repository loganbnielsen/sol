...
let test_run_signaled () =
  ...
  | status ->
      check_status "status" (Sol_process.Signaled 15) status
...
let test_run_command_not_found () =
  let r = Sol_process.run_shell ~echo:false "nonexistent_command_sol_process_test_abc123" in
...
let test_run_ok_failure () =
  let raised =
    try Sol_process.run_shell_ok ~echo:false "false"; false
    with Failure _ -> true
  in
