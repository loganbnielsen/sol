...
let cpu_quantity_of_string s =
  let len = String.length s in
  if len = 0 then
    Error "sol.toml: [infra.scale] cpu quantity must not be empty"
  else
    let valid =
    ...
    if valid then Ok (Cpu_quantity s)
      ...
      Error (Printf.sprintf
        "sol.toml: [infra.scale] cpu quantity %S is invalid — \
...
let memory_quantity_of_string s =
  ...
  if len = 0 then
    ...
    | None ->
      Error (Printf.sprintf
        "sol.toml: [infra.scale] memory quantity %S is invalid — \
...
let hostname_of_string s =
  ...
  if len = 0 || len > 253 then
    ...
    Error (Printf.sprintf
      "sol.toml: [infra.deploy] ingress_host %S is invalid — wildcard hosts are not supported" s)
  else if List.for_all validate_hostname_label (split_on_dot s) then
    Ok (Hostname s)
  else
    Error (Printf.sprintf
      "sol.toml: [infra.deploy] ingress_host %S is invalid — use a DNS hostname like \"api.example.com\"" s)
...
let ingress_path_of_string s =
  ...
  if len = 0 || s.[0] <> '/' then
    Error (Printf.sprintf
      "sol.toml: [infra.deploy] ingress_path %S is invalid — paths must start with \"/\"" s)
  else if has_invalid_char 0 then
    Error (Printf.sprintf
      "sol.toml: [infra.deploy] ingress_path %S is invalid — paths must not contain whitespace or control characters" s)
  else
    Ok (Ingress_path s)
...
let parse_rollout_strategy path s =
  ...
  | other ->
    validation_error path (Printf.sprintf
      "sol.toml: unsupported rollout_strategy %S — \
...
let validate_extra_label_key k =
  ...
  if String.starts_with ~prefix k then
    Error (Printf.sprintf
      "sol.toml: extra_labels key %S is reserved — \
       keys may not start with \"sol.dev/\"" k)
  else
    Ok ()
...
let validate_weight n =
  if n < 0 || n > 100 then
    Error (Printf.sprintf
      "sol.toml: [infra.rollout] canary weight %d is invalid — \
...
let validate_duration d =
  if d <= 0 then
    Error (Printf.sprintf
      "sol.toml: [infra.rollout] pause duration %d is invalid — \
...
let parse_canary_step_value path v =
  let* pairs =
    ...
    with Otoml.Type_error _ ->
      validation_error path
        "sol.toml: [infra.rollout] canary step must be an inline table \
  ...
  | None ->
    (match List.assoc_opt "pause" pairs with
     ...
     | None ->
       validation_error path
         "sol.toml: unsupported [infra.rollout] canary step — \
  ...
  if not (Sys.file_exists path) then Ok empty
    ...
    let* env_config =
      ...
      | Some v ->
        (try
         ...
         with Otoml.Type_error _ ->
           validation_error path
             "sol.toml: [infra.env] config must be an inline table \
    ...
    let* progressive_delivery =
      ...
      | Some "canary" ->
        ...
        if steps = [] then
          validation_error path
            "sol.toml: [infra.rollout] strategy \"canary\" requires at least \
             one step in steps = [...]"
        else
      ...
      | Some other ->
        validation_error path (Printf.sprintf
          "sol.toml: unsupported [infra.rollout] strategy %S — \
    ...
    let* topics =
      ...
      | Some v ->
        (try
         ...
         with Otoml.Type_error _ ->
           validation_error path
             "sol.toml: [service] topics must be an array of strings, \
              e.g. topics = [\"my-topic\"]")
    in
