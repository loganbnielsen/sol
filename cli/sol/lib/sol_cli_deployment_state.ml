...
let deploy_state_configmap_name workspace =
  Printf.sprintf "sol-deploy-state-%s" workspace

let load_deployed_groups workspace =
  let name = deploy_state_configmap_name workspace in
  match
    Sol_cli_kubectl.get ~resource:"configmap" ~name ~namespace:"default"
...
let record_outcome workspace outcome =
  ...
  | Applied { consumer_groups; _ } ->
      save_deployed_groups workspace consumer_groups
