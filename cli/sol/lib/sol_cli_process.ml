  ...
  List.fold_left
    (fun acc secret ->
      if secret = "" then acc
        ...
        let rec loop i =
          if i > alen - slen then Buffer.add_substring buf acc i (alen - i)
  ...
  let rec loop so se =
    if so || se then begin
      ...
      if !timed_out then ()
        ...
        if ready = [] && deadline <> None then timed_out := true
        else begin
          let so =
            so
...
let run ?(echo = false) c =
  ...
  | prog :: _ -> (
      ...
      | Ok saved_cwd -> (
          ...
          | Ok pid ->
              let deadline =
                Option.map (fun s -> Unix.gettimeofday () +. s) c.timeout_s
              in
              ...
              if timed_out then begin
                ...
                let exit_code =
                  status_to_exit_code (Unix.waitpid [] pid |> snd)
                in
                Ok
                  {
                    exit_code;
