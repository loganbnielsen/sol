...
let parse_container_state (c : Yojson.Safe.t) : container_state =
  ...
  | Some state ->
      ...
      if not (is_null waiting) then
        ...
        let reason =
          J.member "reason" w |> to_string_opt
          |> Option.value ~default:"Unknown"
        in
        let message = J.member "message" w |> to_string_opt in
        Waiting { reason; message }
      else if not (is_null running) then Running
      else if not (is_null terminated) then
        let t = Option.get terminated in
        let reason =
          J.member "reason" t |> to_string_opt
          |> Option.value ~default:"Unknown"
        in
        let exit_code =
          J.member "exitCode" t |> to_int_opt |> Option.value ~default:0
        in
...
let parse_last_terminated_reason (c : Yojson.Safe.t) : string option =
  ...
  | Some ls -> (
      ...
      | Some t when not (is_null (Some t)) ->
          J.member "reason" t |> to_string_opt
      | _ -> None)
...
let parse_pod (item : Yojson.Safe.t) : pod_status =
  ...
  | c :: _ ->
      let ready =
        J.member "ready" c |> to_bool_opt |> Option.value ~default:false
      in
      let restarts =
        J.member "restartCount" c |> to_int_opt |> Option.value ~default:0
      in
...
let parse_event (item : Yojson.Safe.t) : event option =
  try
    let involved_name =
      ...
      |> J.member "name" |> to_string_opt |> Option.value ~default:""
    in
    let ev_type =
      J.member "type" item |> to_string_opt |> Option.value ~default:"Normal"
    in
    let reason =
      J.member "reason" item |> to_string_opt |> Option.value ~default:""
    in
    let message =
      J.member "message" item |> to_string_opt |> Option.value ~default:""
    in
    let count =
      J.member "count" item |> to_int_opt |> Option.value ~default:1
    in
...
let render_unhealthy_pods ~service_name (pods : pod_status list)
    (events : event list) : string =
...
let format_service_diagnosis ~service_name (pods : pod_status list)
    (events : event list) : string option =
  if pods = [] then
    Some
      (Printf.sprintf "%s rollout failed\n\nNo pods found for this service.\n"
         service_name)
  else
...
let format_active_run_diagnosis ~service_name (pods : pod_status list)
    (events : event list) : string option =
  let unhealthy =
    List.filter (fun p -> not (is_active_run_pod_ok ~events p)) pods
  in
  if unhealthy = [] then None
  else Some (render_unhealthy_pods ~service_name unhealthy events)
...
let parse_cronjob_status (s : string) : cronjob_status option =
  try
    ...
    | status ->
        let last_schedule_time =
          J.member "lastScheduleTime" status |> to_string_opt
        in
        let last_successful_time =
          J.member "lastSuccessfulTime" status |> to_string_opt
        in
        let active_count, active_job_names =
          ...
          | Some (`List l) ->
              ( List.length l,
                List.filter_map
                  (fun item -> J.member "name" item |> to_string_opt)
                  l )
          | _ -> (0, [])
        in
        Some
...
let format_cronjob_diagnosis ~service_name (result : cronjob_fetch_result) :
    string option =
  match result with
  | Unavailable -> None
  | Missing ->
        ...
        | Some scheduled ->
            ...
            if succeeded_since then None
              ...
              Some
                (Printf.sprintf
                   "%s rollout failed\n\n\
                    Most recently scheduled run (%s) did not complete \
                    successfully.\n"
                   service_name scheduled))

let fetch_namespace_events ~ns : event list =
  match
    Sol_cli_kubectl.get_raw ~args:[ "get"; "events"; "-n"; ns; "-o"; "json" ]
  with
...
let fetch_job_pod_statuses ~ns ~job_name : pod_status list option =
  match
    Sol_cli_kubectl.get_raw
      ~args:
        [ "get"; "pods"; "-n"; ns; "-l"; "job-name=" ^ job_name; "-o"; "json" ]
  with
  | Ok r when r.Sol_cli_process.exit_code = 0 ->
      Some (parse_pods_json r.Sol_cli_process.stdout)
  | _ -> None
...
let fetch_cronjob_status ~ns ~k8s_name : cronjob_fetch_result =
  ...
  | Ok r ->
      if
        Sol_cli_port_forward.string_contains ~needle:"NotFound"
          r.Sol_cli_process.stderr
      then Missing
      else Unavailable

let diagnose_service_live ~pod_expectation ~ns ~service_name ~k8s_name () :
    string option =
