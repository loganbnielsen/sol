...
let progressive_delivery_to_json = function
  ...
  | Some Sol_cli_toml.Blue_green ->
      `Assoc [ ("strategy", `String "blue_green") ]
    ...
    `Assoc
      [
        ...
        ( "progressive_delivery",
          progressive_delivery_to_json s.progressive_delivery );
      ]
  in
  ...
  `Assoc
    [
      ...
      ( "topics",
        `List
          (List.map
             (fun s -> `String (Sol_cli_plan_ids.Topic_name.to_string s))
             t.topics) );
      ( "migrations",
        `List
          (List.map
...
let pp_summary fmt t =
  ...
  List.iter
    (fun (s : service_spec) ->
      ...
      Format.fprintf fmt "  [%s] %s/%s    rollout=%s -> %s@\n"
        ...
        s.domain s.source_name rollout_strategy s.image)
    t.services;
  (match t.topics with
  | [] -> ()
  | topics ->
      ...
      Format.fprintf fmt "topics:   %s@\n"
        (String.concat ", "
           (List.map Sol_cli_plan_ids.Topic_name.to_string topics)));
  ...
  | ms ->
      ...
      Format.fprintf fmt "migrations:   %s@\n"
        (String.concat ", "
           (List.map Sol_cli_plan_ids.Migration_file.to_string ms)));
  ...
  | ss ->
      ...
      Format.fprintf fmt "schema subjects:  %s@\n"
        (String.concat ", "
           (List.map Sol_cli_plan_ids.Schema_subject.to_string ss)));
  ...
  | cgs ->
      ...
      Format.fprintf fmt "consumer groups:  %s@\n"
        (String.concat ", "
           (List.map Sol_cli_plan_ids.Consumer_group.to_string cgs))
...
let image_ref ~registry ~workspace ~k8s_name ~tag =
  Printf.sprintf "%s/%s/%s:%s" registry workspace
...
let of_services_result ~workspace ~env ?resolved_config services =
  let to_spec svc =
    ...
    let* namespace =
      namespace_result ~workspace ~domain:svc.Sol_cli_manifest.domain
    in
    let image =
    ...
    let schedule =
      ...
      | Fn ->
          Some
            (Sol_cli_manifest.extract_schedule ~dir:svc.Sol_cli_manifest.dir
    ...
    in
    {
      ...
      replicas =
        (match
           sol_yml_replicas_override ~resolved_config
