...
let effective_rollout_strategy s =
  ...
  | None ->
    (match s.rollout_strategy with
     ...
     | Some Sol_cli_toml.RollingUpdate
...
let to_json t =
  ...
  `Assoc [
    ...
    "topics",           `List (List.map (fun s -> `String (Sol_cli_plan_ids.Topic_name.to_string s)) t.topics);
    "migrations",       `List (List.map (fun s -> `String (Sol_cli_plan_ids.Migration_file.to_string s)) t.migrations);
    "schema_subjects",  `List (List.map (fun s -> `String (Sol_cli_plan_ids.Schema_subject.to_string s)) t.schema_subjects);
    "consumer_groups",  `List (List.map (fun s -> `String (Sol_cli_plan_ids.Consumer_group.to_string s)) t.consumer_groups);
  ]
...
let namespace_result ~workspace ~domain =
  let value =
    Printf.sprintf "%s-%s" (Sol_cli_kubernetes_name.normalize workspace)
...
let of_services_result ~workspace ~env ?resolved_config services =
  let to_spec svc =
    ...
    let schedule  = match primitive with
      | Fn -> Some (Sol_cli_manifest.extract_schedule
    ...
    ; schedule
    ; replicas              =
        (match sol_yml_replicas_override ~resolved_config ~service_name:svc.Sol_cli_manifest.name with
