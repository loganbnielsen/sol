...
let render ~workspace ?env ?(image = "") ?(secret_backend = Sol_cli_manifest.Kubernetes_live)
    { common; workload } =
      ...
      let deployment_resources ~shape ~ingress_host ~ingress_path ~deployment =
        ...
        | None ->
          let rollout_strategy = Option.value rollout_strategy
                                   ~default:Sol_cli_toml.RollingUpdate in
...
let render_spec ~workspace ?env ?(image = "") ?(secret_backend = Sol_cli_manifest.Kubernetes_live)
