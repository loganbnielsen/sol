          ...
          Array.iter (fun name ->
            ...
            if name.[0] <> '.' && Sys.is_directory dir then
              ...
              | Some primitive ->
                ...
                in
                if included then
                  services := { Sol_cli_manifest.domain; name; primitive; dir } :: !services
          ) (Sys.readdir dp)
      ) (Sys.readdir app_dir)
    with _ -> ());
     ...
     List.iter (fun key ->
       if not (valid_env_key key) then
         add Severity.Error toml_path
           (Printf.sprintf "invalid secret key %S; use uppercase letters, digits, and underscores" key)
     ) toml.Sol_cli_toml.secret_keys;
     (match svc.primitive, toml.schedule with
      | Sol_cli_manifest.Fn, _ -> ()
      | (Svc | Worker), Some _ ->
        add Severity.Warning toml_path "[service] schedule is ignored for non-function workloads"
      | _ -> ()));
  List.rev !findings
...
let run ~filter_path () =
  ...
  | Error err ->
    [ { severity = Severity.Error; path = "app"; message = Sol_cli_manifest.discover_error_to_string err } ]
  | Ok services when services = [] ->
    [ { severity = Severity.Error; path = "app"; message = "no Sol workloads found with a Dockerfile" } ]
  | Ok services ->
    List.concat_map check_service services

let has_errors findings =
  List.exists is_error findings
