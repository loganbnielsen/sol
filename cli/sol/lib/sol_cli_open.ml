...
type scope =
  ...
  | Resource of string * string
      (** [Resource (resource_type, resource_name)] -- a managed infrastructure
          ...
          (local.managed_resources in cli/platform/infra/aws/main.tf) -- adding
...
let parse_scope = function
  ...
  | Some s -> (
      ...
      | _ ->
          Error
            (Printf.sprintf
               "scope must be 'domain', 'domain/service', or \
                'resource/<type>/<name>', got %S"
               s))
...
let dashboard_url ~base_url ~workspace scope =
  ...
  | Workspace ->
      Ok
        (Printf.sprintf "%s/d/sol-workspace-overview?var-workspace=%s" base_url
           workspace)
  | Domain domain ->
      let domain = Sol_cli_kubernetes_name.sanitize_label_value domain in
      Ok
        (Printf.sprintf
           "%s/d/sol-service-template?var-workspace=%s&var-domain=%s" base_url
           workspace domain)
  ...
  | Resource (resource_type, resource_name) ->
      ...
      if String.trim resource_type = "" then
        Error
          "resource type must not be empty (expected 'resource/<type>/<name>')"
      else if String.trim resource_name = "" then
        Error
          "resource name must not be empty (expected 'resource/<type>/<name>')"
      else
        let resource_type =
          Sol_cli_kubernetes_name.sanitize_label_value resource_type
        in
        let resource_name =
          Sol_cli_kubernetes_name.sanitize_label_value resource_name
        in
        Ok
          (Printf.sprintf "%s/d/sol-managed-resource-%s?var-resource=%s"
...
let logs_url ~base_url ~workspace scope =
  ...
  | Workspace ->
      Ok
        (Sol_cli_logs.explore_url ~base_url
  ...
  | Service (domain, name) -> (
      ...
      with
      | Error e, _ | _, Error e ->
          Error (Sol_cli_deployment_plan.plan_error_to_string e)
      | Ok ns, Ok k8s_name ->
          let ns = Sol_cli_deployment_plan.namespace_to_string ns in
  ...
  | Resource (resource_type, _) ->
      ...
      Error
        (Printf.sprintf
           "no logs view for managed resource type %S -- managed resources \
            don't ship through Sol's Loki pipeline; use 'sol open dashboard' \
            or check the provider's own console"
           resource_type)
