...
(** Scan [db/migrations/*.sql] in the current directory and return validated
    {!Sol_cli_plan_ids.Migration_file.t} values, sorted by filename.
...
val derive_consumer_groups : string -> service_spec list -> Sol_cli_plan_ids.Consumer_group.t list
...
(** Build a deployment plan from a discovered service list and an environment
    ...
    [resolved_config], when given (the [sol deploy]/target-resolved path; [sol
    up] never has one), overrides a service's [sol.toml] [replicas] with its
    [sol.yml] entry's [scale_max] (falling back to [scale_min]) when a service
    of the same name sets either. A service with no matching [sol.yml] entry,
    or no [resolved_config] at all, keeps [sol.toml]'s [replicas] unchanged. *)
