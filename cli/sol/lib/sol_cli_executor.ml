...
let dispatch_rendered ~mode spec yaml =
  (match mode with
   ...
   | Emit_to dir ->
     let ns   = Sol_cli_deployment_plan.namespace_to_string spec.Sol_cli_deployment_plan.namespace in
     let name = Sol_cli_deployment_plan.k8s_name_to_string spec.Sol_cli_deployment_plan.k8s_name in
...
let gitops ~workspace ~dir ?(secret_backend = Sol_cli_manifest.Kubernetes_placeholder) spec =
...
let run_plan ~workspace ?env ~mode ?(secret_backend = Sol_cli_manifest.Kubernetes_placeholder) services =
  ...
  let rendered = List.map
    (fun spec -> match Sol_cli_deployment_render.render_spec ~workspace ?env ~secret_backend:backend spec with
  ...
  | _ ->
    ...
    Ok (List.map (fun ((spec : Sol_cli_deployment_plan.service_spec), yaml) ->
