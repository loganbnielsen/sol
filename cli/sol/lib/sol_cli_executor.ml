...
type result = { namespace : string; name : string; image : string }
  ...
  | Emit_to dir ->
      ...
      let name =
        Sol_cli_deployment_plan.k8s_name_to_string
          spec.Sol_cli_deployment_plan.k8s_name
      in
...
let gitops ~workspace ~dir
    ?(secret_backend = Sol_cli_manifest.Kubernetes_placeholder) spec =
  match
    Sol_cli_deployment_render.render_spec ~workspace ~secret_backend spec
  with
  | Error msg -> failwith msg
...
let run_plan ~workspace ?env ~mode
    ?(secret_backend = Sol_cli_manifest.Kubernetes_placeholder) services =
  let backend =
    match mode with
  ...
  let rendered =
    List.map
      (fun spec ->
        match
          Sol_cli_deployment_render.render_spec ~workspace ?env
            ~secret_backend:backend spec
        with
  ...
  | _ ->
      let pairs =
        List.filter_map (function Ok x -> Some x | Error _ -> None) rendered
      in
      Ok
        (List.map
