...
let parse_scalar s =
  ...
  if len = 0 then Ok s
    ...
    if len >= 2 && s.[len - 1] = s.[0] then Ok (String.sub s 1 (len - 2))
...
let index_empty index_name =
  { index_name; partition_key = None; sort_key = None }
    ...
    Fun.protect
      ...
      (fun () ->
        ...
        let require_value k v =
          if v = "" then fail (Printf.sprintf "missing value for %s" k)
          else Ok ()
        in
        let scalar k v =
        ...
        let update_provider provider f =
          ...
          let fields =
            List.assoc_opt provider target.provider_fields
        ...
        let add_resource name =
          ...
          then fail (Printf.sprintf "duplicate resource %S" name)
            ...
            cfg :=
              { !cfg with resources = !cfg.resources @ [ resource_empty name ] };
            Ok ()
          end
        in
        let add_service name =
          if List.exists (fun (s : service) -> s.name = name) !cfg.services then
            ...
            cfg :=
              { !cfg with services = !cfg.services @ [ service_empty name ] };
            Ok ()
          end
        in
        let rec loop () =
          ...
          | raw -> (
              ...
              if trim text = "" then loop ()
                ...
                | 0, "target:", _ ->
                    if !seen_target then
                      fail "duplicate top-level section \"target\""
                    else if !root <> No_root then
                      fail "target must appear before resources or services"
                    else begin
                      seen_target := true;
                      root := No_root;
                      section := Target;
                ...
                | 0, "resources:", _ ->
                    if !seen_resources then
                      fail "duplicate top-level section \"resources\""
                    else begin
                      seen_resources := true;
                      root := Resources_root;
                      section := Resources;
                ...
                | 0, "services:", _ ->
                    if !seen_services then
                      fail "duplicate top-level section \"services\""
                    else begin
                      seen_services := true;
                      root := Services_root;
                      section := Services;
                ...
                | 0, _, Some ("project", v) ->
                    ...
                    loop ()
                | 0, _, Some (k, _) ->
                    fail (Printf.sprintf "unknown top-level key %S" k)
                        ...
                        | (Target | Target_provider _), Some (k, "") ->
                            ...
                            | Target_provider_box provider ->
                                ...
                                if List.mem provider !seen_provider_boxes then
                                  fail
                                    (Printf.sprintf
                                       "duplicate target provider box %S"
                                       provider)
                                else begin
                                  seen_provider_boxes :=
                                    provider :: !seen_provider_boxes;
                                  section := Target_provider provider;
                                  loop ()
                                end
                            | Target_unknown k ->
                                fail
                                  (Printf.sprintf "unsupported provider %S" k)
                            | key ->
                                fail
                                  (Printf.sprintf "missing value for %s"
                                     (target_key_name key))
                            end
                      ...
                      | Target | Target_provider _ ->
                          ...
                          | Target_provider_box provider, "" ->
                              ...
                              if List.mem provider !seen_provider_boxes then
                                fail
                                  (Printf.sprintf
                                     "duplicate target provider box %S" provider)
                              else begin
                                seen_provider_boxes :=
                                  provider :: !seen_provider_boxes;
                                section := Target_provider provider;
                          ...
                          | _ ->
                              let current =
                                Option.value !cfg.target ~default:target_empty
                              in
                              let* target =
                                match key with
                                | Target_registry ->
                ...
                | 4, _, Some (k, v) ->
                    let* () =
                      begin match !section with
                      | Resource name
                      | Resource_indexes name
                      | Resource_index (name, _) ->
                          ...
                          if k = "scale" && v = "" then begin
                            ...
                            let* () =
                              update_service name (fun s ->
                                  ...
                                  | _ ->
                                      fail
                                        (Printf.sprintf "unknown service key %S"
                                           k))
                            in
                            Ok ()
                          ...
                          if v = "" then Ok ()
                            ...
                            update_provider provider (fun fields ->
                                if List.mem_assoc k fields then
                                  fail
                                    (Printf.sprintf
                                       "duplicate %s target field %S" provider k)
                                else Ok (fields @ [ (k, v) ]))
                      | _ -> fail "unsupported sol.yml syntax"
                      end
                    in
                ...
                | 6, _, _ when ends_with ~suffix:":" body ->
                    let* () =
                      ...
                      | Resource_index (resource_name, _) ->
                          let index_name =
                            drop_suffix ~suffix:":" body |> trim
                          in
                          update_resource resource_name (fun r ->
                              if
                                List.exists
                                  (fun i -> i.index_name = index_name)
                                  r.indexes
                              then
                                fail
                                  (Printf.sprintf "duplicate index %S"
                                     index_name)
                              else begin
                                section :=
                                ...
                                Ok
                                  {
                                    r with
                                    indexes =
                                      r.indexes @ [ index_empty index_name ];
                                  }
                              end)
                      | _ -> fail "unsupported sol.yml syntax"
                      end
                      ...
                      | Service_scale name ->
                          ...
                          let* () =
                            update_service name (fun s ->
                                ...
                                | _ ->
                                    fail
                                      (Printf.sprintf "unknown scale key %S" k))
                          in
                          Ok ()
                      | _ -> fail "unsupported sol.yml syntax"
                      end
                        ...
                        let* () =
                          update_resource resource_name (fun r ->
                              Ok
                                {
                                  r with
                                  indexes = List.map update_index r.indexes;
                                })
                        in
                        loop ()
                    | Resource_indexes _
...
let merge_fields a b =
  List.fold_left
    (fun acc (k, v) ->
      upsert_by_name fst k (function None -> (k, v) | Some _ -> (k, v)) acc)
    a b
  ...
  List.fold_left
    (fun acc y ->
      upsert_by_name name (name y)
        (function None -> y | Some x -> merge x y)
        acc)
    xs ys
...
let merge base overlay =
  {
    ...
    resources =
      merge_by
        ...
        merge_resource base.resources overlay.resources;
    services =
      merge_by
        (fun (s : service) -> s.name)
        merge_service base.services overlay.services;
  }

let target_of_path s =
  match String.split_on_char '/' s with
  | [ env; provider; region ]
    when env <> "" && provider <> "" && region <> "" && env <> ".."
         && provider <> ".." && region <> ".." ->
  ...
  | parts when List.exists (( = ) "..") parts ->
      Error
        { path = s; line = 0; message = "target path must not contain '..'" }
  | _ ->
      Error
        {
          path = s;
      ...
      Error
        {
          ...
          line = 0;
          message =
            Printf.sprintf "service %S uses undeclared resource %S" service_name
              ref;
        }
  else
      ...
      let rec validate_services = function
        ...
        | service :: rest ->
            let rec validate_refs = function
              ...
              | ref :: refs ->
                  let* () =
                    validate_use_ref ~target ~resources:resource_names
                      service.name ref
                  in
                  validate_refs refs
            in
...
let load_for_target ~target =
  ...
  let* () =
    if Sys.file_exists "sol.yml" || Sys.file_exists file then Ok ()
    else
      Error
        {
          path = target.name;
          ...
          message =
            Printf.sprintf
              "target %S resolves to neither a sol.yml nor a \
               sol/<env>/<provider>/<region>.yml in this directory — at least \
               one must exist for a target to be real, not just shaped like \
               <env>/<provider>/<region>"
              target.name;
        }
  in
...
let ecr_repositories_var () =
  ...
  |> List.filter_map (fun (s : Sol_cli_manifest.service) ->
      match
        Sol_cli_kubernetes_name.k8s_name_of_source s.Sol_cli_manifest.name
      with
...
let terraform_vars ~workspace cfg =
  ...
  | Some target ->
      ...
      let vars =
        List.assoc_opt
          (Sol_cli_provider.to_string target.provider)
          target.provider_fields
        |> Option.value ~default:[] |> List.rev_append vars
      in
      let has_postgres =
        resources cfg
