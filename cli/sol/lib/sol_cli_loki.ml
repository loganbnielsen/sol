...
let resolve_credentials
  ...
  | Some _, None ->
    Error "--loki-username (or SOL_LOKI_USERNAME) is set without a password \
           -- pass --loki-password or set SOL_LOKI_PASSWORD too"
  | None, Some _ ->
    Error "--loki-password (or SOL_LOKI_PASSWORD) is set without a username \
...
let query ~base_url ~ns ~k8s_name ?credentials ?(limit = 100) ?(timeout_s = 5.0) ()
  ...
  in
  match Sol_cli_process.run (Sol_cli_process.cmd ~timeout_s:(timeout_s +. 2.0) ~redact argv) with
  ...
  | Ok r when r.Sol_cli_process.exit_code <> 0 ->
    ...
    Error (classify_process_error
             (Sol_cli_process.Non_zero
                { exit_code = r.Sol_cli_process.exit_code; stderr = r.Sol_cli_process.stderr }))
  | Ok r ->
