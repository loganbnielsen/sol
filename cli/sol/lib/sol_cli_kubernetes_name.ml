...
let is_alnum = function 'a' .. 'z' | '0' .. '9' -> true | _ -> false
...
let validate_dns_label value =
  ...
  if len = 0 then Error "must be between 1 and 63 characters"
  else if len > 63 then Error "must be between 1 and 63 characters"
    ...
    let rec loop i =
      if i = len then Ok ()
        ...
        Error "must contain only lowercase alphanumeric characters or hyphens"
    in
    loop 0
