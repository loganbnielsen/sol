...
let push_image_ref ~workspace ~sha spec =
  Sol_cli_deployment_plan.image_ref ~registry:push_registry ~workspace
...
let dry_run_spec ~workspace ~sha spec =
  {
    spec with
    Sol_cli_deployment_plan.image = push_image_ref ~workspace ~sha spec;
  }
...
let prepare_build_context ~repo_root =
  ...
  let rsync_cmd =
    Printf.sprintf
      ...
      (Filename.quote repo_root) (Filename.quote ctx_dir)
  in
...
let build_image exec =
  match
    Sol_cli_docker.build ~tag:exec.push_image ~dockerfile:exec.dockerfile
      ~context:exec.context
  with
  | Error e ->
      Error
        (Printf.sprintf "docker build failed: %s\n%s" exec.source_dir
...
let push_image exec =
  ...
  | Error e ->
      Error
        (Printf.sprintf "docker push failed: %s\n%s" exec.push_image
           (Sol_cli_process.error_to_string e))
  | Ok () -> Ok ()
...
let wait_for_service_rollout spec exec =
  ...
  | Sol_cli_deployment_plan.Svc | Sol_cli_deployment_plan.Worker -> (
      ...
      | _ -> (
          let pod_expectation =
            Sol_cli_status.pod_expectation_of_primitive
              (manifest_primitive spec.primitive)
          in
          match
            Sol_cli_rollout_diagnosis.diagnose_service_live ~pod_expectation
              ~ns:exec.namespace ~service_name:spec.source_name
              ~k8s_name:exec.k8s_name ()
          with
          | Some d -> Error d
          | None ->
              Error
                (Printf.sprintf "rollout failed: %s/%s" exec.namespace
                   exec.k8s_name)))
