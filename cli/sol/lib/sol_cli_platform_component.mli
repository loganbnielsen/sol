(** Reads shared platform-component desired state from
    ...
    [sol local infra up] and [cli/platform/infra/base/main.tf]'s [helm_release]
...
(** [merged_values_yaml ~component ~profile] reads
    ...
    wins on key conflicts; nested objects merge recursively, other conflicts
    take the profile's value outright), and returns the merged document as JSON
    text. JSON is valid YAML, so this is suitable as-is for
    {!Sol_cli_helm.upgrade_install}'s [?values_yaml].

    A missing file (a component with nothing to say for that layer, e.g.
    [tempo]'s empty profiles) is treated as an empty object, not an error. Exits
