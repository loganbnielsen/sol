...
let run_phase t ~name (thunk : unit -> (Sol_cli_process.result, Sol_cli_process.error) result)
  ...
  let (ok, contents) = match result with
    | Ok r ->
      (r.Sol_cli_process.exit_code = 0,
       phase_log_content ~stdout:r.Sol_cli_process.stdout ~stderr:r.Sol_cli_process.stderr)
    | Error e ->
