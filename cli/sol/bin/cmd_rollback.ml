...
let run filter_path =
  ...
  List.iter (fun svc ->
    ...
    let k8s_name_val = match Sol_cli_deployment_plan.k8s_name_result svc.name with
      | Ok v -> v
      | Error err ->
        Printf.eprintf "error: %s\n" (Sol_cli_deployment_plan.plan_error_to_string err);
        exit 1
    in
    let namespace_val = match Sol_cli_deployment_plan.namespace_result ~workspace ~domain:svc.domain with
      | Ok v -> v
      | Error err ->
        Printf.eprintf "error: %s\n" (Sol_cli_deployment_plan.plan_error_to_string err);
        exit 1
