...
let run filter_path =
  ...
  List.iter
    (fun svc ->
      ...
      let namespace_val =
        match
          Sol_cli_deployment_plan.namespace_result ~workspace ~domain:svc.domain
        with
        | Ok v -> v
        | Error err ->
            Printf.eprintf "error: %s\n"
...
let path_arg =
  Arg.(
    ...
    & info [] ~docv:"SERVICE"
        ~doc:
          "Service path to roll back, e.g. payments/charge_svc (default: all \
...
let cmd =
  Cmd.v
    (Cmd.info "rollback"
       ~doc:
         "Roll back the last deployment for one or all services. Runs 'kubectl \
          rollout undo' for each matching service and waits for the previous \
          revision to become healthy.")
