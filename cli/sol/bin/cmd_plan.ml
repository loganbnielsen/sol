...
let run target_name =
  match Sol_cli_config.load_for_target ~target:target_name with
  | Error e ->
      Printf.eprintf "error: %s\n" (Sol_cli_config.error_to_string e);
      exit 1
  | Ok cfg -> (
      let project =
        Option.value cfg.project ~default:(Filename.basename (Sys.getcwd ()))
      in
      ...
      | Some target ->
          ...
          print_opt "provider"
            (Some (Sol_cli_provider.to_string target.provider));
            ...
            (fun (s : Sol_cli_config.service) ->
              ...
              if s.uses <> [] then
                Printf.printf "    uses: %s\n"
                  (String.concat ", "
                     (List.map Sol_cli_config.format_use_ref s.uses));
...
let cmd =
  Cmd.v
    (Cmd.info "plan"
       ~doc:"Print the merged Sol app/resource/service plan for a target.")
