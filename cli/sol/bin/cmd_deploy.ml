...
let git_sha () =
  match
    Sol_cli_process.run
      (Sol_cli_process.cmd [ "git"; "rev-parse"; "--short"; "HEAD" ])
  with
...
let print_service_urls (results : Sol_cli_executor.result list) =
  let deployed_names =
    List.map
      (fun (r : Sol_cli_executor.result) -> r.Sol_cli_executor.name)
      results
  in
  let namespaces =
  ...
  List.iter
    (fun ns ->
      ...
      with
      | Ok r
        when r.Sol_cli_process.exit_code = 0 && r.Sol_cli_process.stdout <> ""
        ->
          ...
          |> List.iter (fun name ->
              match
                Sol_cli_kubectl.get ~resource:"svc" ~name ~namespace:ns
                  ~output:("jsonpath=" ^ port80_jsonpath)
              with
              | Ok gr
                when gr.Sol_cli_process.exit_code = 0
                     && gr.Sol_cli_process.stdout <> "" ->
...
let check_consumer_group_changes ~workspace ~confirm_group_change plan =
  ...
  in
  let removed =
    Sol_cli_deployment_state.removed_consumer_groups ~prev:prev_groups
      ~next:next_groups
  in
  if removed <> [] && not confirm_group_change then begin
    Printf.eprintf
      "\n\
       warning: the following consumer group(s) are no longer present in this \
       deploy plan:\n";
...
let build_plan ctx ~emit_to =
  let env_target =
    match
      Sol_cli_env_target.customer_cloud_defaults ~registry:ctx.registry
        ~image_tag:ctx.sha ~emit_to ()
    with
    | Ok t -> t
    | Error msg ->
...
let write_plan_if_requested ~emit_plan_to plan =
  ...
  | Some path ->
      let json_str =
        Yojson.Safe.pretty_to_string (Sol_cli_deployment_plan.to_json plan)
      in
      if path = "-" then begin
...
let run_emit ctx ~dir =
  ...
  let results =
    run_plan_or_exit ~workspace:ctx.workspace
      ~target_env:ctx.target_cfg.Sol_cli_config.env
      ~mode:(Sol_cli_executor.Emit_to dir) ~secret_backend:ctx.secret_backend
      plan
  in
  List.iter
    (fun (r : Sol_cli_executor.result) ->
      let path =
        Filename.concat dir
          (Printf.sprintf "%s-%s.yaml" r.Sol_cli_executor.namespace
  ...
  Printf.printf
    "Commit and push to your GitOps repo, then Argo CD will apply them.\n"
...
let push_deploy_events ~workspace ~target_cfg ~loki_push_url plan =
  ...
  in
  try
    Cmd_deploy_event.push_all ~backend ~explicit_url:loki_push_url deploy_events
  with
...
let run_apply ctx ~filter_path ~confirm_group_change ~loki_push_url =
  ...
  check_consumer_group_changes ~workspace:ctx.workspace ~confirm_group_change
    plan;
  write_plan_if_requested ~emit_plan_to:ctx.emit_plan_to plan;
  print_planned_services plan;
  let results =
    run_plan_or_exit ~workspace:ctx.workspace ~target_env
      ~mode:Sol_cli_executor.Apply ~secret_backend:ctx.secret_backend plan
  in
  List.iter
    (fun (r : Sol_cli_executor.result) ->
  ...
  push_deploy_events ~workspace:ctx.workspace ~target_cfg:ctx.target_cfg
    ~loki_push_url plan
...
let run (req : Sol_cli_command_request.deploy_request) =
  ...
  if not (Sys.file_exists (Sol_cli_config.target_file target_cfg)) then begin
    Printf.eprintf
      "error: no %s for target %S -- sol deploy requires an explicit target \
       file, even an empty one, so a typo'd or unintended target can't \
  ...
  let registry =
    ...
    | Some r -> r
    | None -> (
        match target_cfg.Sol_cli_config.registry with Some r -> r | None -> "")
  in
...
let target_arg =
  Arg.(
    ...
    & info [] ~docv:"TARGET"
        ~doc:
          "Deployment target path: <env>/<provider>/<region>, e.g. \
           dev/aws/us-east-1 — same convention as 'sol plan'. Resolves sol.yml \
           + sol/<env>/<provider>/<region>.yml for registry/env defaults. \
           Unlike 'sol up' (local-only, no target concept), this is required.")
...
let emit_to_arg =
  Arg.(
    ...
    & info [ "emit-to" ] ~docv:"DIR"
        ~doc:
          "Write YAML files to DIR instead of applying (GitOps mode). One file \
           per service: <namespace>-<name>.yaml")
...
let emit_plan_to_arg =
  Arg.(
    ...
    & info [ "emit-plan-to" ] ~docv:"FILE"
        ~doc:
          "Write the deployment plan as JSON to FILE before executing. Use '-' \
           to print to stdout. Plan format is experimental.")
...
let image_tag_arg =
  Arg.(
    ...
    & info [ "image-tag" ] ~docv:"TAG"
        ~doc:
          "Image tag to deploy (default: short git SHA). In CI, pass the exact \
           SHA built by the preceding job.")
...
let registry_arg =
  Arg.(
    ...
    & info [ "registry" ] ~docv:"URL"
        ~doc:
          "Container registry prefix, e.g. \
           123456789.dkr.ecr.us-east-1.amazonaws.com. Omit to fall back to the \
           resolved target's own registry (sol/<env>/<provider>/<region>.yml); \
...
let secret_backend_arg =
  Arg.(
    ...
    & info [ "secret-backend" ] ~docv:"BACKEND"
        ~doc:
          "Secret backend for GitOps output. 'kubernetes-placeholder' \
           (default) emits a redacted Kubernetes Secret; 'external-secrets' \
           emits an ExternalSecret CRD for the External Secrets Operator. Only \
           meaningful with --emit-to.")
...
let secret_store_kind_arg =
  Arg.(
    ...
    & info [ "secret-store-kind" ] ~docv:"KIND"
        ~doc:
          "Kind of the secret store reference (default: ClusterSecretStore). \
...
let key_prefix_arg =
  Arg.(
    ...
    & info [ "key-prefix" ] ~docv:"PREFIX"
        ~doc:
          "Prefix to prepend to each secret key when looking up in the \
           external store (default: \"\"). Example: 'myworkspace/' produces \
           keys like 'myworkspace/POSTGRES_URL'.")
...
let refresh_interval_arg =
  Arg.(
    ...
    & info [ "refresh-interval" ] ~docv:"INTERVAL"
        ~doc:
          "How often ESO should sync the secret from the external store \
           (default: 1h). Examples: '1h', '30m', '5m'.")
...
let secret_backend_term =
  let build str store_ref store_kind key_prefix refresh_interval emit_to =
    ...
    | "external-secrets" when emit_to = None ->
        Printf.eprintf
          "warning: --secret-backend external-secrets is only meaningful with \
           --emit-to; using kubernetes-placeholder.\n";
        `Ok Sol_cli_manifest.Kubernetes_placeholder
    | "external-secrets" -> (
        ...
        | None ->
            `Error
              ( true,
                "--secret-store-ref is required when \
    ...
    | other ->
        `Error
          ( true,
            Printf.sprintf
              "unknown --secret-backend value %S (expected: \
               kubernetes-placeholder | external-secrets)"
              other )
  in
  Term.(
    ret
      (const build $ secret_backend_arg $ secret_store_ref_arg
     $ secret_store_kind_arg $ key_prefix_arg $ refresh_interval_arg
     $ emit_to_arg))
...
let confirm_group_change_flag =
  Arg.(
    ...
    & info [ "confirm-group-change" ]
        ~doc:
          "Acknowledge that consumer group IDs have changed and proceed with \
           deploy")
...
let loki_push_url_arg =
  Arg.(
    ...
    & info [ "loki-push-url" ] ~docv:"URL"
        ~doc:
          "Loki push URL for this deploy's release-event log line (OBS-037), \
           e.g. https://logs-prod-000.grafana.net. When omitted: for the \
           'local'/'self_hosted_durable' observability backends, sol deploy \
           probes the cluster for an in-cluster Loki (svc/loki -n monitoring) \
           and, if found, port-forwards to it for the duration of the push; \
           for 'external' there is no in-cluster Loki and no configured push \
           URL, so pass this flag to record the event at all. A push failure \
...
let cmd =
  Cmd.v
    (Cmd.info "deploy"
       ~doc:
         "Deploy pre-built images to a cluster (CI/CD integration). Like 'sol \
          up' but skips the build step — images must already be in the \
          registry. Takes a required TARGET positional \
          (<env>/<provider>/<region>, e.g. dev/aws/us-east-1), unlike 'sol up' \
          whose positional is the optional service-path filter — 'sol up' is \
          local-only and has no target to resolve.")
    Term.(
      const
        (fun
          target
        ...
        ->
          match
            Sol_cli_command_request.make_deploy_request ~target ~filter_path
              ~dry_run ~emit_to ~emit_plan_to ~image_tag ~registry
              ~secret_backend ~confirm_group_change ~loki_push_url ~git_sha
          with
          | Ok req -> run req
          | Error msg ->
              Printf.eprintf "error: %s\n" msg;
              exit 1)
      $ target_arg $ path_arg $ dry_run_flag $ emit_to_arg $ emit_plan_to_arg
      $ image_tag_arg $ registry_arg $ secret_backend_term
      $ confirm_group_change_flag $ loki_push_url_arg)
