...
let run (req : Sol_cli_command_request.deploy_request) =
  ...
  if not (Sys.file_exists (Sol_cli_config.target_file target_cfg)) then begin
    Printf.eprintf "error: no %s for target %S -- sol deploy requires an \
                     explicit target file, even an empty one, so a typo'd \
                     or unintended target can't silently inherit sol.yml's \
                     shared defaults and deploy anyway.\n"
      (Sol_cli_config.target_file target_cfg) req.target;
    exit 1
  end;
  ...
  Arg.(required & pos 0 (some string) None &
       info [] ~docv:"TARGET"
         ~doc:"Deployment target path: <env>/<provider>/<region>, e.g. \
               dev/aws/us-east-1 — same convention as 'sol plan'. Resolves \
               sol.yml + sol/<env>/<provider>/<region>.yml for registry/env \
               defaults. Unlike 'sol up' (local-only, no target concept), \
               this is required.")
  ...
  let build str store_ref store_kind key_prefix refresh_interval emit_to =
    ...
    | "external-secrets" ->
      (match store_ref with
       ...
       | Some sref ->
         `Ok (Sol_cli_manifest.External_secrets {
           store_ref        = sref;
  ...
  Arg.(value & opt (some string) None &
       info ["loki-push-url"] ~docv:"URL"
         ~doc:"Loki push URL for this deploy's release-event log line \
               ...
               observability backends, sol deploy probes the cluster for \
  ...
  Cmd.v
    (Cmd.info "deploy"
       ~doc:"Deploy pre-built images to a cluster (CI/CD integration). \
             Like 'sol up' but skips the build step — images must already \
             be in the registry. Takes a required TARGET positional \
             (<env>/<provider>/<region>, e.g. dev/aws/us-east-1), unlike \
             'sol up' whose positional is the optional service-path filter — \
             'sol up' is local-only and has no target to resolve.")
    Term.(const (fun target filter_path dry_run emit_to emit_plan_to image_tag registry
        ...
        match Sol_cli_command_request.make_deploy_request
