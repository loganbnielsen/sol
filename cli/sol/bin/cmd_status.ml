...
let dashboard_reachability ~backend ~base_domain =
  ...
  | Sol_cli_observability_url.Url url ->
    Sol_cli_status.reachability_of_probe ~probe_url:(Some url) ~is_reachable:http_reachable
...
let print_signal_line ~label ~signal ~backend ~explicit_url ~default_local_url ~probe_path =
  let probe_url = Sol_cli_status.probe_url ~backend ~explicit_url ~default_local_url ~probe_path in
  Printf.printf "  %-8s %s\n" label
...
let print_raw_diagnostics ~ns ~domain ~only_k8s_name =
  ...
  if ns_exists ns then begin
    ...
    Printf.printf "  (not deployed — run 'sol up')\n%!";
  Printf.printf "\n%!"
...
let print_domain_status ~workspace ~domain ~backend ~base_domain
  ...
  let status = Sol_cli_status.rollup_domain_status ~ns_exists:exists (List.map snd named) in
  Printf.printf "\n%s  %s  %s\n" domain
    (Sol_cli_observability_url.backend_to_string backend)
  ...
  if named = [] then Printf.printf "  (none)\n"
    ...
    List.iter (fun (k8s_name, diagnosis) ->
      let service_status = Sol_cli_status.rollup_domain_status ~ns_exists:true [diagnosis] in
      Printf.printf "  %-12s %s\n" k8s_name (Sol_cli_status.domain_status_to_string service_status)
    ) named;
...
let print_service_status ~workspace ~domain ~service_name ~backend ~base_domain
  ...
  Printf.printf "\n%s/%s  %s\n" domain k8s_name (Sol_cli_status.domain_status_to_string status);
  print_observability_block ~backend ~base_domain
    ~explicit_loki_url ~explicit_prometheus_url;
...
let run scope_str explicit_backend explicit_base_domain target
  ...
  let scope = match Sol_cli_open.parse_scope scope_str with
    | Ok s -> s
