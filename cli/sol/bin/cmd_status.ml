...
let declared_services ~domain :
    (string * string * Sol_cli_manifest.primitive) list =
  ...
  |> List.filter_map (fun (s : Sol_cli_manifest.service) ->
      ...
      | Ok k8s_name ->
          Some
            ( s.name,
              Sol_cli_deployment_plan.k8s_name_to_string k8s_name,
              s.primitive ))
...
let service_diagnoses_named ~ns ~domain : (string * string option) list =
  ...
  |> List.map (fun (service_name, k8s_name, primitive) ->
      ...
      in
      ( k8s_name,
        Sol_cli_rollout_diagnosis.diagnose_service_live ~pod_expectation ~ns
...
let curl_status_code url ~timeout_s : (int, string) result =
  match
    Sol_cli_process.run
      (Sol_cli_process.cmd ~timeout_s:(timeout_s +. 2.0)
         [
           "curl";
           "-s";
           "-o";
...
let dashboard_reachability ~backend ~base_domain =
  ...
  | Sol_cli_observability_url.Url url ->
      Sol_cli_status.reachability_of_probe ~probe_url:(Some url)
...
let print_signal_line ~label ~signal ~backend ~explicit_url ~default_local_url
    ~probe_path =
  let probe_url =
    Sol_cli_status.probe_url ~backend ~explicit_url ~default_local_url
      ~probe_path
  in
  Printf.printf "  %-8s %s\n" label
    (Sol_cli_status.reachability_line ~signal ~backend ~probe_url
       ~is_reachable:health_check_reachable)

let print_observability_lines ~backend ~explicit_loki_url
    ~explicit_prometheus_url =
  print_signal_line ~label:"logs" ~signal:Sol_cli_status.Loki ~backend
    ~explicit_url:explicit_loki_url ~default_local_url:"http://localhost:3100"
    ~probe_path:"/ready";
  print_signal_line ~label:"metrics" ~signal:Sol_cli_status.Prometheus ~backend
    ~explicit_url:explicit_prometheus_url
    ~default_local_url:"http://localhost:9090" ~probe_path:"/-/healthy";
  flush stdout

let print_observability_block ~backend ~base_domain ~explicit_loki_url
    ~explicit_prometheus_url =
  Printf.printf "\nObservability\n";
  print_observability_lines ~backend ~explicit_loki_url ~explicit_prometheus_url;
  Printf.printf "  dashboard  %s\n%!"
    (Sol_cli_status.reachability_to_string
       (dashboard_reachability ~backend ~base_domain))
...
let print_raw_diagnostics ~ns ~domain ~only_k8s_name =
  ...
  if ns_exists ns then begin
    ...
    let deploy_args =
      ...
      | Some k8s_name ->
          [ "get"; "deployments"; "-n"; ns; "-l"; "app=" ^ k8s_name ]
    in
    let image_jsonpath =
      "-o=jsonpath={range \
...
let print_workspace_index ~workspace ~domains ~backend ~explicit_loki_url
    ~explicit_prometheus_url =
  Printf.printf "\nDomains\n";
  List.iter
    (fun domain ->
      ...
      let status =
        Sol_cli_status.rollup_domain_status ~ns_exists:exists diagnoses
      in
      Printf.printf "  %-12s %s\n" domain
...
let print_domain_status ~workspace ~domain ~backend ~base_domain
  ...
  if named = [] then Printf.printf "  (none)\n"
    ...
    List.iter
      ...
      named;
  print_observability_block ~backend ~base_domain ~explicit_loki_url
    ~explicit_prometheus_url;
...
let print_service_status ~workspace ~domain ~service_name ~backend ~base_domain
  ...
  if not (Sol_cli_status.service_is_declared ~k8s_name declared_k8s_names) then begin
  ...
  let diagnosis =
    if exists then
      Sol_cli_rollout_diagnosis.diagnose_service_live ~pod_expectation ~ns
        ~service_name:k8s_name ~k8s_name ()
    else None
  in
  let status =
    Sol_cli_status.rollup_domain_status ~ns_exists:exists [ diagnosis ]
  in
  Printf.printf "\n%s/%s  %s\n" domain k8s_name
    (Sol_cli_status.domain_status_to_string status);
  print_observability_block ~backend ~base_domain ~explicit_loki_url
...
let run scope_str explicit_backend explicit_base_domain target explicit_loki_url
    explicit_prometheus_url =
  ...
  | Sol_cli_open.Domain domain | Sol_cli_open.Service (domain, _) -> (
      ...
      | Sol_cli_open.Service (_, service_name) ->
          print_service_status ~workspace ~domain ~service_name ~backend
            ~base_domain ~explicit_loki_url ~explicit_prometheus_url
      | _ ->
...
let loki_base_url_arg =
  Arg.(
    ...
    & info [ "loki-base-url" ] ~docv:"URL"
        ~doc:
          "Base URL of the Loki instance to check for the Observability \
           block's reachability line. When omitted: checked at \
           http://localhost:3100 for the local backend; for any other backend, \
           nothing is guessed and the line instead explains why and prints the \
           exact 'kubectl port-forward' command to run.")
...
let prometheus_base_url_arg =
  Arg.(
    ...
    & info [ "prometheus-base-url" ] ~docv:"URL"
        ~doc:
          "Base URL of the Prometheus instance to check for the Observability \
           block's reachability line. When omitted: checked at \
           http://localhost:9090 for the local backend; for any other backend, \
           nothing is guessed and the line instead explains why and prints the \
           exact 'kubectl port-forward' command to run.")
      ...
      const
        ...
        ->
          run scope
            ...
            base_domain target loki_base_url prometheus_base_url)
      $ domain_arg $ Cmd_logs.observability_backend_arg
      $ Cmd_logs.base_domain_arg $ Cmd_logs.target_arg $ loki_base_url_arg
