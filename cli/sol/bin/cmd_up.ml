...
let ensure_postgres_url () =
  if is_known_local_dev_context () then
    begin match Sys.getenv_opt "POSTGRES_URL" with
    | None | Some "" ->
...
let print_service_start spec =
  Printf.printf "[%s] %s/%s\n%!"
    (primitive_label
       (to_manifest_primitive spec.Sol_cli_deployment_plan.primitive))
...
let dry_run_service ~workspace ~sha
  ...
  match
    Sol_cli_up_execution.apply_service_manifest ~workspace ~dry_run:true
...
let apply_service ~workspace ~ctx_dir ~sha ~pf_failed
    (spec : Sol_cli_deployment_plan.service_spec) =
  let exec =
    Sol_cli_up_execution.service_execution ~workspace ~ctx_dir ~sha spec
  in
  ...
  (match
     Sol_cli_up_execution.apply_service_manifest ~workspace ~dry_run:false spec
   with
  | Ok _ -> ()
  | Error msg -> raise (Deploy_failed msg));
  ...
  | Sol_cli_deployment_plan.Svc ->
      ...
      if not (Sol_cli_port_forward.is_running exec.k8s_name) then begin
        if
          Sol_cli_port_forward.detect_stale ~local_port
      ...
      let pf_alive =
        Sol_cli_port_forward.check_alive ~name:exec.k8s_name ~local_port
      in
...
let run_dry_run ~workspace ~sha ~services =
  ...
  List.iter
    (dry_run_service ~workspace ~sha)
    plan.Sol_cli_deployment_plan.services

let run_apply ~workspace ~sha ~filter_path ~services ~repo_root
  ...
  Sol_cli_up_execution.remove_build_context ~ctx_dir;
  let summary =
    Sol_cli_up_execution.post_deploy_summary ~cwd:(Sys.getcwd ()) plan
  in
  ...
  if summary.pending_migrations > 0 then
    Printf.printf
      "\n\
       Note: %d migration file(s) found in db/migrations/ — run 'sol migrate' \
       to apply.\n"
...
let run (req : Sol_cli_command_request.up_request) =
  ...
  | Apply ->
      ...
      run_apply ~workspace ~sha ~filter_path:req.filter_path ~services
        ~repo_root ~confirm_group_change:req.confirm_group_change
...
let path_arg =
  Arg.(
    ...
    & info [] ~docv:"PATH"
        ~doc:
          "Service path to build and deploy (default: all services in \
           workspace). 'sol up' is local-only and has no target concept, so \
           unlike 'sol deploy TARGET [path]' this positional is the optional \
...
let cmd =
  Cmd.v
    (Cmd.info "up"
       ~doc:
         "Build images, synthesize k8s manifests, and deploy to the local \
          cluster. Local-only — no target concept, unlike 'sol deploy'.")
    Term.(
      const (fun filter_path dry_run tag confirm_group_change ->
          match
            Sol_cli_command_request.make_up_request ~filter_path ~dry_run ~tag
              ~confirm_group_change ~git_sha
          with
          | Ok req -> run req
          | Error msg ->
              Printf.eprintf "error: %s\n" msg;
