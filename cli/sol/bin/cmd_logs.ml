...
let workload_exists ~ns ~primitive ~k8s_name =
  let kind = match (primitive : Sol_cli_manifest.primitive) with
    | Fn -> "cronjob"
    | Svc | Worker -> "deployment"
  in
  match Sol_cli_process.run
      (Sol_cli_process.cmd ["kubectl"; "get"; kind; k8s_name; "-n"; ns]) with
...
let run ~service_arg ~follow ~tail ~explicit_backend ~explicit_base_domain
  ...
  let (backend, base_domain) =
    match Sol_cli_observability_url.effective_backend_and_base_domain
            ~explicit_backend ~explicit_base_domain ~target () with
    | Error msg -> Printf.eprintf "error: %s\n" msg; exit 1
    | Ok pair -> pair
  in
  (match Sol_cli_observability_url.resolve ~backend ?base_domain
  ...
  if follow then
    ...
    match Sol_cli_status.probe_url ~backend ~explicit_url:explicit_loki_url
    ...
    | Some loki_base_url ->
      let credentials =
        ...
        | Error msg -> Printf.eprintf "error: %s\n" msg; exit 1
      in
      match Sol_cli_loki.query ~base_url:loki_base_url ~ns ~k8s_name
  ...
  Arg.(required & pos 0 (some string) None &
       info [] ~docv:"SERVICE"
         ~doc:"Service to stream logs from, in 'domain/name' or bare 'name' form \
               ...
               Underscores are accepted; Sol maps them to hyphens in Kubernetes.")
  ...
  Arg.(value & opt (some string) None &
       info ["grafana-base-url"] ~docv:"URL"
         ~doc:"Override the Grafana base URL instead of resolving it from \
               --observability-backend. Sol prints a copyable Grafana \
...
let observability_backend_arg =
  Arg.(value & opt (some string) None &
       info ["observability-backend"] ~docv:"BACKEND"
         ~doc:"Which observability_backend (see cli/platform/infra/base) this \
...
let target_arg =
  Arg.(value & opt (some string) None &
       info ["target"] ~docv:"ENV/PROVIDER/REGION"
         ~doc:"Deployment target path (same as sol plan/sol cloud tf, e.g. \
               prod/aws/us-east-1). When given, its sol.yml config supplies \
  ...
  Arg.(value & opt (some string) None &
       info ["loki-base-url"] ~docv:"URL"
         ~doc:"Base URL of the Loki instance. When omitted: checked at \
               ...
               is skipped and 'sol logs' goes straight to 'kubectl logs' \
               rather than guessing. 'sol logs' queries Loki first for a \
  ...
  Arg.(value & opt (some string) None &
       info ["loki-username"] ~docv:"USERNAME"
         ~doc:"Basic-auth username for the Loki query request (e.g. a \
               ...
               (cli/platform/infra/base). Falls back to SOL_LOKI_USERNAME; \
               the flag wins when both are set. Must be paired with \
               --loki-password (or SOL_LOKI_PASSWORD).")
  ...
  Arg.(value & opt (some string) None &
       info ["loki-password"] ~docv:"PASSWORD"
         ~doc:"Basic-auth password/API key for the Loki query request -- \
               ...
               external_loki_password (cli/platform/infra/base). Falls back to \
               SOL_LOKI_PASSWORD; the flag wins when both are set. Must \
               be paired with --loki-username (or SOL_LOKI_USERNAME). Prefer \
               SOL_LOKI_PASSWORD on shared hosts because command-line flags \
  ...
  Cmd.v
    (Cmd.info "logs"
       ~doc:"Stream logs from a deployed service. \
             Wraps 'kubectl logs' with Sol's namespace convention \
             (<workspace>-<domain>).")
