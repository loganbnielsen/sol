    ...
    (try
       Array.iter
         (fun domain ->
           ...
           if domain.[0] <> '.' && Sys.is_directory domain_path then
             begin try
               Array.iter
                 (fun svc ->
                   if svc.[0] <> '.' then begin
...
let resolve_service arg =
  if String.contains arg '/' then
    ...
    | _ ->
        Printf.eprintf
          "error: service path must be in 'domain/name' form, got '%s'.\n" arg;
        exit 1
    end
  else
    begin match find_service_by_name arg with
    | [] ->
        Printf.eprintf "error: service '%s' not found in app/.\n" arg;
        Printf.eprintf
          "  Use 'domain/name' form or run from the workspace root.\n";
        exit 1
    | [ (domain, name) ] -> Some (domain, name)
    | matches ->
        ...
        Printf.eprintf "  Specify the full path, e.g. 'sol logs %s/%s'.\n"
          (fst (List.hd matches))
          arg;
        exit 1
    end
...
let run ~service_arg ~follow ~tail ~explicit_backend ~explicit_base_domain
  ...
  let domain, name =
    match resolve_service service_arg with Some p -> p | None -> exit 1
  in
  ...
  in
  (match
     Sol_cli_observability_url.resolve ~backend ?base_domain
  ...
  let fallback_to_kubectl () =
    ...
    (match
       Sol_cli_rollout_diagnosis.diagnose_service_live
         ~pod_expectation:
           (Sol_cli_status.pod_expectation_of_primitive primitive)
         ~ns ~service_name:name ~k8s_name ()
     with
    | Some diagnosis -> Printf.printf "%s\n%!" diagnosis
    | None -> ());
    exec_kubectl_logs ~ns ~target:kubectl_target ~follow ~tail
  in
  if follow then fallback_to_kubectl ()
  else
    match
      Sol_cli_status.probe_url ~backend ~explicit_url:explicit_loki_url
        ~default_local_url:"http://localhost:3100" ~probe_path:""
    with
    | None ->
        ...
        Printf.printf "(%s. Showing Kubernetes logs.)\n%!"
          (Sol_cli_status.not_configured_message ~signal:Sol_cli_status.Loki
             ~backend);
        fallback_to_kubectl ()
    | Some loki_base_url -> (
        let credentials =
        ...
        in
        match
          Sol_cli_loki.query ~base_url:loki_base_url ~ns ~k8s_name ?credentials
            ~limit:tail ()
        with
        | Ok [] ->
            Printf.printf
        ...
        | Ok lines ->
            List.iter
              (fun (l : Sol_cli_loki.line) -> print_endline l.text)
              lines
        | Error e ->
           ...
           wrong URL, not the "nothing to check" case above. *)
            Printf.printf "(%s. Falling back to Kubernetes logs for %s...)\n%!"
              (Sol_cli_status.unreachable_message ~url:loki_base_url
...
let service_arg =
  Arg.(
    ...
    & info [] ~docv:"SERVICE"
        ~doc:
          "Service to stream logs from, in 'domain/name' or bare 'name' form \
           (e.g. payments/charge_svc or charge_svc). Underscores are accepted; \
           Sol maps them to hyphens in Kubernetes.")
...
let follow_flag =
  Arg.(
    ...
    & info [ "follow"; "f" ]
        ~doc:
          "Stream logs in real time. This is the default; pass --no-follow for \
           a snapshot.")
...
let tail_arg =
  Arg.(
    ...
    & info [ "tail" ] ~docv:"N"
        ~doc:
          "Number of recent lines to show before following (default: 100). \
    ...
    & info [ "grafana-base-url" ] ~docv:"URL"
        ~doc:
          "Override the Grafana base URL instead of resolving it from \
           --observability-backend. Sol prints a copyable Grafana Explore URL \
           with a LogQL query before streaming kubectl logs.")
...
let observability_backend_arg =
  Arg.(
    ...
    & info
        ...
        ~docv:"BACKEND"
        ~doc:
          "Which observability_backend (see cli/platform/infra/base) this \
           target uses: local, self_hosted_durable, or external. Overrides \
           whatever --target's config supplies; defaults to local when neither \
           is given. Determines the Grafana URL Sol resolves when \
    ...
    & info [ "base-domain" ] ~docv:"DOMAIN"
        ~doc:
          "Base domain for the self_hosted_durable backend's Grafana Ingress \
           (grafana.<base-domain>). Overrides whatever --target's config \
           supplies. Required for that backend unless --grafana-base-url \
    ...
    & info [ "target" ] ~docv:"ENV/PROVIDER/REGION"
        ~doc:
          "Deployment target path (same as sol plan/sol cloud tf, e.g. \
           ...
           observability_backend/base_domain defaults instead of the hardcoded \
           local default.")
...
let loki_base_url_arg =
  Arg.(
    ...
    & info [ "loki-base-url" ] ~docv:"URL"
        ~doc:
          "Base URL of the Loki instance. When omitted: checked at \
           http://localhost:3100 for the local backend, otherwise Loki is \
           skipped and 'sol logs' goes straight to 'kubectl logs' rather than \
           guessing. 'sol logs' queries Loki first for a snapshot \
           (--no-follow) and falls back to 'kubectl logs' if the query fails, \
...
let loki_username_arg =
  Arg.(
    ...
    & info [ "loki-username" ] ~docv:"USERNAME"
        ~doc:
          "Basic-auth username for the Loki query request (e.g. a Grafana \
           Cloud stack's instance ID) -- the read-side counterpart of Alloy's \
           external_loki_username (cli/platform/infra/base). Falls back to \
           SOL_LOKI_USERNAME; the flag wins when both are set. Must be paired \
           with --loki-password (or SOL_LOKI_PASSWORD).")
...
let loki_password_arg =
  Arg.(
    ...
    & info [ "loki-password" ] ~docv:"PASSWORD"
        ~doc:
          "Basic-auth password/API key for the Loki query request -- the \
           read-side counterpart of Alloy's external_loki_password \
           (cli/platform/infra/base). Falls back to SOL_LOKI_PASSWORD; the \
           flag wins when both are set. Must be paired with --loki-username \
           (or SOL_LOKI_USERNAME). Prefer SOL_LOKI_PASSWORD on shared hosts \
           because command-line flags can be visible in shell history and \
           process listings.")
...
let follow_term =
  let combine follow no_follow =
    ...
    | true, true ->
        Printf.eprintf
          "error: --follow and --no-follow are mutually exclusive\n%!";
        exit 1
    | _, true -> false
    | _, false -> true
  in
...
let cmd =
  Cmd.v
    (Cmd.info "logs"
       ~doc:
         "Stream logs from a deployed service. Wraps 'kubectl logs' with Sol's \
      ...
      const
        ...
        ->
          ...
          run ~service_arg ~follow ~tail ~explicit_backend
            ...
            ~explicit_loki_password:loki_password ?grafana_base_url ())
      $ service_arg $ follow_term $ tail_arg $ observability_backend_arg
      $ base_domain_arg $ target_arg $ grafana_base_url_arg $ loki_base_url_arg
