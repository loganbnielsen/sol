...
let print_outputs infra_dir =
  ...
  | Ok r -> (
      try
        ...
        match json with
        | `Assoc pairs ->
            List.iter (fun (key, obj) -> print_output_field key obj) pairs
        | _ -> ()
      with _ -> Printf.printf "  (error parsing terraform outputs)\n%!")
...
let configure_kubectl infra_dir =
  ...
  | Some kubeconfig_command -> (
      ...
      | Ok r when r.Sol_cli_process.exit_code = 0 ->
          Printf.printf
            "  kubectl configured -- sol status/deploy/migrate can reach this \
             cluster now.\n\
             %!"
      | _ ->
          Printf.printf
            "  (could not auto-configure kubectl -- run this yourself before \
             using sol status/deploy/migrate:)\n\
            \  %s\n\
             %!"
            kubeconfig_command)
...
let infra_dir provider =
  ...
  let dir =
    Filename.concat sol_home (Printf.sprintf "cli/platform/infra/%s" pname)
  in
...
let exit_code_of r =
  match r with Ok r -> r.Sol_cli_process.exit_code | Error _ -> 1
...
let require_terraform_success r =
  match r with Ok r when r.Sol_cli_process.exit_code = 0 -> () | _ -> exit 1
...
let aws_no_ecr_repositories ~region ~workspace_name =
  ...
  let query =
    Printf.sprintf
      "repositories[?starts_with(repositoryName, `%s`)].repositoryName" prefix
  in
  match
    Sol_cli_process.run
      (Sol_cli_process.cmd
  ...
  | Error _ ->
      Printf.eprintf
        "error: AWS ECR verification failed: aws CLI unavailable.\n";
      false
...
let aws_no_load_balancers ~region ~cluster_name =
  ...
  | None ->
      Printf.eprintf
        "error: AWS load balancer verification failed: aws CLI unavailable or \
         errored.\n";
      false
...
let delete_loadbalancer_services ~region ~cluster_name =
  ...
  if not update_ok then
    ...
    | Some context ->
        ...
        (match
           Sol_cli_process.run ~echo:false
...
{end}|};
        ...
        | Ok r when r.Sol_cli_process.exit_code = 0 ->
            ...
            if services <> [] then begin
              Printf.printf
                "  Deleting %d LoadBalancer Service(s) before terraform \
                 destroy (AUDIT-064) -- their AWS load balancer isn't tracked \
                 by Terraform and must be removed first:\n\
                 %!"
              ...
              let rec wait_for_lbs_gone n =
                if n = 0 then
                  Printf.printf
                    "  (warning: load balancer(s) may still be deprovisioning \
        ...
        | _ ->
            Printf.printf
              "  (could not list Services in cluster %s -- skipping \
...
let verify_aws_destroy ~var_files ~vars =
  ...
  | Some cluster_name ->
      ...
      Printf.printf
        "  AWS verification passed: EKS/RDS/ECR/load-balancers not found.\n%!"
      ...
      | Ok cfg -> (
          ...
          | Some resolved_target -> (
           ...
           defaults and terraform apply/destroy anyway. *)
              ...
              then begin
                Printf.eprintf
                  "error: no %s for target %S -- terraform apply/destroy \
                   require an explicit target file, even an empty one, so a \
                   typo'd or unintended target can't silently inherit \
                   sol.yml's shared defaults and mutate infrastructure anyway.\n"
                  (Sol_cli_config.target_file resolved_target)
                  target_path;
                exit 1
              end;
              match
                Sol_cli_config.terraform_vars ~workspace:(workspace_name ()) cfg
              with
              | Error msg ->
                  Printf.eprintf "error: %s\n" msg;
                  exit 1
...
let cloud_init ~target ~var_file ~vars ~action () =
  ...
  let var_file =
    match var_file with Some _ -> var_file | None -> config_var_file
  in
  let vars = config_vars @ vars in
  let var_files =
    match var_file with None -> [] | Some f -> [ normalize_var_file f ]
  in
  match action with
  | Plan ->
      require_terraform_success
  ...
  | Apply ->
      ...
      | Sol_cli_provider.Aws -> (
          ...
          | Some cluster_name ->
              let region =
                Option.value
                  (resolved_var "region" ~var_files ~vars
...
let var_file_arg =
  Arg.(
    ...
    & info [ "var-file" ] ~docv:"PATH"
        ~doc:
          "Path to a Terraform .tfvars file. Passed as -var-file to terraform.")
...
let plan_flag =
  Arg.(
    ...
    & info [ "plan" ]
        ~doc:
          "Run terraform plan only. No infrastructure is changed. This is the \
           default.")
...
let destroy_cmd =
  Cmd.v
    (Cmd.info "destroy"
       ~doc:
         "Destroy cloud infrastructure via Terraform. Requires the same \
          target/provider used with apply.")
    Term.(
