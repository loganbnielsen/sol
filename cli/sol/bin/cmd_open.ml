...
let run kind scope_str links explicit_backend explicit_base_domain target
    grafana_base_url =
    ...
    & info [] ~docv:"SCOPE"
        ~doc:
          "Scope to open: omit for the workspace view, 'domain' for a domain, \
           'domain/service' for a single service, or 'resource/<type>/<name>' \
           for a managed infrastructure resource dashboard (OBS-044), e.g. \
  ...
  Cmd.v (Cmd.info name ~doc)
    Term.(
      const (fun scope_str links backend base_domain target grafana_base_url ->
          run kind scope_str links (backend_of_arg backend) base_domain target
            grafana_base_url)
      $ scope_arg $ links_flag $ Cmd_logs.observability_backend_arg
      $ Cmd_logs.base_domain_arg $ Cmd_logs.target_arg
...
let cmd =
  Cmd.group
    ...
    [
      make_subcmd "logs" Sol_cli_open.Logs
        "Open (or print) the Grafana Explore logs view.";
      make_subcmd "metrics" Sol_cli_open.Metrics
        "Open (or print) the Grafana metrics dashboard.";
      make_subcmd "dashboard" Sol_cli_open.Dashboard
