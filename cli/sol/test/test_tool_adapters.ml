...
let test_kubectl_apply_dry_run_argv () =
  let c = Sol_cli_process.cmd ["kubectl"; "apply"; "-f"; "x.yaml"; "--dry-run=server"] in
  check_bool "has dry-run flag"
    true (List.mem "--dry-run=server" c.Sol_cli_process.argv)

let test_kubectl_get_argv () =
  let c = Sol_cli_process.cmd
...
let test_docker_push_argv () =
  ...
  check_str "image_ref"  "registry.example.com/myapp:v1" (List.nth c.Sol_cli_process.argv 2)

let test_docker_build_failure () =
  assert_error
    (Sol_cli_docker.build ~tag:"test:v0"
...
let test_helm_upgrade_install_argv () =
  ...
  check_bool "has --create-namespace"
    true (List.mem "--create-namespace" c.Sol_cli_process.argv);
  check_bool "has --wait"
    true (List.mem "--wait" c.Sol_cli_process.argv)
...
let test_terraform_plan_argv () =
  let c = Sol_cli_process.cmd
    ["terraform"; "-chdir=/some/dir"; "plan"; "-var-file=prod.tfvars"; "-var=cluster_name=sol-smoke"] in
  check_str "argv[2]" "plan" (List.nth c.Sol_cli_process.argv 2);
  check_bool "has var-file"
    true (List.mem "-var-file=prod.tfvars" c.Sol_cli_process.argv);
  check_bool "has var"
    true (List.mem "-var=cluster_name=sol-smoke" c.Sol_cli_process.argv)
...
let test_terraform_plan_destroy_argv () =
  ...
  check_bool "has -destroy"
    true (List.mem "-destroy" c.Sol_cli_process.argv)
...
let test_terraform_apply_argv () =
  ...
  check_bool "has -auto-approve"
    true (List.mem "-auto-approve" c.Sol_cli_process.argv)
