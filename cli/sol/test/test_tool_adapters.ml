...
let test_kubectl_apply_dry_run_argv () =
  ...
  in
  check_bool "has dry-run flag" true
    (List.mem "--dry-run=server" c.Sol_cli_process.argv)
...
let test_kubectl_get_failure () =
  assert_error
    (Sol_cli_kubectl.get ~resource:"pod" ~name:"nonexistent-abc123"
...
let test_kubectl_probe_false_on_missing_binary () =
  let result =
    Sol_cli_kubectl.probe ~args:[ "nonexistent-abc123-subcommand" ]
  in
...
let test_docker_build_argv () =
  ...
  check_str "dockerfile" "/ctx/svc/Dockerfile"
    (List.nth c.Sol_cli_process.argv 5);
  check_str "context" "/ctx" (List.nth c.Sol_cli_process.argv 6)

let test_docker_push_argv () =
  let c =
    Sol_cli_process.cmd [ "docker"; "push"; "registry.example.com/myapp:v1" ]
  in
  check_str "argv[1]" "push" (List.nth c.Sol_cli_process.argv 1);
  check_str "image_ref" "registry.example.com/myapp:v1"
    (List.nth c.Sol_cli_process.argv 2)

let test_docker_build_failure () =
  assert_error
    (Sol_cli_docker.build ~tag:"test:v0" ~dockerfile:"/nonexistent/Dockerfile"
       ~context:"/nonexistent")

let test_docker_push_failure () =
  assert_error
    (Sol_cli_docker.push ~image_ref:"localhost:9999/nonexistent:nope")
...
let test_helm_repo_add_argv () =
  ...
  check_str "url" "https://charts.bitnami.com/bitnami"
...
let test_helm_upgrade_install_argv () =
  ...
  check_bool "has --create-namespace" true
...
let test_helm_set_flags_str () =
  let c =
    Sol_cli_process.cmd
      ...
      @ [ "--wait"; "--timeout"; "3m" ])
  in
  check_bool "has --set-string" true
    (List.mem "--set-string" c.Sol_cli_process.argv)
...
let test_terraform_plan_argv () =
  ...
  check_bool "has var-file" true
    (List.mem "-var-file=prod.tfvars" c.Sol_cli_process.argv);
  check_bool "has var" true
    (List.mem "-var=cluster_name=sol-smoke" c.Sol_cli_process.argv)

let test_terraform_plan_destroy_argv () =
  let c =
    Sol_cli_process.cmd [ "terraform"; "-chdir=/some/dir"; "plan"; "-destroy" ]
  in
  check_str "argv[2]" "plan" (List.nth c.Sol_cli_process.argv 2);
...
let test_terraform_apply_argv () =
  let c =
    Sol_cli_process.cmd
      [ "terraform"; "-chdir=/some/dir"; "apply"; "-auto-approve" ]
  in
  check_str "argv[2]" "apply" (List.nth c.Sol_cli_process.argv 2);
  check_bool "has -auto-approve" true
    (List.mem "-auto-approve" c.Sol_cli_process.argv)

let test_terraform_destroy_argv () =
  let c =
    Sol_cli_process.cmd
      [ "terraform"; "-chdir=/some/dir"; "destroy"; "-auto-approve" ]
  in
    ...
    [
      ( "kubectl_argv",
        [
          ...
          Alcotest.test_case "apply dry_run argv" `Quick
            test_kubectl_apply_dry_run_argv;
          Alcotest.test_case "get argv" `Quick test_kubectl_get_argv;
          Alcotest.test_case "rollout status argv" `Quick
            test_kubectl_rollout_status_argv;
          Alcotest.test_case "rollout undo argv" `Quick
            test_kubectl_rollout_undo_argv;
          Alcotest.test_case "rollout restart argv" `Quick
            test_kubectl_rollout_restart_argv;
          Alcotest.test_case "patch argv" `Quick test_kubectl_patch_argv;
          Alcotest.test_case "config current-context" `Quick
            test_kubectl_config_current_context_argv;
        ] );
      ( "kubectl_failures",
        [
          Alcotest.test_case "apply propagates error" `Quick
            test_kubectl_apply_failure;
          Alcotest.test_case "get propagates error" `Quick
            test_kubectl_get_failure;
          Alcotest.test_case "probe false for missing" `Quick
      ...
      ( "docker_failures",
        [
          Alcotest.test_case "build propagates error" `Quick
            test_docker_build_failure;
          Alcotest.test_case "push propagates error" `Quick
            test_docker_push_failure;
          Alcotest.test_case "inspect digest fallback" `Quick
            test_docker_inspect_digest_fallback;
        ] );
      ( "helm_argv",
        [
          Alcotest.test_case "repo add argv" `Quick test_helm_repo_add_argv;
          Alcotest.test_case "repo update argv" `Quick
            test_helm_repo_update_argv;
          Alcotest.test_case "upgrade install argv" `Quick
            test_helm_upgrade_install_argv;
          Alcotest.test_case "upgrade install --version argv" `Quick
      ...
      ( "terraform_argv",
        [
          ...
          Alcotest.test_case "plan destroy argv" `Quick
            test_terraform_plan_destroy_argv;
          ...
          Alcotest.test_case "output json argv" `Quick
            test_terraform_output_json_argv;
          Alcotest.test_case "which_check returns bool" `Quick
