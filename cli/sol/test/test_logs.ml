...
let test_encode_space () =
  check_string "space encoded" "hello%20world"
    (Sol_cli_logs.url_encode_logql "hello world")

let test_encode_plain_chars () =
  check_string "plain alphanum unchanged" "abcXYZ0123"
    (Sol_cli_logs.url_encode_logql "abcXYZ0123")

let test_encode_percent () =
  check_string
    "percent encoded (a raw % would look like a malformed escape to a URL \
     parser)"
    "50%25"
...
let test_encode_question_mark () =
  check_string "question mark encoded" "a%3Fb"
    (Sol_cli_logs.url_encode_logql "a?b")

let test_encode_hash () =
  check_string "hash encoded (unescaped would start a URL fragment)" "a%23b"
    (Sol_cli_logs.url_encode_logql "a#b")
...
let test_url_no_raw_braces () =
  ...
  let query_start =
    try Str.search_forward (Str.regexp "?") url 0 with Not_found -> 0
  in
...
let test_url_default_base () =
  let url =
    Sol_cli_logs.grafana_explore_url ~base_url:"http://localhost:3000"
      ~ns:"ws-dom" ~k8s_name:"my-svc"
  in
...
let test_kubectl_logs_deployment_target () =
  check_string "argv"
    ...
    (String.concat " "
       (Sol_cli_logs.kubectl_logs_argv ~ns:"acme-payments"
          ~target:(Sol_cli_logs.Deployment "charge-svc") ~follow:true ~tail:50))

let test_kubectl_logs_fn_target () =
  check_string "argv"
    "kubectl logs -n acme-billing -l app=invoice-fn --all-containers=true \
     --tail=25"
    (String.concat " "
       (Sol_cli_logs.kubectl_logs_argv ~ns:"acme-billing"
          ~target:(Sol_cli_logs.App_selector "invoice-fn") ~follow:false
          ~tail:25))
...
let () =
  Alcotest.run "logs"
    [
      ...
      ( "grafana_explore_url",
        [
          Alcotest.test_case "contains base_url" `Quick
            test_url_contains_base_url;
          Alcotest.test_case "contains namespace" `Quick
            test_url_contains_namespace;
          Alcotest.test_case "contains k8s_name" `Quick
            test_url_contains_k8s_name;
          Alcotest.test_case "no raw braces in query" `Quick
            test_url_no_raw_braces;
          Alcotest.test_case "= encoded as %3D" `Quick
            test_url_no_raw_equals_in_logql;
          Alcotest.test_case "no raw double-quotes" `Quick
            test_url_no_raw_double_quotes;
          Alcotest.test_case "default base prefix" `Quick test_url_default_base;
        ] );
      ( "kubectl_logs_argv",
        [
          Alcotest.test_case "deployment target" `Quick
