...
let test_profile_overrides_common () =
  with_fake_sol_home ~component:"widget"
    ...
    (fun () ->
      let merged =
        Sol_cli_platform_component.merged_values_yaml ~component:"widget"
          ~profile:"local"
      in
      let json = Yojson.Safe.from_string merged in
      (* profile's scalar wins outright *)
      check_str "a" "2"
        (Yojson.Safe.to_string (Yojson.Safe.Util.member "a" json));
...
let suite =
  [
    ( "platform_component",
      [
        Alcotest.test_case "profile overrides common, deep-merged" `Quick
          test_profile_overrides_common;
        Alcotest.test_case "missing profile file treated as empty" `Quick
          test_missing_profile_file_is_empty_object;
        Alcotest.test_case "missing common file treated as empty" `Quick
