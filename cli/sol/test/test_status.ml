...
let test_one_degraded () =
  check_bool "one Some -> Degraded" true
    (S.rollup_domain_status ~ns_exists:true
       [ None; Some "charge-svc rollout failed" ]
    = S.Degraded)
...
let test_domain_status_to_string () =
  check_bool "Healthy label" true
    (S.domain_status_to_string S.Healthy = "healthy");
...
let test_reachability_to_string () =
  check_bool "Healthy label" true
    (S.reachability_to_string S.Healthy = "healthy");
  check_bool "Unreachable label" true
    (S.reachability_to_string S.Unreachable = "unreachable");
  check_bool "Not_checked label" true
...
let test_not_configured_message_names_backend_and_flag () =
  ...
  check_bool "mentions the port-forward command" true
    (let re =
       Str.regexp_string "kubectl port-forward -n monitoring svc/loki 3100:3100"
     in
     try
       ignore (Str.search_forward re msg 0);
       true
...
let test_not_configured_message_distinct_from_unreachable_message () =
  let not_configured =
    S.not_configured_message ~signal:S.Loki ~backend:O.External
  in
  let unreachable =
...
let test_unreachable_message_names_url_and_error () =
  let msg =
    S.unreachable_message ~url:"http://loki.example:3100"
...
let test_reachability_line_not_configured () =
  check_bool "no probe_url -> not_configured_message" true
    (S.reachability_line ~signal:S.Loki ~backend:O.Self_hosted_durable
       ~probe_url:None ~is_reachable:(fun _ -> Ok ())
    = S.not_configured_message ~signal:S.Loki ~backend:O.Self_hosted_durable)

let test_reachability_line_healthy () =
  check_bool "reachable -> \"healthy\"" true
    (S.reachability_line ~signal:S.Loki ~backend:O.Local
       ~probe_url:(Some "http://x") ~is_reachable:(fun _ -> Ok ())
    = "healthy")

let test_reachability_line_unreachable () =
  check_bool "unreachable -> unreachable_message" true
    (S.reachability_line ~signal:S.Prometheus ~backend:O.External
       ~probe_url:(Some "http://x") ~is_reachable:(fun _ ->
         Error "connection failed")
    = S.unreachable_message ~url:"http://x" ~error:"connection failed")
    ...
    [
      ( "rollup_domain_status",
        [
          ...
          Alcotest.test_case "not deployed overrides" `Quick
      ...
      ( "probe_url",
        [
          Alcotest.test_case "explicit always wins" `Quick
            test_probe_url_explicit_always_wins;
          Alcotest.test_case "local default when no explicit" `Quick
            test_probe_url_local_default_when_no_explicit;
          Alcotest.test_case "non-local without explicit -> None" `Quick
            test_probe_url_non_local_without_explicit_is_none;
        ] );
      ( "reachability_of_probe",
        [
          Alcotest.test_case "None -> Not_checked" `Quick
            test_reachability_of_probe_not_checked;
          Alcotest.test_case "reachable -> Healthy" `Quick
            test_reachability_of_probe_healthy;
          Alcotest.test_case "unreachable -> Unreachable" `Quick
            test_reachability_of_probe_unreachable;
          Alcotest.test_case "reachability_to_string labels" `Quick
            test_reachability_to_string;
        ] );
      ( "not_configured_message / unreachable_message",
        [
          Alcotest.test_case "names backend and flag" `Quick
            test_not_configured_message_names_backend_and_flag;
          Alcotest.test_case
            "prometheus signal uses prometheus flag/port-forward" `Quick
            test_not_configured_message_prometheus_signal;
          Alcotest.test_case "no trailing period" `Quick
            test_not_configured_message_has_no_trailing_period;
          Alcotest.test_case "distinct from unreachable_message" `Quick
            test_not_configured_message_distinct_from_unreachable_message;
          Alcotest.test_case "names url and error" `Quick
            test_unreachable_message_names_url_and_error;
        ] );
      ( "reachability_line",
        [
          Alcotest.test_case "no probe_url -> not_configured_message" `Quick
            test_reachability_line_not_configured;
          Alcotest.test_case "reachable -> \"healthy\"" `Quick
            test_reachability_line_healthy;
          Alcotest.test_case "unreachable -> unreachable_message" `Quick
            test_reachability_line_unreachable;
        ] );
      ( "service_is_declared",
        [
          Alcotest.test_case "declared name -> true" `Quick
            test_service_is_declared_true_for_declared_name;
          Alcotest.test_case "undeclared name -> false" `Quick
            test_service_is_declared_false_for_undeclared_name;
          Alcotest.test_case "empty domain -> false" `Quick
            test_service_is_declared_false_for_empty_domain;
        ] );
      ( "pod_expectation_of_primitive",
        [
          Alcotest.test_case "maps each primitive" `Quick
