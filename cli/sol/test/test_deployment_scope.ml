...
let test_parses_domain_and_unit () =
  Alcotest.(check (result string string)) "domain" (Ok "payments") (parsed (Some "payments"));
...
let test_workspace_selects_everything () =
  match select_named Whole_workspace (units ()) with
...
let test_domain_selects_its_units () =
  match select_named (Whole_domain "payments") (units ()) with
  | Error message -> Alcotest.fail message
  | Ok (scope, selected) ->
...
let test_unit_takes_its_kind_from_discovery () =
  ...
  match select_named (Unit_named ("payments", "settle_worker")) (units ()) with
  | Error message -> Alcotest.fail message
  | Ok (scope, selected) ->
    (match scope with
     | Unit { kind = Worker; _ } -> ()
...
let test_unknown_domain_fails_closed () =
  match select_named (Whole_domain "logistics") (units ()) with
...
let test_unknown_unit_names_what_exists () =
  match select_named (Unit_named ("payments", "refund_svc")) (units ()) with
...
let test_unit_in_unknown_domain_lists_domains () =
  match select_named (Unit_named ("logistics", "ship_worker")) (units ()) with
...
let () =
  Alcotest.run
    "deployment_scope"
    [ ( "deployment_scope"
      , [ Alcotest.test_case "absent means the whole workspace" `Quick test_absent_is_the_whole_workspace
        ; Alcotest.test_case "parses a domain and a unit" `Quick test_parses_domain_and_unit
        ; Alcotest.test_case
            "rejects a path"
            `Quick
            test_rejects_what_it_cannot_understand
        ; Alcotest.test_case "workspace selects everything" `Quick test_workspace_selects_everything
        ; Alcotest.test_case "domain selects its units" `Quick test_domain_selects_its_units
        ; Alcotest.test_case
