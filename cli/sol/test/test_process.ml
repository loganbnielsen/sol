...
let test_captured_stderr () =
  let r = ok_result (Sol_cli_process.run
    (Sol_cli_process.cmd ["sh"; "-c"; "echo oops >&2; exit 1"])) in
...
let test_stdout_and_stderr_separate () =
  let r = ok_result (Sol_cli_process.run
    (Sol_cli_process.cmd ["sh"; "-c"; "echo out; echo err >&2"])) in
  check_str "stdout" "out" r.Sol_cli_process.stdout;
  check_str "stderr" "err" r.Sol_cli_process.stderr

let test_spawn_failed () =
  match err_result (Sol_cli_process.run
    (Sol_cli_process.cmd ["/nonexistent-binary-xyz"])) with
...
let test_chdir_failed () =
  match err_result (Sol_cli_process.run
    (Sol_cli_process.cmd ~cwd:"/nonexistent-dir-xyz" ["pwd"])) with
...
let test_redaction_in_echo () =
  ...
  let output = capture_stdout (fun () ->
    ignore (Sol_cli_process.run ~echo:true
...
let test_no_shell_expansion () =
  let r = ok_result (Sol_cli_process.run
    (Sol_cli_process.cmd ["echo"; "$HOME"])) in
...
let test_error_to_string_nonzero () =
  let s = Sol_cli_process.error_to_string
    (Sol_cli_process.Non_zero { exit_code = 5; stderr = "bad" }) in
