...
let with_fake_sol_home f =
  ...
  Fun.protect
    ...
    (fun () ->
      ...
      write_file (Filename.concat root "cli/platform/infra/base/alloy/logs.alloy.tftpl") fake_template;
...
let test_alloy_render_expands_taxonomy_loop () =
  with_fake_sol_home (fun sol_home ->
    let river = Sol_cli_dev_observability.render_alloy_config ~sol_home
