...
let test_scaffolded_workspace_has_a_real_deploy_target () =
  ...
  | Error e ->
      Alcotest.fail
        ("load_for_target failed: " ^ Sol_cli_config.error_to_string e)
  | Ok cfg -> (
...
let test_dockerfile_paths_are_workspace_relative () =
  ...
  assert_contains "Dockerfile" content
    "COPY --from=build \
     /workspace/_build/default/app/payments/charge_svc/bin/main.exe";
  assert_contains "Dockerfile" content
...
let test_readme_migrate_hint_substituted () =
  ...
  check_bool "README has no template placeholder" false
...
let test_scaffold_compiles () =
  ...
  check_bool "freshly scaffolded workspace builds with `dune build`" true
    (rc = 0)
...
let test_workspace_generated_json_decoders_are_result_based () =
  ...
  assert_contains "event" event
    "let* amount_cents = required_int fields \"amount_cents\"";
...
let test_parse_domain_name_rejects_malformed_names () =
  List.iter
    (fun arg ->
      ...
      | Ok (domain, name) ->
          Alcotest.failf "expected %S to be rejected, got (%S, %S)" arg domain
            name
      | Error msg ->
  ...
  Fun.protect
    ...
    (fun () ->
      ...
      let result =
        ...
        (match saved with
        | None ->
            Unix.putenv "SOL_HOME" "" (* can't unset, but empty won't match *)
...
let test_ancestor_walk_finds_bundle_root () =
  ...
  Fun.protect
    ...
    (fun () ->
      ...
      let result =
        Sol_cli_cmd_new.find_ancestor Sol_cli_cmd_new.is_sol_home bin_dir
      in
...
let test_golden_new_svc_files () =
  ...
  check_generated_file "svc handler" "app/comms/notify_svc/lib/handler.ml"
    Sol_cli_scaffold_templates.svc_handler_ml;
  check_generated_file "svc lib dune" "app/comms/notify_svc/lib/dune"
    (Sol_cli_scaffold.subst v Sol_cli_scaffold_templates.svc_lib_dune);
  check_generated_file "svc bin main" "app/comms/notify_svc/bin/main.ml"
    (Sol_cli_scaffold.subst v Sol_cli_scaffold_templates.svc_bin_ml);
  check_generated_file "svc bin dune" "app/comms/notify_svc/bin/dune"
    (Sol_cli_scaffold.subst v Sol_cli_scaffold_templates.svc_bin_dune);
  check_generated_file "svc sol.toml" "app/comms/notify_svc/sol.toml"
    Sol_cli_scaffold_templates.tpl_sol_toml;
  check_generated_file "svc Dockerfile" "app/comms/notify_svc/Dockerfile"
...
let test_golden_new_worker_files () =
  ...
  check_generated_file "worker lib dune" "app/comms/notify_worker/lib/dune"
    (Sol_cli_scaffold.subst v Sol_cli_scaffold_templates.worker_lib_dune);
  check_generated_file "worker bin main" "app/comms/notify_worker/bin/main.ml"
    (Sol_cli_scaffold.subst v Sol_cli_scaffold_templates.worker_bin_ml);
  check_generated_file "worker bin dune" "app/comms/notify_worker/bin/dune"
    (Sol_cli_scaffold.subst v Sol_cli_scaffold_templates.worker_bin_dune);
  check_generated_file "worker sol.toml" "app/comms/notify_worker/sol.toml"
    Sol_cli_scaffold_templates.tpl_sol_toml;
  check_generated_file "worker Dockerfile" "app/comms/notify_worker/Dockerfile"
    (Sol_cli_scaffold.subst v Sol_cli_scaffold_templates.tpl_dockerfile)
...
let test_golden_new_fn_files () =
  ...
  check_generated_file "fn lib" "app/comms/notify_fn/lib/notify_fn.ml"
    (Sol_cli_scaffold.subst v Sol_cli_scaffold_templates.fn_lib_ml);
  check_generated_file "fn lib dune" "app/comms/notify_fn/lib/dune"
    (Sol_cli_scaffold.subst v Sol_cli_scaffold_templates.fn_lib_dune);
  check_generated_file "fn bin main" "app/comms/notify_fn/bin/main.ml"
    (Sol_cli_scaffold.subst v Sol_cli_scaffold_templates.fn_bin_ml);
  check_generated_file "fn bin dune" "app/comms/notify_fn/bin/dune"
    (Sol_cli_scaffold.subst v Sol_cli_scaffold_templates.fn_bin_dune);
  check_generated_file "fn sol.toml" "app/comms/notify_fn/sol.toml"
    Sol_cli_scaffold_templates.tpl_fn_sol_toml;
  check_generated_file "fn Dockerfile" "app/comms/notify_fn/Dockerfile"
    (Sol_cli_scaffold.subst v Sol_cli_scaffold_templates.tpl_dockerfile)
...
let () =
  Alcotest.run "scaffold"
    [
      ( "ci_workflow",
        [
          Alcotest.test_case "sol-ci.yml created" `Quick
            test_ci_workflow_created;
          Alcotest.test_case "deploy.yml still created" `Quick
            test_deploy_workflow_created;
          Alcotest.test_case "deploy.yml passes target" `Quick
            test_deploy_workflow_passes_target;
          Alcotest.test_case "contains sol deploy" `Quick
            test_ci_contains_sol_deploy;
          Alcotest.test_case "deploy steps pass target" `Quick
            test_ci_deploy_steps_pass_target;
          Alcotest.test_case "--emit-plan-to present" `Quick
            test_ci_contains_emit_plan_to;
          Alcotest.test_case "--emit-to present" `Quick test_ci_contains_emit_to;
          Alcotest.test_case "dune build + runtest" `Quick
            test_ci_contains_dune_commands;
          Alcotest.test_case "no KUBECONFIG_B64 in workflow" `Quick
            test_ci_no_kubeconfig_in_build_job;
          Alcotest.test_case "registry secret placeholders" `Quick
            test_ci_registry_secrets;
          Alcotest.test_case "CI contract comment present" `Quick
            test_ci_contract_comment_present;
          Alcotest.test_case "deploy phase uses sol deploy" `Quick
            test_ci_contract_deploy_phase_uses_sol_deploy;
          Alcotest.test_case "no raw kubectl apply" `Quick
            test_ci_no_raw_kubectl_apply;
          Alcotest.test_case "build-images has TODO(sol-build)" `Quick
            test_ci_build_images_has_todo_sol_build;
        ] );
      ( "existing_files",
        [
          Alcotest.test_case "all prior files still present" `Quick
            test_existing_files_still_generated;
          Alcotest.test_case "has a real deploy target" `Quick
            test_scaffolded_workspace_has_a_real_deploy_target;
          Alcotest.test_case "dune-project generated" `Quick
            test_workspace_has_dune_project;
          Alcotest.test_case "Dockerfile paths relative" `Quick
            test_dockerfile_paths_are_workspace_relative;
          Alcotest.test_case "README hints substituted" `Quick
            test_readme_migrate_hint_substituted;
          Alcotest.test_case "Sol sources linked" `Quick test_sol_sources_linked;
          Alcotest.test_case "scaffold actually compiles" `Quick
            test_scaffold_compiles;
          Alcotest.test_case "bare fn library compiles" `Quick
            test_bare_fn_library_compiles;
          Alcotest.test_case "charge_svc publishes event" `Quick
            test_charge_svc_publishes_kafka_event;
          Alcotest.test_case "JSON decoders are result based" `Quick
            test_workspace_generated_json_decoders_are_result_based;
          Alcotest.test_case "startup helpers are flattened" `Quick
            test_workspace_startup_helpers_are_flattened;
        ] );
      ( "worker_ack",
        [
          Alcotest.test_case "generated worker has no ack param" `Quick
            test_worker_has_no_ack_param;
        ] );
      ( "domain_name_parser",
        [
          Alcotest.test_case "normalizes valid domain/name" `Quick
            test_parse_domain_name_normalizes_valid_name;
          Alcotest.test_case "rejects malformed names" `Quick
            test_parse_domain_name_rejects_malformed_names;
        ] );
      ( "bundle_resolution",
        [
          Alcotest.test_case "complete bundle layout resolves sol_home" `Quick
            test_bundle_layout_resolves_sol_home;
          Alcotest.test_case "incomplete bundle is rejected" `Quick
      ...
      ( "pending_migrations",
        [
          Alcotest.test_case "no db/migrations dir → 0" `Quick
            test_pending_migrations_no_dir;
          Alcotest.test_case "empty db/migrations dir → 0" `Quick
            test_pending_migrations_empty_dir;
          Alcotest.test_case "counts only .sql files" `Quick
            test_pending_migrations_counts_sql_files;
          Alcotest.test_case "scaffold workspace → 1 migration" `Quick
            test_pending_migrations_workspace_scaffold;
        ] );
      ( "golden",
        [
          Alcotest.test_case "sol-ci.yml" `Quick test_golden_ci_workflow;
          Alcotest.test_case "charge_svc Dockerfile" `Quick
            test_golden_dockerfile;
          Alcotest.test_case "charge_svc bin/main.ml" `Quick
            test_golden_svc_bin_ml;
          Alcotest.test_case "notify_worker bin/main.ml" `Quick
            test_golden_worker_bin_ml;
          Alcotest.test_case "test/dune" `Quick test_golden_test_dune;
          Alcotest.test_case "new svc files" `Quick test_golden_new_svc_files;
          Alcotest.test_case "new worker files" `Quick
            test_golden_new_worker_files;
          Alcotest.test_case "new fn files" `Quick test_golden_new_fn_files;
        ] );
      ( "mkdir_p",
        [
          Alcotest.test_case "creates nested directories" `Quick
            test_mkdir_p_creates_nested_dirs;
          Alcotest.test_case "tolerates an existing directory" `Quick
