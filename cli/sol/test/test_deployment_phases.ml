...
let test_state_dry_run_is_noop () =
  Sol_cli_deployment_state.record_outcome "myapp" Sol_cli_deployment_state.Dry_run
;;
...
let test_gitops_shares_plan_type () =
  with_temp_dir (fun dir ->
    ...
    let gitops_results =
      List.map
        (Sol_cli_executor.gitops ~workspace:plan.Sol_cli_deployment_plan.workspace ~dir)
        plan.Sol_cli_deployment_plan.services
    in
    let direct_results =
      List.map
  ...
  Alcotest.run
    ...
    ; ( "plan_construction"
      , [ Alcotest.test_case "local mode fields" `Quick test_plan_local_mode_fields
        ...
        ; Alcotest.test_case
            "consumer groups from workers"
            `Quick
            test_plan_consumer_groups_derived_from_workers
        ; Alcotest.test_case
            "Svc yields no consumer group"
            `Quick
    ...
    ; ( "render_artifacts"
      , [ Alcotest.test_case
        ...
        ; Alcotest.test_case
            "fn: CronJob, no Deployment"
            `Quick
            test_render_fn_produces_cronjob
        ; Alcotest.test_case
            "namespace yaml non-empty"
            `Quick
    ...
    ; ( "state_update"
      , [ Alcotest.test_case
        ...
        ; Alcotest.test_case
            "removed consumer groups"
            `Quick
            test_state_removed_consumer_groups
        ; Alcotest.test_case
            "stable plan: no removals"
            `Quick
            test_state_no_removal_when_stable
        ] )
    ; ( "path_consistency"
      , [ Alcotest.test_case
            "local and direct share plan type"
            `Quick
        ...
        ; Alcotest.test_case
            "all paths start from same workspace"
            `Quick
            test_all_paths_start_from_same_plan_workspace
        ; Alcotest.test_case
            "up execution descriptor uses host push image"
            `Quick
