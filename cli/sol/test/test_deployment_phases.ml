...
let test_up_request_uses_explicit_tag () =
  let r =
    Sol_cli_command_request.make_up_request ~filter_path:None ~dry_run:false
      ~tag:(Some "v1.2.3") ~confirm_group_change:false ~git_sha:(fun () ->
        Alcotest.fail "git_sha should not be called when tag is explicit")
  in
  match r with
  | Ok req ->
      Alcotest.(check string)
        "explicit tag" "v1.2.3" req.Sol_cli_command_request.image_tag
  | Error msg -> Alcotest.fail msg

let test_up_request_falls_back_to_git_sha () =
  let r =
    Sol_cli_command_request.make_up_request ~filter_path:None ~dry_run:false
      ~tag:None ~confirm_group_change:false ~git_sha:(fun () -> "sha-deadbeef")
  in
  match r with
  | Ok req ->
      Alcotest.(check string)
        "git sha fallback" "sha-deadbeef" req.Sol_cli_command_request.image_tag
  | Error msg -> Alcotest.fail msg

let test_up_request_preserves_mode () =
  let r =
    Sol_cli_command_request.make_up_request ~filter_path:None ~dry_run:true
      ~tag:(Some "t") ~confirm_group_change:false ~git_sha:(fun () -> "")
  in
  match r with
  | Ok req ->
      Alcotest.(check bool)
...
let test_deploy_request_uses_explicit_tag () =
  let r =
    Sol_cli_command_request.make_deploy_request ~target:"dev/aws/us-east-1"
      ...
      ~confirm_group_change:false ~loki_push_url:None ~git_sha:(fun () ->
        Alcotest.fail "git_sha should not be called")
  in
  match r with
  | Ok req ->
      Alcotest.(check string)
        "explicit tag" "sha-abc" req.Sol_cli_command_request.image_tag
  | Error msg -> Alcotest.fail msg
...
let test_deploy_request_gitops_action () =
  let r =
    Sol_cli_command_request.make_deploy_request ~target:"dev/aws/us-east-1"
      ...
      ~emit_plan_to:None ~image_tag:(Some "tag") ~registry:(Some "reg")
      ~secret_backend:Sol_cli_manifest.Kubernetes_placeholder
      ~confirm_group_change:false ~loki_push_url:None ~git_sha:(fun () -> "")
  in
  match r with
...
let test_deploy_request_dry_run_action_preserves_emit_to () =
  ...
  | Ok req ->
      Alcotest.(check bool)
        "dry-run action preserves emit_to" true
        (match req.Sol_cli_command_request.action with
        | Sol_cli_command_request.Deploy_dry_run
...
let test_deploy_request_rejects_empty_target () =
  let r =
    Sol_cli_command_request.make_deploy_request ~target:"" ~filter_path:None
      ~dry_run:false ~emit_to:None ~emit_plan_to:None ~image_tag:(Some "tag")
      ~registry:(Some "reg")
      ~secret_backend:Sol_cli_manifest.Kubernetes_placeholder
      ~confirm_group_change:false ~loki_push_url:None ~git_sha:(fun () -> "")
  in
...
let test_deploy_request_registry_omitted_stays_none () =
  let r =
    Sol_cli_command_request.make_deploy_request ~target:"dev/aws/us-east-1"
      ~filter_path:None ~dry_run:false ~emit_to:None ~emit_plan_to:None
      ~image_tag:(Some "tag") ~registry:None
      ~secret_backend:Sol_cli_manifest.Kubernetes_placeholder
      ~confirm_group_change:false ~loki_push_url:None ~git_sha:(fun () -> "")
  in
  match r with
  | Ok req ->
      Alcotest.(check (option string))
        "registry stays None" None req.Sol_cli_command_request.registry
  | Error msg -> Alcotest.fail msg
...
let test_plan_local_mode_fields () =
  ...
  Alcotest.(check string)
    "workspace" "myapp" plan.Sol_cli_deployment_plan.workspace;
  Alcotest.(check bool)
    "mode Local" true
...
let render_ok spec =
  match
    Sol_cli_deployment_render.render_spec ~workspace:"myapp"
      ~secret_backend:Sol_cli_manifest.Kubernetes_placeholder spec
  with
...
let run_plan_ok ~mode ?secret_backend plan =
  match
    Sol_cli_executor.run_plan ~workspace:plan.Sol_cli_deployment_plan.workspace
      ~mode ?secret_backend plan.Sol_cli_deployment_plan.services
  with
  | Ok rs -> rs
...
let test_render_artifact_image_matches_spec () =
  ...
  Alcotest.(check string)
    "artifact image" "registry.example.com/myapp/charge-svc:abc123"
...
let test_render_no_docker_or_k8s_calls () =
  ...
  let results = run_plan_ok ~mode:Sol_cli_executor.Dry_run plan in
  Alcotest.(check bool)
    "renders without side effects" true
...
let test_gitops_emit_uses_placeholder_backend () =
  with_temp_dir (fun dir ->
      ...
      let content =
        ...
        s
      in
      assert_contains "placeholder comment present" content
...
let test_local_executor_result_fields () =
  ...
  Alcotest.(check string)
    "local namespace" "myapp-payments" r.Sol_cli_executor.namespace;
  Alcotest.(check string) "local name" "charge-svc" r.Sol_cli_executor.name;
  Alcotest.(check string)
    "local image" "registry.example.com/myapp/charge-svc:abc123"
    r.Sol_cli_executor.image

let test_direct_executor_result_fields () =
  let r = Sol_cli_executor.local ~workspace:"myapp" ~dry_run:true svc_spec in
  Alcotest.(check string)
    "direct namespace" "myapp-payments" r.Sol_cli_executor.namespace;
  Alcotest.(check string) "direct name" "charge-svc" r.Sol_cli_executor.name;
  Alcotest.(check string)
    "direct image" "registry.example.com/myapp/charge-svc:abc123"
    r.Sol_cli_executor.image
...
let test_gitops_executor_result_fields () =
  with_temp_dir (fun dir ->
      ...
      Alcotest.(check string)
        "gitops namespace" "myapp-payments" r.Sol_cli_executor.namespace;
      Alcotest.(check string) "gitops name" "charge-svc" r.Sol_cli_executor.name;
      Alcotest.(check string)
        "gitops image" "registry.example.com/myapp/charge-svc:abc123"
        r.Sol_cli_executor.image)

let test_local_worker_executor_result_fields () =
  let r = Sol_cli_executor.local ~workspace:"myapp" ~dry_run:true worker_spec in
  Alcotest.(check string)
    "local worker namespace" "myapp-comms" r.Sol_cli_executor.namespace;
  Alcotest.(check string)
    "local worker name" "notify-worker" r.Sol_cli_executor.name

let test_direct_fn_executor_result_fields () =
  let r = Sol_cli_executor.local ~workspace:"myapp" ~dry_run:true fn_spec in
  Alcotest.(check string)
    "direct fn namespace" "myapp-billing" r.Sol_cli_executor.namespace;
  Alcotest.(check string) "direct fn name" "invoice-fn" r.Sol_cli_executor.name

(* ── Phase 6: state update ──────────────────────────────────────────────── *)

let test_state_applied_does_not_raise () =
  Sol_cli_deployment_state.record_outcome "myapp"
...
let test_state_failed_is_noop () =
  Sol_cli_deployment_state.record_outcome "myapp"
    (Sol_cli_deployment_state.Failed
       { phase = "render"; message = "YAML error" })
...
let test_local_and_direct_share_plan_type () =
  ...
  let local_results =
    List.map
      (Sol_cli_executor.local ~workspace:plan.Sol_cli_deployment_plan.workspace
         ~dry_run:true)
      plan.Sol_cli_deployment_plan.services
  in
  let direct_results =
  ...
  Alcotest.(check string)
    "local namespace = direct namespace" lr.Sol_cli_executor.namespace
    dr.Sol_cli_executor.namespace;
  Alcotest.(check string)
    "local name = direct name" lr.Sol_cli_executor.name dr.Sol_cli_executor.name;
  Alcotest.(check string)
    "local image = direct image" lr.Sol_cli_executor.image
      ...
      let gitops_results =
        List.map
          (Sol_cli_executor.gitops
             ~workspace:plan.Sol_cli_deployment_plan.workspace ~dir)
          plan.Sol_cli_deployment_plan.services
      in
      let direct_results =
        List.map
          (Sol_cli_executor.local
             ~workspace:plan.Sol_cli_deployment_plan.workspace ~dry_run:true)
          plan.Sol_cli_deployment_plan.services
      in
      let gr = List.hd gitops_results and dr = List.hd direct_results in
      Alcotest.(check string)
        "gitops namespace = direct namespace" gr.Sol_cli_executor.namespace
        dr.Sol_cli_executor.namespace;
      Alcotest.(check string)
        "gitops name = direct name" gr.Sol_cli_executor.name
        dr.Sol_cli_executor.name;
      Alcotest.(check string)
        "gitops image = direct image" gr.Sol_cli_executor.image
        dr.Sol_cli_executor.image)
...
let test_change_set_build_is_path_agnostic () =
  with_temp_dir (fun dir ->
      ...
      in
      let id_dry =
        id (List.hd (run_plan_ok ~mode:Sol_cli_executor.Dry_run plan))
      in
      let id_emit =
        id (List.hd (run_plan_ok ~mode:(Sol_cli_executor.Emit_to dir) plan))
      in
      Alcotest.(check bool)
        "dry vs emit: identity identical" true (id_dry = id_emit))
...
let test_all_paths_start_from_same_plan_workspace () =
  ...
  List.iter
    (fun plan ->
      Alcotest.(check string)
        "workspace consistent" "myapp" plan.Sol_cli_deployment_plan.workspace;
      Alcotest.(check int)
        "service count consistent" 1
        (List.length plan.Sol_cli_deployment_plan.services))
    [ plan_local; plan_direct; plan_gitops; plan_hosted ]

let test_up_execution_descriptor_uses_host_push_image () =
  let exec =
    Sol_cli_up_execution.service_execution ~workspace:"myapp"
      ~ctx_dir:"/tmp/myapp.docker-ctx" ~sha:"abc123" svc_spec
  in
  Alcotest.(check string) "k8s name" "charge-svc" exec.k8s_name;
  Alcotest.(check string) "namespace" "myapp-payments" exec.namespace;
  Alcotest.(check string)
    "push image" "localhost:5000/myapp/charge-svc:abc123" exec.push_image;
  Alcotest.(check string)
    "dockerfile" "/tmp/myapp.docker-ctx/app/payments/charge_svc/Dockerfile"
    exec.dockerfile
...
let () =
  Alcotest.run "deployment_phases"
    [
      ( "request_validation",
        [
          Alcotest.test_case "up: explicit tag used" `Quick
            test_up_request_uses_explicit_tag;
          Alcotest.test_case "up: git sha fallback" `Quick
            test_up_request_falls_back_to_git_sha;
          Alcotest.test_case "up: mode preserved" `Quick
            test_up_request_preserves_mode;
          Alcotest.test_case "deploy: explicit tag used" `Quick
            test_deploy_request_uses_explicit_tag;
          Alcotest.test_case "deploy: local mode Ok" `Quick
            test_deploy_request_local_mode_builds_request;
          Alcotest.test_case "deploy: gitops action" `Quick
            test_deploy_request_gitops_action;
          Alcotest.test_case "deploy: dry-run action preserves emit_to" `Quick
            test_deploy_request_dry_run_action_preserves_emit_to;
          Alcotest.test_case "deploy: empty target rejected" `Quick
            test_deploy_request_rejects_empty_target;
          Alcotest.test_case "deploy: registry omitted stays None" `Quick
            test_deploy_request_registry_omitted_stays_none;
        ] );
      ( "plan_construction",
        [
          Alcotest.test_case "local mode fields" `Quick
            test_plan_local_mode_fields;
          Alcotest.test_case "customer_cloud mode fields" `Quick
            test_plan_customer_cloud_mode_fields;
          Alcotest.test_case "service count preserved" `Quick
            test_plan_service_count;
          Alcotest.test_case "all three primitives" `Quick
            test_plan_service_primitives;
          Alcotest.test_case "consumer groups from workers" `Quick
            test_plan_consumer_groups_derived_from_workers;
          Alcotest.test_case "Svc yields no consumer group" `Quick
            test_plan_svc_does_not_produce_consumer_group;
        ] );
      ( "render_artifacts",
        [
          Alcotest.test_case "svc: Deployment + Service" `Quick
            test_render_svc_produces_deployment_and_service;
          Alcotest.test_case "worker: Deployment, no Service" `Quick
            test_render_worker_has_deployment_no_service;
          Alcotest.test_case "fn: CronJob, no Deployment" `Quick
            test_render_fn_produces_cronjob;
          Alcotest.test_case "namespace yaml non-empty" `Quick
            test_render_namespace_yaml_is_non_empty;
          Alcotest.test_case "one artifact per service" `Quick
            test_render_artifact_count_matches_services;
          Alcotest.test_case "artifact image from spec" `Quick
            test_render_artifact_image_matches_spec;
          Alcotest.test_case "build is side-effect-free" `Quick
            test_render_no_docker_or_k8s_calls;
        ] );
      ( "gitops_emit",
        [
          Alcotest.test_case "file created" `Quick test_gitops_emit_creates_file;
          Alcotest.test_case "file contains YAML separator" `Quick
            test_gitops_emit_file_contains_yaml_separator;
          Alcotest.test_case "file has Namespace kind" `Quick
            test_gitops_emit_file_has_namespace_kind;
          Alcotest.test_case "uses placeholder backend" `Quick
            test_gitops_emit_uses_placeholder_backend;
          Alcotest.test_case "one file per service" `Quick
            test_gitops_emit_one_file_per_service;
        ] );
      ( "executor_commands",
        [
          Alcotest.test_case "local: svc result fields" `Quick
            test_local_executor_result_fields;
          Alcotest.test_case "direct: svc result fields" `Quick
            test_direct_executor_result_fields;
          Alcotest.test_case "gitops: svc result fields" `Quick
            test_gitops_executor_result_fields;
          Alcotest.test_case "local: worker result fields" `Quick
            test_local_worker_executor_result_fields;
          Alcotest.test_case "direct: fn result fields" `Quick
            test_direct_fn_executor_result_fields;
        ] );
      ( "state_update",
        [
          Alcotest.test_case "Applied does not raise" `Quick
            test_state_applied_does_not_raise;
          Alcotest.test_case "Dry_run is no-op" `Quick
            test_state_dry_run_is_noop;
          Alcotest.test_case "Failed is no-op" `Quick test_state_failed_is_noop;
          Alcotest.test_case "Emitted is no-op" `Quick
            test_state_emitted_is_noop;
          Alcotest.test_case "removed consumer groups" `Quick
            test_state_removed_consumer_groups;
          Alcotest.test_case "stable plan: no removals" `Quick
            test_state_no_removal_when_stable;
        ] );
      ( "path_consistency",
        [
          Alcotest.test_case "local and direct share plan type" `Quick
            test_local_and_direct_share_plan_type;
          Alcotest.test_case "gitops shares plan type" `Quick
            test_gitops_shares_plan_type;
          Alcotest.test_case "build is mode-agnostic" `Quick
            test_change_set_build_is_path_agnostic;
          Alcotest.test_case "all paths start from same workspace" `Quick
            test_all_paths_start_from_same_plan_workspace;
          Alcotest.test_case "up execution descriptor uses host push image"
            `Quick test_up_execution_descriptor_uses_host_push_image;
        ] );
    ]
