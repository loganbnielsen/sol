...
let test_mode_of_env_accepts_customer_cloud_aliases () =
  List.iter
    (fun env -> check_mode env Sol_cli_secret.Customer_cloud (Sol_cli_secret.mode_of_env env))
...
let test_mode_of_env_rejects_unknown () =
  ...
  | Error msg ->
    check_string "error"
      "unknown secret environment \"staging\"; expected one of: hosted, sol_hosted, sol-hosted, local, dev, cloud, customer_cloud, customer-cloud"
      msg
...
let test_redacted_result_hides_value () =
  let out = Sol_cli_secret.redacted_result (Sol_cli_secret.Applied [ "myapp-payments" ]) in
...
let test_hosted_stub_boundary () =
  ...
  | Error msg ->
    check_string "hosted boundary"
      "hosted secret management will use the Sol control-plane API; no hosted endpoint is configured yet"
      msg

let test_list_rejects_unknown_env () =
  match Sol_cli_secret.list
          ~env:"staging"
          ~workspace:"myapp"
