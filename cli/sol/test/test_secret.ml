...
let test_key_validation_accepts_env_style_key () =
  check_bool "valid key" true
    (Sol_cli_secret.validate_key "DATABASE_URL" = Ok ())
...
let test_key_validation_rejects_lowercase () =
  ...
  | Error msg ->
      check_string "error" "secret key must start with an uppercase letter" msg
...
let test_mode_of_env_accepts_hosted_aliases () =
  List.iter
    (fun env ->
      check_mode env Sol_cli_secret.Sol_hosted (Sol_cli_secret.mode_of_env env))
    [ "hosted"; "sol_hosted"; "sol-hosted" ]

let test_mode_of_env_accepts_local_aliases () =
  List.iter
    (fun env ->
      check_mode env Sol_cli_secret.Local (Sol_cli_secret.mode_of_env env))
    [ "local"; "dev" ]

let test_mode_of_env_accepts_customer_cloud_aliases () =
  List.iter
    (fun env ->
      check_mode env Sol_cli_secret.Customer_cloud
        (Sol_cli_secret.mode_of_env env))
...
let test_mode_of_env_rejects_unknown () =
  ...
  | Error msg ->
      check_string "error"
        "unknown secret environment \"staging\"; expected one of: hosted, \
         sol_hosted, sol-hosted, local, dev, cloud, customer_cloud, \
         customer-cloud"
        msg
...
let test_secret_manifest_contains_value_boundary () =
  let yaml =
    Sol_cli_secret.secret_manifest
      ...
      ~namespace:"myapp-payments" ~key:"DATABASE_URL" ~value:"postgres://secret"
  in
  check_bool "manifest names Secret" true (contains yaml "kind: Secret");
  check_bool "manifest has runtime secret name" true
    (contains yaml "name: sol-secrets");
...
let test_secret_manifest_yaml_escapes_special_values () =
  let yaml =
    Sol_cli_secret.secret_manifest
      ...
      ~namespace:"myapp-payments" ~key:"SPECIAL_VALUE"
...
let test_hosted_stub_boundary () =
  match
    Sol_cli_secret.set ~env:"hosted" ~workspace:"myapp"
      ~namespaces:[ "myapp-payments" ] ~key:"DATABASE_URL"
      ~value:"postgres://secret"
  with
  | Ok _ -> Alcotest.fail "hosted set unexpectedly succeeded"
  | Error msg ->
      check_string "hosted boundary"
        "hosted secret management will use the Sol control-plane API; no \
         hosted endpoint is configured yet"
        msg

let test_list_rejects_unknown_env () =
  match
    Sol_cli_secret.list ~env:"staging" ~workspace:"myapp"
...
let () =
  Alcotest.run "secret"
    [
      ( "validation",
        [
          Alcotest.test_case "accepts env style key" `Quick
            test_key_validation_accepts_env_style_key;
          Alcotest.test_case "rejects lowercase" `Quick
            test_key_validation_rejects_lowercase;
          Alcotest.test_case "rejects hyphen" `Quick
            test_key_validation_rejects_hyphen;
        ] );
      ( "env",
        [
          Alcotest.test_case "accepts hosted aliases" `Quick
            test_mode_of_env_accepts_hosted_aliases;
          Alcotest.test_case "accepts local aliases" `Quick
            test_mode_of_env_accepts_local_aliases;
          Alcotest.test_case "accepts customer cloud aliases" `Quick
            test_mode_of_env_accepts_customer_cloud_aliases;
          Alcotest.test_case "rejects unknown parser input" `Quick
            test_mode_of_env_rejects_unknown;
          Alcotest.test_case "rejects unknown operation env" `Quick
            test_list_rejects_unknown_env;
        ] );
      ( "rendering",
        [
          Alcotest.test_case "k8s materialization manifest" `Quick
            test_secret_manifest_contains_value_boundary;
          Alcotest.test_case "yaml escapes special values" `Quick
            test_secret_manifest_yaml_escapes_special_values;
          Alcotest.test_case "redacted result" `Quick
            test_redacted_result_hides_value;
        ] );
      ( "hosted",
