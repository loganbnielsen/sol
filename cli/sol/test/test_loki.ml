...
let test_classify_curl_timeout_exit_code () =
  match L.classify_process_error (Sol_cli_process.Non_zero { exit_code = 28; stderr = "" }) with
  | L.Timeout -> ()
  | _ -> Alcotest.fail "expected Timeout for curl exit 28"

let test_classify_connection_failed () =
  match L.classify_process_error (Sol_cli_process.Non_zero { exit_code = 7; stderr = "" }) with
  | L.Connection_failed -> ()
  | _ -> Alcotest.fail "expected Connection_failed for curl exit 7"

let test_classify_other () =
  match L.classify_process_error (Sol_cli_process.Non_zero { exit_code = 22; stderr = "boom" }) with
...
let test_query_range_argv_config_adds_config_path_not_secret () =
  let argv = L.query_range_argv ~base_url:"http://localhost:3100" ~ns:"acme-payments"
      ...
      ~curl_config:"/tmp/sol-loki-curl.conf" () in
