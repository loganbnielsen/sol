...
let error_body =
  {|{"status": "error", "error": "parse error", "errorType": "bad_data"}|}
...
let test_classify_curl_timeout_exit_code () =
  match
    L.classify_process_error
      (Sol_cli_process.Non_zero { exit_code = 28; stderr = "" })
  with
  | L.Timeout -> ()
...
let test_classify_connection_failed () =
  match
    L.classify_process_error
      (Sol_cli_process.Non_zero { exit_code = 7; stderr = "" })
  with
  | L.Connection_failed -> ()
...
let test_classify_other () =
  ...
  with
  | L.Other msg ->
      check_int "message mentions exit code" 1
...
let test_query_range_argv_contains_logql_labels () =
  let argv =
    L.query_range_argv ~base_url:"http://localhost:3100" ~ns:"acme-payments"
  ...
  check_int "mentions namespace label" 1
    (if
       try
         ignore
           (Str.search_forward (Str.regexp_string "acme-payments") joined 0);
         true
...
let test_resolve_credentials_neither_set_is_ok_none () =
  match
    L.resolve_credentials ~flag_username:None ~flag_password:None
      ~env_username:None ~env_password:None
  with
  | Ok None -> ()
...
let test_resolve_credentials_empty_values_are_unset () =
  match
    L.resolve_credentials ~flag_username:(Some " ") ~flag_password:(Some "")
      ~env_username:None ~env_password:None
  with
  | Ok None -> ()
...
let test_resolve_credentials_flags_only () =
  match
    L.resolve_credentials ~flag_username:(Some "flag-user")
      ~flag_password:(Some "flag-pass") ~env_username:None ~env_password:None
  with
...
let test_resolve_credentials_env_only () =
  match
    L.resolve_credentials ~flag_username:None ~flag_password:None
      ~env_username:(Some "env-user") ~env_password:(Some "env-pass")
  with
...
let test_resolve_credentials_flag_wins_over_env () =
  match
    L.resolve_credentials ~flag_username:(Some "flag-user")
      ~flag_password:(Some "flag-pass") ~env_username:(Some "env-user")
...
let test_resolve_credentials_flag_username_wins_env_password_fills_in () =
  (* Fields are resolved independently: a flag username paired with only an
     ...
     all-or-nothing pair. *)
  match
    L.resolve_credentials ~flag_username:(Some "flag-user") ~flag_password:None
      ~env_username:(Some "env-user") ~env_password:(Some "env-pass")
  with
...
let test_resolve_credentials_password_without_username_is_error () =
  match
    L.resolve_credentials ~flag_username:None ~flag_password:None
      ~env_username:None ~env_password:(Some "env-pass")
  with
  | Error _ -> ()
    ...
    [
      ( "split_body_and_status",
        [
          ...
          Alcotest.test_case "no newline" `Quick
            test_split_body_and_status_no_newline;
        ] );
      ( "parse_query_range_body",
        [
          Alcotest.test_case "success orders oldest first" `Quick
            test_parse_success_orders_oldest_first;
          Alcotest.test_case "empty result is Ok []" `Quick
            test_parse_empty_result_is_ok_empty;
          Alcotest.test_case "error status is Error" `Quick
            test_parse_error_status_is_error;
          Alcotest.test_case "malformed JSON is Error" `Quick
            test_parse_malformed_json_is_error;
        ] );
      ( "classify_process_error",
        [
          Alcotest.test_case "Timeout" `Quick test_classify_timeout;
          Alcotest.test_case "curl exit 28 -> Timeout" `Quick
            test_classify_curl_timeout_exit_code;
          Alcotest.test_case "curl exit 7 -> Connection_failed" `Quick
            test_classify_connection_failed;
          Alcotest.test_case "other exit code -> Other" `Quick
            test_classify_other;
        ] );
      ( "query_range_argv",
        [
          Alcotest.test_case "contains logql labels" `Quick
            test_query_range_argv_contains_logql_labels;
          Alcotest.test_case "no config -> no --config flag" `Quick
            test_query_range_argv_no_config_omits_config_flag;
          Alcotest.test_case "config -> --config path" `Quick
            test_query_range_argv_config_adds_config_path_not_secret;
        ] );
      ( "resolve_credentials",
        [
          Alcotest.test_case "neither set -> Ok None" `Quick
            test_resolve_credentials_neither_set_is_ok_none;
          Alcotest.test_case "empty values are unset" `Quick
            test_resolve_credentials_empty_values_are_unset;
          Alcotest.test_case "flags only" `Quick
            test_resolve_credentials_flags_only;
          Alcotest.test_case "env only" `Quick test_resolve_credentials_env_only;
          Alcotest.test_case "flag wins over env" `Quick
            test_resolve_credentials_flag_wins_over_env;
          Alcotest.test_case "flag/env resolved independently per field" `Quick
            test_resolve_credentials_flag_username_wins_env_password_fills_in;
          Alcotest.test_case "username without password -> Error" `Quick
            test_resolve_credentials_username_without_password_is_error;
          Alcotest.test_case "password without username -> Error" `Quick
