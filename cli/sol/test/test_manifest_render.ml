...
let render_spec_ok ?(workspace = "myapp") ?env ?image ?secret_backend spec =
  match
    Sol_cli_deployment_render.render_spec ~workspace ?env ?image ?secret_backend
      spec
  with
  | Ok v -> v
...
let test_svc_image () =
  let _ns, workload = render_spec_ok svc_spec in
  assert_contains "svc image" workload
  ...
  let spec =
    {
      svc_spec with
      secrets =
        [ ("DATABASE_URL", "postgres://secret"); ("API_TOKEN", "token-value") ];
    }
  in
  let _ns, workload =
...
let test_user_secret_key_in_secret_resource () =
  ...
  in
  let secret_block = extract_kind_block workload "kind: Secret" in
  assert_contains "STRIPE_KEY present in Secret resource" secret_block
    "STRIPE_KEY:"
...
let test_multiple_user_secret_keys_in_secret_resource () =
  let spec =
    { svc_spec with secrets = [ ("STRIPE_KEY", ""); ("SENDGRID_API_KEY", "") ] }
  in
  let _ns, workload =
    render_spec_ok ~secret_backend:Sol_cli_manifest.Kubernetes_placeholder spec
  in
...
let test_gitops_redacts_all_secret_values () =
  let spec =
    { svc_spec with secrets = [ ("STRIPE_KEY", "sk_live_should_not_render") ] }
  in
  let _ns, workload =
    render_spec_ok ~secret_backend:Sol_cli_manifest.Kubernetes_placeholder spec
  in
  let secret_block = extract_kind_block workload "kind: Secret" in
  assert_contains "redaction comment" secret_block
    "Populate these values before applying";
  assert_contains "default POSTGRES_URL key retained" secret_block
    {|POSTGRES_URL: ""|};
  ...
  assert_absent "user secret value redacted" secret_block
...
let test_svc_image_override () =
  ...
  assert_contains "svc push image in dry-run" workload
    "localhost:5000/myapp/charge-svc:abc123";
  assert_absent "svc cluster image absent" workload
    "sol-registry:5000/myapp/charge-svc:abc123"
...
let test_worker_image () =
  ...
  assert_contains "worker image" workload
    "sol-registry:5000/myapp/notify-worker:abc123"
...
let test_worker_metrics_port () =
  ...
  assert_contains "worker prometheus scrape" workload
    "prometheus.io/scrape: \"true\"";
  assert_contains "worker prometheus port" workload
...
let test_fn_image () =
  ...
  assert_contains "fn image" workload
    "sol-registry:5000/myapp/invoice-fn:abc123"
...
let test_fn_cronjob_pod_template_has_app_label () =
  ...
  assert_contains "fn cronjob pod template app label" cronjob_block
    "app: invoice-fn"
...
let test_rollout_rolling_update () =
  ...
  let spec =
    { svc_spec with rollout_strategy = Some Sol_cli_toml.RollingUpdate }
  in
  let _ns, workload = render_spec_ok spec in
...
let test_progressive_blue_green_rollout () =
  let spec =
    { svc_spec with progressive_delivery = Some Sol_cli_toml.Blue_green }
  in
  let _ns, workload = render_spec_ok spec in
  assert_contains "rollout kind" workload "kind: Rollout";
  assert_contains "blueGreen block" workload "blueGreen:";
  assert_contains "active service strategy" workload
    "activeService: charge-svc-active";
  assert_contains "preview service strategy" workload
    "previewService: charge-svc-preview";
...
let test_rollout_canary_secrets_use_sol_secrets () =
  ...
  assert_contains "rollout uses charge-svc-secrets ref" rollout_block
    "name: charge-svc-secrets";
  assert_contains "rollout has STRIPE_KEY ref" rollout_block "key: STRIPE_KEY";
  assert_contains "rollout has DATABASE_URL ref" rollout_block
    "key: DATABASE_URL";
...
let test_rollout_blue_green_secrets_use_sol_secrets () =
  ...
  in
  let rollout_block = extract_kind_block workload "kind: Rollout" in
  assert_contains "blue-green rollout uses charge-svc-secrets" rollout_block
    "name: charge-svc-secrets";
  assert_contains "blue-green rollout has API_TOKEN ref" rollout_block
    "key: API_TOKEN";
  assert_absent "no global sol-secrets name" rollout_block "name: sol-secrets"
...
let test_ingress_host_override () =
  ...
  let spec =
    { svc_spec with ingress_host = Some (hostname "payments.example.com") }
  in
  let _ns, workload = render_spec_ok spec in
...
let test_extra_labels_appear_in_pod_template () =
  ...
  let spec =
    {
      svc_spec with
      extra_labels = [ ("team", "platform"); ("tier", "backend") ];
    }
  in
  let _ns, workload = render_spec_ok spec in
...
let test_toml_valid_rollout_recreate () =
  ...
  Sys.remove path;
  check_bool "rollout_strategy is Recreate" true
...
|};
  ...
  let toml = Sol_cli_toml.load path in
  Sys.remove path;
  check_bool "ingress_host parsed" true
    (Option.map Sol_cli_toml.hostname_to_string toml.Sol_cli_toml.ingress_host
    = Some "api.example.com");
  check_bool "ingress_path parsed" true
    (Option.map Sol_cli_toml.ingress_path_to_string
       toml.Sol_cli_toml.ingress_path
    = Some "/v1")
...
|};
  ...
  let toml = Sol_cli_toml.load path in
  Sys.remove path;
  check_bool "secret keys parsed" true
...
|};
  ...
  | Some
      ...
      ()
  | _ ->
      Alcotest.fail "expected canary progressive_delivery from [infra.rollout]"
...
|};
  ...
  let toml = Sol_cli_toml.load path in
  Sys.remove path;
  check_bool "blue-green parsed" true
...
let test_toml_load_result_validation_error () =
  ...
  | Error (Sol_cli_toml.Validation { path = _; message }) ->
      assert_contains "typed validation error" message
...
|};
  ...
  let toml = Sol_cli_toml.load path in
  Sys.remove path;
  check_bool "multi-line secrets array parsed" true
    (toml.Sol_cli_toml.secret_keys = [ "DATABASE_URL"; "API_TOKEN" ])
...
|};
  ...
  let toml = Sol_cli_toml.load path in
  Sys.remove path;
  check_bool "replicas from dotted header" true
    (toml.Sol_cli_toml.replicas = Some 3);
  check_bool "cpu from dotted header" true
    (Option.map Sol_cli_toml.cpu_quantity_to_string toml.Sol_cli_toml.cpu
    = Some "250m");
  check_bool "memory from dotted header" true
...
let test_external_secret_doc_no_stringdata () =
  let doc =
    Sol_cli_manifest.external_secret_doc ~store_ref:"aws-secrets-manager"
      ~store_kind:"ClusterSecretStore" ~key_prefix:"myapp/"
      ~refresh_interval:"1h"
...
let test_external_secret_doc_keys_present () =
  let doc =
    Sol_cli_manifest.external_secret_doc ~store_ref:"aws-secrets-manager"
      ~store_kind:"ClusterSecretStore" ~key_prefix:"" ~refresh_interval:"1h"
...
let test_external_secret_doc_target_name () =
  let doc =
    Sol_cli_manifest.external_secret_doc ~store_ref:"my-store"
      ~store_kind:"ClusterSecretStore" ~key_prefix:"" ~refresh_interval:"1h"
      ~secret_keys:[ "POSTGRES_URL" ] ~ns:"myapp-payments" ~name:"charge-svc"
  in
  assert_contains "target name is charge-svc-secrets" doc
    "name: charge-svc-secrets"
...
let test_live_backend_missing_user_secret_returns_error () =
  ...
  in
  (match
     Sol_cli_deployment_render.render_spec ~workspace:"myapp"
       ~secret_backend:Sol_cli_manifest.Kubernetes_live spec_with_secret
   with
  | Ok _ -> ()
  ...
  match
    Sol_cli_deployment_render.render_spec ~workspace:"myapp"
      ~secret_backend:Sol_cli_manifest.Kubernetes_live spec_missing
  with
  | Error msg ->
      check_bool "error mentions the missing key" true (contains msg absent_key)
  | Ok _ ->
      Alcotest.fail
        "Expected Error when required secret env var is absent, got Ok"
...
let test_live_backend_multiple_missing_secrets_all_reported () =
  ...
  match
    Sol_cli_deployment_render.render_spec ~workspace:"myapp"
      ~secret_backend:Sol_cli_manifest.Kubernetes_live spec
  with
  | Error msg ->
  ...
  | Ok _ ->
      Alcotest.fail
        "Expected Error when required secret env vars are absent, got Ok"
...
let test_gitops_secret_redacted () =
  ...
  let spec =
    { svc_spec with secrets = [ ("SECRET_KEY", "real-value-must-not-appear") ] }
  in
  let _ns, workload =
    render_spec_ok ~secret_backend:Sol_cli_manifest.Kubernetes_placeholder spec
  in
  assert_absent "no real secret value in gitops output" workload
...
let test_shape_http_service_deployment_has_ports () =
  let doc =
    Sol_cli_manifest.deployment_doc ~shape:Sol_cli_manifest.Http_service
      ~replicas:1 ~cpu:"100m" ~memory:"128Mi" ~ns:"myapp-payments"
      ~name:"charge-svc" ~image:"sol-registry:5000/myapp/charge-svc:abc123"
      ~workspace:"myapp" ~domain:"payments" ~primitive:"svc" ()
  in
  ...
  assert_contains "Http_service has prometheus.io/scrape (OBS-011)" doc
    "prometheus.io/scrape: \"true\""

let test_shape_background_worker_deployment_has_metrics_port () =
  let doc =
    Sol_cli_manifest.deployment_doc ~shape:Sol_cli_manifest.Background_worker
      ~replicas:1 ~cpu:"100m" ~memory:"128Mi" ~ns:"myapp-comms"
      ~name:"notify-worker"
      ~image:"sol-registry:5000/myapp/notify-worker:abc123" ~workspace:"myapp"
      ~domain:"comms" ~primitive:"worker" ()
  in
  assert_contains "Background_worker metrics containerPort" doc
    "containerPort: 9090";
  assert_absent "Background_worker no readinessProbe" doc "readinessProbe:";
  assert_contains "Background_worker has prometheus.io/scrape" doc
    "prometheus.io/scrape: \"true\"";
  assert_contains "Background_worker scrape port" doc
    "prometheus.io/port: \"9090\""

let test_shape_rollout_http_service_has_ports () =
  let doc =
    Sol_cli_manifest.rollout_doc ~shape:Sol_cli_manifest.Http_service
      ~replicas:1 ~cpu:"100m" ~memory:"128Mi" ~ns:"myapp-payments"
      ~name:"charge-svc" ~image:"sol-registry:5000/myapp/charge-svc:abc123"
...
let test_shape_rollout_background_worker_metrics_port () =
  let doc =
    Sol_cli_manifest.rollout_doc ~shape:Sol_cli_manifest.Background_worker
      ~replicas:1 ~cpu:"100m" ~memory:"128Mi" ~ns:"myapp-comms"
      ~name:"notify-worker"
      ~image:"sol-registry:5000/myapp/notify-worker:abc123"
      ~pd:(Sol_cli_toml.Canary { steps = [ Sol_cli_toml.Weight 50 ] })
      ~workspace:"myapp" ~domain:"comms" ~primitive:"worker" ()
  in
  assert_contains "rollout Background_worker metrics containerPort" doc
    "containerPort: 9090";
  assert_contains "rollout Background_worker prometheus scrape" doc
    "prometheus.io/scrape: \"true\"";
  assert_contains "rollout Background_worker prometheus port" doc
    "prometheus.io/port: \"9090\"";
  assert_absent "rollout Background_worker no readinessProbe" doc
    "readinessProbe:"
...
let test_taxonomy_labels_match_dashboard_link_normalization () =
  let spec =
    {
      ...
      namespace =
        namespace ~workspace:"Sol_Obs_Review_App" ~domain:"Payments_Team";
    }
  in
  let _, workload = render_spec_ok ~workspace:"Sol_Obs_Review_App" spec in
  assert_contains "workspace label matches sol open's normalization" workload
    {|workspace: "sol-obs-review-app"|};
  assert_contains "domain label matches sol open's normalization" workload
    {|domain: "payments-team"|};
  let dashboard_url =
    match
      Sol_cli_open.url ~base_url:"http://localhost:3000"
        ~workspace:"Sol_Obs_Review_App" ~kind:Sol_cli_open.Dashboard
  ...
  assert_contains "dashboard link uses the identical normalized workspace"
    dashboard_url "var-workspace=sol-obs-review-app";
...
let test_release_of_image_trailing_non_alnum_after_truncation () =
  ...
  let spec =
    { svc_spec with image = "sol-registry:5000/myapp/charge-svc:" ^ tag }
  in
  let _, workload = render_spec_ok spec in
  assert_contains "trailing '.' replaced with a safe char" workload
    (Printf.sprintf {|release: "%s0"|} (String.make 62 'a'))

let test_release_of_image_numeric_tag_quoted () =
  let spec =
    { svc_spec with image = "sol-registry:5000/myapp/charge-svc:123" }
  in
  let _, workload = render_spec_ok spec in
  assert_contains "numeric-looking release stays a quoted string" workload
    {|release: "123"|}
...
let test_sanitize_label_value_lowercases_and_replaces_underscores () =
  check_string "mixed case + underscores normalized" "sol-obs-review-app"
...
let test_discover_services_hyphenated_filter_matches_underscored_dir () =
  ...
  let found =
    Sol_cli_manifest.discover_services
      ~filter_path:(Some "app/payments/charge-svc")
  in
  Alcotest.(check (list string))
    "hyphenated full path matches" [ "charge_svc" ] (names found)

let test_discover_services_hyphenated_basename_filter_matches () =
  with_charge_svc_workspace @@ fun () ->
  let found =
    Sol_cli_manifest.discover_services ~filter_path:(Some "charge-svc")
  in
  Alcotest.(check (list string))
    "hyphenated basename matches" [ "charge_svc" ] (names found)

let test_discover_services_underscored_filter_still_matches () =
  with_charge_svc_workspace @@ fun () ->
  let found =
    Sol_cli_manifest.discover_services ~filter_path:(Some "charge_svc")
  in
  Alcotest.(check (list string))
    "underscored basename still matches" [ "charge_svc" ] (names found)

let test_discover_services_non_matching_filter_excludes () =
  with_charge_svc_workspace @@ fun () ->
  let found =
    Sol_cli_manifest.discover_services ~filter_path:(Some "notify-worker")
  in
    ...
    [
      ( "svc",
        [
          ...
          Alcotest.test_case "has Service resource" `Quick
            test_svc_has_service_resource;
          Alcotest.test_case "has Ingress" `Quick test_svc_has_ingress;
          Alcotest.test_case "NetworkPolicy allows monitoring ingress" `Quick
          ...
          Alcotest.test_case "default resources (replicas/cpu/memory)" `Quick
            test_svc_default_resources;
          Alcotest.test_case "extra config in map" `Quick test_svc_extra_config;
          Alcotest.test_case "env label when resolved" `Quick
            test_svc_env_label_present_when_resolved;
          Alcotest.test_case "env label absent by default" `Quick
            test_svc_env_label_absent_by_default;
          Alcotest.test_case "default postgres url" `Quick
            test_svc_default_postgres_url;
          Alcotest.test_case "POSTGRES_URL not in ConfigMap" `Quick
            test_postgres_url_not_in_configmap;
          Alcotest.test_case "POSTGRES_URL in Secret" `Quick
            test_postgres_url_in_secret;
          Alcotest.test_case "POSTGRES_URL env in live Secret" `Quick
            test_live_secret_uses_postgres_url_env;
          Alcotest.test_case "default redpanda admin" `Quick
            test_svc_default_redpanda_admin_url;
          Alcotest.test_case "secret refs no values" `Quick
            test_svc_secret_refs_without_values;
          Alcotest.test_case "namespace in workload" `Quick
            test_svc_namespace_in_workload;
          Alcotest.test_case "image override (up dry-run)" `Quick
            test_svc_image_override;
          Alcotest.test_case "user secret key in Secret resource" `Quick
            test_user_secret_key_in_secret_resource;
          Alcotest.test_case "user secret key ref in Deployment" `Quick
            test_user_secret_key_ref_in_deployment;
          Alcotest.test_case "multiple user secret keys in Secret" `Quick
            test_multiple_user_secret_keys_in_secret_resource;
          Alcotest.test_case "default secrets preserved with user secrets"
            `Quick test_default_secrets_preserved_with_user_secrets;
          Alcotest.test_case "GitOps redacts secret values" `Quick
      ...
      ( "worker",
        [
          ...
          Alcotest.test_case "metrics containerPort" `Quick
            test_worker_metrics_port;
      ...
      ( "fn",
        [
          ...
          Alcotest.test_case "pod template has app label (AUDIT-040)" `Quick
            test_fn_cronjob_pod_template_has_app_label;
        ] );
      ( "escape_hatches",
        [
          Alcotest.test_case "rollout Recreate" `Quick test_rollout_recreate;
          Alcotest.test_case "rollout RollingUpdate" `Quick
            test_rollout_rolling_update;
          Alcotest.test_case "rollout default=RollingUpdate" `Quick
            test_rollout_default_is_rolling_update;
          Alcotest.test_case "progressive default Deployment" `Quick
            test_progressive_default_is_deployment;
          Alcotest.test_case "progressive canary Rollout" `Quick
            test_progressive_canary_rollout;
          Alcotest.test_case "progressive worker canary" `Quick
            test_progressive_canary_worker_no_service;
          Alcotest.test_case "progressive blue-green Rollout" `Quick
            test_progressive_blue_green_rollout;
          Alcotest.test_case "rollout canary secrets use name-secrets" `Quick
            test_rollout_canary_secrets_use_sol_secrets;
          Alcotest.test_case "rollout blue-green secrets use name-secrets"
            `Quick test_rollout_blue_green_secrets_use_sol_secrets;
          Alcotest.test_case "ingress host override" `Quick
            test_ingress_host_override;
          Alcotest.test_case "ingress path override" `Quick
            test_ingress_path_override;
          Alcotest.test_case "ingress default path" `Quick
            test_ingress_default_path;
          Alcotest.test_case "extra_labels in pod template" `Quick
            test_extra_labels_appear_in_pod_template;
          Alcotest.test_case "extra_labels empty default" `Quick
            test_extra_labels_empty_by_default;
          Alcotest.test_case "invalid rollout_strategy" `Quick
            test_toml_invalid_rollout_strategy;
          Alcotest.test_case "reserved label key rejected" `Quick
            test_toml_reserved_label_key;
          Alcotest.test_case "valid Recreate from toml" `Quick
            test_toml_valid_rollout_recreate;
          Alcotest.test_case "valid ingress overrides toml" `Quick
            test_toml_valid_ingress_overrides;
          Alcotest.test_case "invalid cpu quantity" `Quick
            test_toml_invalid_cpu_quantity;
          Alcotest.test_case "invalid memory quantity" `Quick
            test_toml_invalid_memory_quantity;
          Alcotest.test_case "invalid ingress host" `Quick
            test_toml_invalid_ingress_host;
          Alcotest.test_case "invalid ingress path" `Quick
            test_toml_invalid_ingress_path;
          Alcotest.test_case "secret keys from toml" `Quick
            test_toml_secret_keys;
          Alcotest.test_case "valid canary rollout toml" `Quick
            test_toml_valid_canary_rollout;
          Alcotest.test_case "valid blue-green rollout toml" `Quick
            test_toml_valid_blue_green_rollout;
          Alcotest.test_case "invalid progressive strategy" `Quick
            test_toml_invalid_progressive_strategy;
          Alcotest.test_case "canary requires steps" `Quick
            test_toml_canary_requires_steps;
          Alcotest.test_case "canary rejects bad weight" `Quick
            test_toml_canary_rejects_bad_weight;
          Alcotest.test_case "malformed TOML raises" `Quick
            test_toml_rejects_malformed;
          Alcotest.test_case "load_result validation error" `Quick
            test_toml_load_result_validation_error;
          Alcotest.test_case "load_result syntax error" `Quick
            test_toml_load_result_syntax_error;
          Alcotest.test_case "multi-line secrets array" `Quick
            test_toml_multiline_array_secrets;
          Alcotest.test_case "dotted section headers" `Quick
            test_toml_dotted_section_headers;
          Alcotest.test_case "canary pause steps" `Quick
            test_toml_canary_pause_steps;
        ] );
      ( "external_secrets",
        [
          Alcotest.test_case "external_secret_doc: no stringData" `Quick
            test_external_secret_doc_no_stringdata;
          Alcotest.test_case "external_secret_doc: keys present" `Quick
            test_external_secret_doc_keys_present;
          Alcotest.test_case "external_secret_doc: target name" `Quick
            test_external_secret_doc_target_name;
          Alcotest.test_case "render_spec ESO: no k8s Secret" `Quick
            test_render_spec_eso_backend_no_k8s_secret;
          Alcotest.test_case "render_spec ESO: all keys in data" `Quick
            test_render_spec_eso_backend_all_keys;
          Alcotest.test_case "render_spec ESO: no stringData" `Quick
            test_render_spec_eso_backend_no_stringdata;
          Alcotest.test_case "render_spec default: k8s placeholder" `Quick
            test_render_spec_k8s_placeholder_default;
        ] );
      ( "config_parsing_policy",
        [
          Alcotest.test_case "live: missing user secret → Error" `Quick
          ...
          Alcotest.test_case "live: no user secrets → always Ok" `Quick
            test_live_backend_no_user_secrets_always_succeeds;
        ] );
      ( "workload_shape",
        [
          Alcotest.test_case "Http_service deployment has ports" `Quick
            test_shape_http_service_deployment_has_ports;
          Alcotest.test_case "Background_worker deployment metrics port" `Quick
            test_shape_background_worker_deployment_has_metrics_port;
          Alcotest.test_case "Http_service rollout has ports" `Quick
            test_shape_rollout_http_service_has_ports;
          Alcotest.test_case "Background_worker rollout metrics port" `Quick
            test_shape_rollout_background_worker_metrics_port;
        ] );
      ( "artifact_invariants",
        [
          Alcotest.test_case "svc satisfies security invariants" `Quick
            test_svc_satisfies_invariants;
          Alcotest.test_case "worker satisfies security invariants" `Quick
            test_worker_satisfies_invariants;
          Alcotest.test_case "fn satisfies security invariants" `Quick
            test_fn_satisfies_invariants;
          Alcotest.test_case "canary rollout satisfies security invariants"
            `Quick test_rollout_canary_satisfies_invariants;
          Alcotest.test_case "blue-green rollout satisfies security invariants"
            `Quick test_rollout_blue_green_satisfies_invariants;
          Alcotest.test_case "GitOps mode redacts secret values" `Quick
      ...
      ( "taxonomy_labels",
        [
          ...
          Alcotest.test_case "not in selector" `Quick
            test_taxonomy_labels_not_in_selector;
          Alcotest.test_case "matches sol open dashboard link normalization"
            `Quick test_taxonomy_labels_match_dashboard_link_normalization;
          Alcotest.test_case "malformed image -> unknown" `Quick
            test_release_of_image_malformed;
          Alcotest.test_case "oversized tag truncated to 63 chars" `Quick
            test_release_of_image_oversized_tag_truncated;
          Alcotest.test_case "trailing non-alnum after truncation fixed up"
            `Quick test_release_of_image_trailing_non_alnum_after_truncation;
          Alcotest.test_case "numeric-looking tag stays quoted" `Quick
            test_release_of_image_numeric_tag_quoted;
          Alcotest.test_case "sanitize_label_value bounds length" `Quick
            test_sanitize_label_value_bounds_length;
          Alcotest.test_case "sanitize_label_value fixes trailing non-alnum"
            `Quick test_sanitize_label_value_fixes_trailing_non_alnum;
          Alcotest.test_case "sanitize_label_value no-op when already safe"
            `Quick test_sanitize_label_value_no_op_when_already_safe;
          Alcotest.test_case "sanitize_label_value empty -> unknown" `Quick
            test_sanitize_label_value_empty_falls_back_to_unknown;
          Alcotest.test_case
            "sanitize_label_value lowercases + replaces underscores" `Quick
            test_sanitize_label_value_lowercases_and_replaces_underscores;
          Alcotest.test_case "sanitize_label_value replaces internal space"
            `Quick test_sanitize_label_value_replaces_internal_space;
          Alcotest.test_case "sanitize_label_value strips leading non-alnum"
            `Quick test_sanitize_label_value_strips_leading_non_alnum;
        ] );
      ( "discover_services_filter",
        [
          Alcotest.test_case "hyphenated full path matches underscored dir"
            `Quick
            test_discover_services_hyphenated_filter_matches_underscored_dir;
          Alcotest.test_case "hyphenated basename matches" `Quick
            test_discover_services_hyphenated_basename_filter_matches;
          Alcotest.test_case "underscored filter still matches" `Quick
            test_discover_services_underscored_filter_still_matches;
          Alcotest.test_case "non-matching filter excludes" `Quick
