...
let render_spec_ok ?(workspace = "myapp") ?env ?image ?secret_backend spec =
  match Sol_cli_deployment_render.render_spec ~workspace ?env ?image ?secret_backend spec with
  | Ok v    -> v
...
let test_svc_secret_refs_without_values () =
  ...
  let (_ns, workload) = render_spec_ok
      ~secret_backend:Sol_cli_manifest.Kubernetes_placeholder spec in
...
let test_svc_image_override () =
  ...
  assert_absent "svc cluster image absent" workload "sol-registry:5000/myapp/charge-svc:abc123"
  ...
  let spec = {
    ...
    progressive_delivery =
      Some (Sol_cli_toml.Canary {
        steps = [ Sol_cli_toml.Weight 10; Sol_cli_toml.Weight 40; Sol_cli_toml.Weight 100 ];
      });
  } in
  let (_ns, workload) = render_spec_ok spec in
...
let test_toml_invalid_rollout_strategy () =
  ...
  close_out oc;
  let raised =
    try let _ = Sol_cli_toml.load path in false
    with Failure _ -> true
  in
  Sys.remove path;
...
let test_toml_valid_rollout_recreate () =
  ...
  check_bool "rollout_strategy is Recreate"
    true (toml.Sol_cli_toml.rollout_strategy = Some Sol_cli_toml.Recreate)
...
|};
  ...
  check_bool "secret keys parsed"
    true (toml.Sol_cli_toml.secret_keys = [ "DATABASE_URL"; "API_TOKEN" ])
...
|};
  ...
  check_bool "blue-green parsed"
    true (toml.Sol_cli_toml.progressive_delivery = Some Sol_cli_toml.Blue_green)
...
|};
  ...
  check_bool "replicas from dotted header"
    true (toml.Sol_cli_toml.replicas = Some 3);
...
let test_external_secret_doc_no_stringdata () =
  let doc = Sol_cli_manifest.external_secret_doc
    ~store_ref:"aws-secrets-manager"
    ~store_kind:"ClusterSecretStore"
    ~key_prefix:"myapp/"
    ~refresh_interval:"1h"
...
let test_live_backend_missing_user_secret_returns_error () =
  ...
  (match Sol_cli_deployment_render.render_spec ~workspace:"myapp"
      ~secret_backend:Sol_cli_manifest.Kubernetes_live spec_with_secret with
  | Ok _ -> ()
  ...
  (match Sol_cli_deployment_render.render_spec ~workspace:"myapp"
      ~secret_backend:Sol_cli_manifest.Kubernetes_live spec_missing with
  | Error msg ->
...
let test_live_backend_multiple_missing_secrets_all_reported () =
  ...
  (match Sol_cli_deployment_render.render_spec ~workspace:"myapp"
      ~secret_backend:Sol_cli_manifest.Kubernetes_live spec with
  | Error msg ->
...
let test_rollout_canary_satisfies_invariants () =
  let spec = {
    svc_spec with
    progressive_delivery =
      Some (Sol_cli_toml.Canary { steps = [ Sol_cli_toml.Weight 50; Sol_cli_toml.Weight 100 ] });
  } in
  let (_ns, workload) = render_spec_ok spec in
...
let test_shape_http_service_deployment_has_ports () =
  let doc = Sol_cli_manifest.deployment_doc
...
let test_shape_rollout_http_service_has_ports () =
  let doc = Sol_cli_manifest.rollout_doc
    ~shape:Sol_cli_manifest.Http_service
    ~replicas:1 ~cpu:"100m" ~memory:"128Mi"
    ~ns:"myapp-payments" ~name:"charge-svc"
    ~image:"sol-registry:5000/myapp/charge-svc:abc123"
...
let test_taxonomy_labels_match_dashboard_link_normalization () =
  ...
  assert_contains "workspace label matches sol open's normalization"
    workload {|workspace: "sol-obs-review-app"|};
  assert_contains "domain label matches sol open's normalization"
    workload {|domain: "payments-team"|};
  let dashboard_url =
    match Sol_cli_open.url ~base_url:"http://localhost:3000"
            ~workspace:"Sol_Obs_Review_App" ~kind:Sol_cli_open.Dashboard
            (Sol_cli_open.Domain "Payments_Team") with
    | Ok url -> url
...
let test_discover_services_hyphenated_filter_matches_underscored_dir () =
  ...
  let found = Sol_cli_manifest.discover_services ~filter_path:(Some "app/payments/charge-svc") in
...
let () =
  Alcotest.run "manifest_render"
    ...
    ; "escape_hatches", [
      ...
      ; Alcotest.test_case "rollout canary secrets use name-secrets"     `Quick test_rollout_canary_secrets_use_sol_secrets
      ; Alcotest.test_case "rollout blue-green secrets use name-secrets" `Quick test_rollout_blue_green_secrets_use_sol_secrets
    ...
    ; "taxonomy_labels", [
      ...
      ; Alcotest.test_case "matches sol open dashboard link normalization" `Quick test_taxonomy_labels_match_dashboard_link_normalization
