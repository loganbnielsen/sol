...
let test_scan_detects_tempo () =
  with_tmpdir (fun tmpdir ->
      ...
      output_string oc
        "(executable\n\
        \ (name main)\n\
        \ (libraries sol_svc obs-eio obs-loki-eio obs-prometheus-eio \
         obs-tempo-eio))\n";
      close_out oc;
...
let test_scan_tempo_absent_by_default () =
  with_tmpdir (fun tmpdir ->
      ...
      close_out oc;
      let req = Sol_cli_workspace.scan ~dir:tmpdir in
      check_bool "no tempo dep -> not detected" false
        req.Sol_cli_workspace.tempo)
    ...
    [
      ( "find_root",
        [
          ...
          Alcotest.test_case "nested service dir" `Quick
            test_finds_root_from_nested_service_dir;
          Alcotest.test_case "none outside a workspace" `Quick
            test_none_outside_any_workspace;
          Alcotest.test_case "app must be a directory" `Quick
            test_app_must_be_a_directory;
        ] );
      ( "scan",
        [
          Alcotest.test_case "detects tempo dependency" `Quick
            test_scan_detects_tempo;
          Alcotest.test_case "tempo absent by default" `Quick
