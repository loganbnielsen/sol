...
let test_backend_of_string_valid () =
  ...
  check_bool "self_hosted_durable" true
...
let test_resolve_self_hosted_durable_with_base_domain () =
  check_string "self_hosted_durable" "https://grafana.acme.com"
    (url_of
       (U.resolve ~backend:U.Self_hosted_durable ~base_domain:"acme.com" ()))
...
let test_resolve_self_hosted_durable_blank_base_domain () =
  check_bool "blank base_domain -> No_url" true
    (String.length
       (reason_of
          (U.resolve ~backend:U.Self_hosted_durable ~base_domain:"   " ()))
    > 0)

let test_resolve_external_never_guesses () =
  check_bool "external -> No_url even with base_domain" true
    (String.length
       (reason_of (U.resolve ~backend:U.External ~base_domain:"acme.com" ()))
    > 0)
...
let test_effective_no_target_no_flags_defaults_local () =
  let backend, base_domain =
    ok_pair
      (U.effective_backend_and_base_domain ~explicit_backend:None
         ~explicit_base_domain:None ~target:None ())
  in
...
let test_effective_target_supplies_backend_and_base_domain () =
  with_temp_dir (fun () ->
      ...
      let backend, base_domain =
        ok_pair
          (U.effective_backend_and_base_domain ~explicit_backend:None
             ~explicit_base_domain:None ~target:(Some "prod/aws/us-east-1") ())
      in
      check_bool "backend from target" true (backend = U.Self_hosted_durable);
      check_bool "base_domain from target" true
        (base_domain = Some "acme.example.com"))
...
|};
      let backend, base_domain =
        ok_pair
          (U.effective_backend_and_base_domain ~explicit_backend:None
             ~explicit_base_domain:None ~target:(Some "dev/aws/us-east-1") ())
      in
...
let test_effective_explicit_flag_overrides_target () =
  with_temp_dir (fun () ->
      ...
      check_bool "explicit base_domain wins" true
        (base_domain = Some "override.example.com"))
  ...
  with_temp_dir (fun () ->
      check_bool "malformed target path errors" true
        (String.length
           (err_msg
              (U.effective_backend_and_base_domain ~explicit_backend:None
                 ~explicit_base_domain:None ~target:(Some "not-a-valid-path") ()))
        > 0))
    ...
    [
      ( "backend_of_string",
        [
          ...
          Alcotest.test_case "invalid value" `Quick
            test_backend_of_string_invalid;
      ...
      ( "resolve",
        [
          ...
          Alcotest.test_case "self_hosted_durable with base_domain" `Quick
            test_resolve_self_hosted_durable_with_base_domain;
          Alcotest.test_case "self_hosted_durable without base_domain" `Quick
            test_resolve_self_hosted_durable_without_base_domain;
          Alcotest.test_case "self_hosted_durable blank base_domain" `Quick
            test_resolve_self_hosted_durable_blank_base_domain;
          Alcotest.test_case "external never guesses" `Quick
            test_resolve_external_never_guesses;
          Alcotest.test_case "override wins for every backend" `Quick
            test_resolve_override_wins_for_every_backend;
        ] );
      ( "effective_backend_and_base_domain",
        [
          Alcotest.test_case "no target/flags -> Local default" `Quick
            test_effective_no_target_no_flags_defaults_local;
          Alcotest.test_case "target supplies backend+base_domain" `Quick
            test_effective_target_supplies_backend_and_base_domain;
          Alcotest.test_case "target without the field -> Local" `Quick
            test_effective_target_without_observability_backend_falls_back_to_local;
          Alcotest.test_case "explicit flag overrides target" `Quick
            test_effective_explicit_flag_overrides_target;
          Alcotest.test_case "invalid observability_backend errors" `Quick
            test_effective_invalid_observability_backend_errors;
          Alcotest.test_case "unknown/malformed target path errors" `Quick
