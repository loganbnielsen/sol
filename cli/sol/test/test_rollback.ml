...
let test_execute_no_op_returns_ok () =
  ...
  | Ok () -> ()
  | Error e -> Alcotest.failf "expected Ok, got error: %s" (Sol_cli_rollback.error_to_string e)

(* ── Plugin_missing error_to_string ─────────────────────────────────────── *)

let test_plugin_missing_error_contains_install_link () =
  let err = Sol_cli_rollback.Plugin_missing { namespace = "myapp-payments"; name = "charge-svc" } in
...
let test_non_zero_error_to_string () =
  let err = Sol_cli_rollback.Non_zero { command = "kubectl rollout undo"; exit_code = 1 } in
  let msg = Sol_cli_rollback.error_to_string err in
