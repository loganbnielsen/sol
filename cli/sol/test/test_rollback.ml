...
let test_blue_green_gives_argo_rollout () =
  let spec =
    { base_spec with progressive_delivery = Some Sol_cli_toml.Blue_green }
  in
  match Sol_cli_rollback.rollback_target_of_service spec with
  | Sol_cli_rollback.Argo_rollout _ -> ()
  | _ ->
      Alcotest.fail "expected Argo_rollout for progressive_delivery=blue_green"
...
let test_worker_with_argo_gives_argo_rollout () =
  ...
  | Sol_cli_rollback.Argo_rollout _ -> ()
  | _ ->
      Alcotest.fail "expected Argo_rollout for Worker with progressive_delivery"
...
let test_fn_with_progressive_delivery_still_no_op () =
  ...
  | _ ->
      Alcotest.fail
        "Fn primitive must always produce No_op regardless of \
...
let test_execute_no_op_returns_ok () =
  match
    Sol_cli_rollback.execute_rollback (Sol_cli_rollback.No_op "test reason")
  with
  | Ok () -> ()
  | Error e ->
      Alcotest.failf "expected Ok, got error: %s"
        (Sol_cli_rollback.error_to_string e)

(* ── Plugin_missing error_to_string ─────────────────────────────────────── *)

let test_plugin_missing_error_contains_install_link () =
  let err =
    Sol_cli_rollback.Plugin_missing
...
let test_non_zero_error_to_string () =
  let err =
    Sol_cli_rollback.Non_zero
      { command = "kubectl rollout undo"; exit_code = 1 }
  in
  let msg = Sol_cli_rollback.error_to_string err in
  assert (
...
let () =
  Alcotest.run "rollback"
    [
      ( "rollback_target_of_service",
        [
          Alcotest.test_case "Svc -> Standard_deployment" `Quick
            test_svc_gives_standard_deployment;
          Alcotest.test_case "Worker -> Standard_deployment" `Quick
            test_worker_gives_standard_deployment;
          Alcotest.test_case "Fn -> No_op" `Quick test_fn_gives_no_op;
          Alcotest.test_case "canary -> Argo_rollout" `Quick
            test_progressive_delivery_gives_argo_rollout;
          Alcotest.test_case "blue_green -> Argo_rollout" `Quick
            test_blue_green_gives_argo_rollout;
          Alcotest.test_case "Worker+canary -> Argo_rollout" `Quick
            test_worker_with_argo_gives_argo_rollout;
          Alcotest.test_case "Fn+canary still No_op" `Quick
            test_fn_with_progressive_delivery_still_no_op;
        ] );
      ( "execute_rollback",
        [
          Alcotest.test_case "No_op always Ok" `Quick
            test_execute_no_op_returns_ok;
        ] );
      ( "error_to_string",
        [
          Alcotest.test_case "Plugin_missing contains install URL" `Quick
            test_plugin_missing_error_contains_install_link;
          Alcotest.test_case "Plugin_missing contains names" `Quick
            test_plugin_missing_error_contains_namespace_and_name;
          Alcotest.test_case "Non_zero contains command and code" `Quick
