...
let test_namespace_rejects_invalid_domain () =
  match Sol_cli_deployment_plan.namespace_result ~workspace:"myapp" ~domain:"payments.api" with
  | Error (Sol_cli_deployment_plan.Invalid_kubernetes_name { field; value; message }) ->
...
let test_namespace_rejects_overlong () =
  match Sol_cli_deployment_plan.namespace_result
...
let test_image_ref_local () =
  check_string "local k3d image ref" "sol-registry:5000/myapp/charge-svc:abc123"
    (Sol_cli_deployment_plan.image_ref
       ~registry:"sol-registry:5000" ~workspace:"myapp"
...
let test_to_json_mode_strings () =
  let check_mode mode expected =
    let env : Sol_cli_deployment_plan.env_config = {
      ...
      region = None; base_domain = None; secret_backend = Sol_cli_manifest.Kubernetes_placeholder;
    } in
...
topics = ["top.event"]
|};
    write_file "events/payments/sol.toml"    {|[service]
topics = ["payments.charged"]
|};
    write_file "events/orders/sol.toml"      {|[service]
...
schedule = "30 6 * * 1"
|};
    let schedule = Sol_cli_manifest.extract_schedule
...
let test_of_services_result_surfaces_toml_parse_error () =
  ...
  with_cwd tmp (fun () ->
    ...
    | Error (Sol_cli_deployment_plan.Toml_error (Sol_cli_toml.Validation { path; message })) ->
      Alcotest.(check string) "error path"
        "app/payments/charge_svc/sol.toml" path;
...
let test_sol_yml_scale_overrides_toml_replicas_on_resolved_target () =
  ...
  with_cwd tmp (fun () ->
    ...
    | Ok plan -> Alcotest.(check int) "sol.yml scale_max wins" 5 (replicas_of_sole_service plan)
    | Error err -> Alcotest.fail (Sol_cli_deployment_plan.plan_error_to_string err))
...
let test_sol_yml_scale_falls_back_to_scale_min_when_no_max () =
  ...
  with_cwd tmp (fun () ->
    ...
    match Sol_cli_deployment_plan.of_services_result
            ~workspace:"myworkspace" ~env:deploy_env ~resolved_config
            [ charge_svc_service ] with
    | Ok plan -> Alcotest.(check int) "sol.yml scale_min used when no scale_max" 3 (replicas_of_sole_service plan)
    | Error err -> Alcotest.fail (Sol_cli_deployment_plan.plan_error_to_string err))
...
let test_no_resolved_config_uses_toml_replicas () =
  ...
  with_cwd tmp (fun () ->
    ...
    match Sol_cli_deployment_plan.of_services_result
            ...
            [ charge_svc_service ] with
    | Ok plan -> Alcotest.(check int) "sol.toml replicas unchanged" 2 (replicas_of_sole_service plan)
    | Error err -> Alcotest.fail (Sol_cli_deployment_plan.plan_error_to_string err))
...
let test_no_matching_sol_yml_service_uses_toml_replicas () =
  ...
  with_cwd tmp (fun () ->
    ...
    match Sol_cli_deployment_plan.of_services_result
            ~workspace:"myworkspace" ~env:deploy_env ~resolved_config
            [ charge_svc_service ] with
    | Ok plan -> Alcotest.(check int) "no matching sol.yml service falls back to sol.toml" 2 (replicas_of_sole_service plan)
    | Error err -> Alcotest.fail (Sol_cli_deployment_plan.plan_error_to_string err))
...
let () =
  Alcotest.run "deployment_plan"
    ...
    ; "discover_topics", [
        Alcotest.test_case "finds topic from sol.toml"          `Quick test_discover_topics_finds_topic
      ...
      ; Alcotest.test_case "top-level events/sol.toml"          `Quick test_discover_topics_top_level_toml
    ...
    ; "extract_schedule", [
        Alcotest.test_case "reads schedule from sol.toml"            `Quick test_extract_schedule_reads_from_sol_toml
      ; Alcotest.test_case "default when no sol.toml"                `Quick test_extract_schedule_defaults_when_no_toml
    ...
    ; "of_services", [
      ...
      ; Alcotest.test_case "sol.yml scale_max overrides sol.toml replicas" `Quick test_sol_yml_scale_overrides_toml_replicas_on_resolved_target
      ; Alcotest.test_case "sol.yml scale_min used when no scale_max" `Quick test_sol_yml_scale_falls_back_to_scale_min_when_no_max
      ; Alcotest.test_case "no resolved config keeps sol.toml replicas" `Quick test_no_resolved_config_uses_toml_replicas
      ; Alcotest.test_case "no matching sol.yml service keeps sol.toml replicas" `Quick test_no_matching_sol_yml_service_uses_toml_replicas
      ]
    ; "plan_ids", [
