...
let test_local_registry () =
  ...
  check_string "cluster registry" "sol-registry:5000"
...
let test_local_constructor () =
  ...
  Alcotest.(check bool)
    "constructor is Local" true
    (match t with Sol_cli_env_target.Local _ -> true | _ -> false)
...
let test_customer_direct_registry () =
  match
    Sol_cli_env_target.customer_cloud_defaults
      ...
      ~image_tag:"sha-deadbeef" ~emit_to:None ()
  with
...
let test_customer_direct_constructor () =
  ...
  | Ok t ->
      Alcotest.(check bool)
        "constructor is Customer_direct" true
        (match t with
...
let test_customer_gitops_constructor () =
  ...
  | Ok t ->
      Alcotest.(check bool)
        "constructor is Customer_gitops" true
        (match t with
...
let test_empty_registry_fails () =
  match
    Sol_cli_env_target.customer_cloud_defaults ~registry:"" ~image_tag:"sha-abc"
      ~emit_to:None ()
  with
...
let test_whitespace_registry_fails () =
  Alcotest.(check bool)
    "whitespace registry fails" true
    (Result.is_error
       (Sol_cli_env_target.customer_cloud_defaults ~registry:"   "
          ~image_tag:"sha-abc" ~emit_to:None ()))
...
let test_to_env_config_local () =
  ...
  check_string "registry" "sol-registry:5000"
    cfg.Sol_cli_deployment_plan.registry;
...
let test_to_env_config_customer () =
  match
    Sol_cli_env_target.customer_cloud_defaults ~registry:"gcr.io/myproject"
      ~image_tag:"v1.2.3" ~emit_to:None ()
  with
  | Error msg -> Alcotest.fail ("unexpected error: " ^ msg)
  | Ok t ->
      ...
      Alcotest.(check bool)
        ...
        (cfg.Sol_cli_deployment_plan.mode
...
let test_local_default_backend () =
  let t = Sol_cli_env_target.local_defaults ~image_tag:"dev" in
  check_backend "Local → Kubernetes_live" Sol_cli_manifest.Kubernetes_live
...
let test_to_env_config_local_backend () =
  ...
  check_backend "to_env_config Local → Kubernetes_live"
    Sol_cli_manifest.Kubernetes_live cfg.Sol_cli_deployment_plan.secret_backend
    ...
    [
      ...
      ( "customer_cloud_defaults",
        [
          ...
          Alcotest.test_case "direct constructor" `Quick
            test_customer_direct_constructor;
          Alcotest.test_case "gitops constructor" `Quick
            test_customer_gitops_constructor;
          Alcotest.test_case "empty registry fails" `Quick
            test_empty_registry_fails;
          Alcotest.test_case "whitespace registry fails" `Quick
      ...
      ( "default_secret_backend",
        [
          ...
          Alcotest.test_case "Customer_direct → live" `Quick
            test_customer_direct_default_backend;
          Alcotest.test_case "Customer_gitops → placeholder" `Quick
            test_customer_gitops_default_backend;
        ] );
      ( "to_env_config secret_backend",
        [
          Alcotest.test_case "Local → live" `Quick
