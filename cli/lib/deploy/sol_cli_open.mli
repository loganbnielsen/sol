...
type scope =
  ...
  (** [Resource (resource_type, resource_name)] -- a managed infrastructure
          ...
          matching platform/cloud's generic-by-resource-type Terraform
          shape. *)
...
(** [url ~base_url ~workspace ~kind scope] builds the Grafana URL for [kind] at
    ...
    - [Logs] builds an Explore URL scoped by namespace (and service, when scoped
      to one); [Resource] scope has no logs view ([Error]) -- managed resources
      don't ship through Sol's Loki pipeline.
    - [Metrics] and [Dashboard] both deep-link into OBS-011's provisioned
      ...
      (platform/cloud/modules/platform's dashboards/managed-resource.json.tftpl, keyed
