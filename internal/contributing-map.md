...
docs/       for users: guides, reference (the application contract), deployment, architecture
internal/   maintainer machinery: ci/, qualification/, pipeline/, planning/, specs/, tooling/, fixtures/
```

If you are working on the product, you want `cli/`, `docs/reference/`
(the application contract), and `framework/`. `internal/` is not part of the product or the application
...
- [`/style-audit`](pipeline/audits/STYLE_AUDIT.md)
  for repository style and consistency work, against the checklist in
  `internal/pipeline/audits/STYLE_AUDIT.md`.
- [`/audit`](pipeline/audits/AUDIT.md) for product and implementation correctness
  checks.
- [`/scaffold-audit`](pipeline/audits/SCAFFOLD_AUDIT.md) for generated workspace and
  service contract checks.
- [`/e2e`](fixtures/local-demo/test/test_e2e.ml) for end-to-end local
  workflow verification.
  [`internal/tooling/scripts/run_tests.sh`](tooling/scripts/run_tests.sh)
...
validated by `internal/pipeline/audits/SCAFFOLD_AUDIT.md` and tests in
...
checklists in `internal/pipeline/audits/` define manual verification expectations.
