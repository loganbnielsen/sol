...
let apply_venus_migrations ~fs pool =
  Migration.apply
    pool
    ~dir:"internal/fixtures/venus/db/migrations"
...
let () =
  ...
  List.iter
    (fun (customer_id, amount_cents, currency) ->
       ...
       in
       Printf.printf
         "[client]        POST /charges customer=%-10s  %5d %s  -> HTTP %d  corr=%s\n%!"
         customer_id
         amount_cents
         currency
         status
  ...
  (match pushgateway_url with
   ...
   | Some url ->
     (match
        Obs_prometheus.push ~net:env#net ~clock:env#clock ~url ~job:"venus" render
      with
