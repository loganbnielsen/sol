...
| FND-0014 | AWS (live) | The deploy identity cannot maintain the release record: it may not `list` ConfigMaps in `default` (**prune skipped**), and — the same apply→patch mechanism as FND-0011 — it may not `patch` the release **pointer**, which therefore still names an older release while the deploy reports success | `DEC-037` → `INFRA-054` (record failure is fatal) + `INFRA-055` (write mechanism, no generic `patch` needed — checked against the boundary lease) · `INFRA-051` (prune) | `VERIFIED_DEFECT` (severity **escalated**: fail-open) | `OPEN` — decided 2026-09-20; fixes in flight, delivered separately |
...
| FND-0016 | AWS (live) | A scoped deploy requires its call graph to be inside the scope, so a unit that calls another unit cannot be deployed alone — and with FND-0012 the workspace has **no route that deploys all its services** | `DEC-036` (option 4: inventory resolution, fail closed, no widening) → `INFRA-053` | `VERIFIED_DEFECT` | `OPEN` — decided 2026-09-20; fix in flight |
