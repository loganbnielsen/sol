...
| FND-0014 | AWS (live) | The deploy identity cannot maintain the release record: it may not `list` ConfigMaps in `default` (**prune skipped**), and — the same apply→patch mechanism as FND-0011 — it may not `patch` the release **pointer**, which therefore still names an older release while the deploy reports success | `INFRA-051` | `VERIFIED_DEFECT` (severity **escalated**: fail-open) | `OPEN` — two manifestations, both needing a contract decision |
...
| FND-0016 | AWS (live) | A scoped deploy requires its call graph to be inside the scope, so a unit that calls another unit cannot be deployed alone — and with FND-0012 the workspace has **no route that deploys all its services** | needs a `DEC` first (four options framed in the finding) | `VERIFIED_DEFECT` | `OPEN` |
