# Sol Framework: Developer Experience Audit

This is a reusable audit template. When performing an audit, copy this file (e.g., `EXPERIENCE_FINDINGS_01.md`), work through every section as if you are a startup engineer encountering Sol for the first time, and record every gap in the Findings section using the format at the bottom.
...
**What this audits:** Whether a startup engineer who wants to focus on business logic can start a project, develop it locally, and ship it to the cloud using only Sol's documented commands — without consulting external documentation, asking for help, or understanding the infrastructure underneath.

**Mission alignment lens:** The developer experience must teach Sol's architecture while staying frictionless. A passing UX does not merely get a service running; it makes domain ownership, typed event contracts, explicit auth, generated infrastructure, observability, and day-2 operations feel like the normal way to build.
...
A startup engineer should be able to install Sol with a single command and immediately have a working `sol` binary. They should not need to install a language toolchain, understand package managers, or read a prerequisites list longer than one line.

### Docs gate
* [ ] The README or a linked getting-started guide explains how to install Sol in one step
...
**Invariants:**
* [ ] `sol --version` works immediately after following the installation guide
...
After installing Sol, a new engineer should be able to scaffold a working project with one command and understand its structure from the generated files and documentation alone.
...
**Invariants:**
* [ ] `sol new workspace` runs without errors
...
### Docs gate
* [ ] The guide shows the exact `sol deploy` command with all required flags
...
**Invariants:**
* [ ] `sol deploy` builds images, pushes to the registry, applies manifests, and waits for rollout — all in one command
...
### Docs gate
* [ ] The guide explains the change → deploy cycle in terms of Sol commands only
...
**Invariants:**
* [ ] `sol deploy` picks up the code change, rebuilds, and deploys without additional flags
* [ ] The change is live within 2 minutes of the command completing
* [ ] `sol rollback` (or equivalent) restores the previous version if called immediately after
...
### Docs gate
* [ ] The guide shows how to add a new event contract with `sol new event <domain>/<name>`
...
* [ ] The guide explains what infrastructure Sol will synthesize for the new domain/service
...
* [ ] Schema compatibility is checked by a documented Sol command or generated test path before deploy
...
Sol is explicitly designed for AI-assisted development. A typical agent should be able to make a small feature change from the generated structure and local docs without guessing infrastructure details or inventing conventions.
