...
- Add a CI check: every directory under `cloud/<provider>/` exists under every other provider (rule 4).
...
- The mirror check fails when a root is missing on one provider (with a mutation test in the style of `internal/ci/test_*.sh`).
