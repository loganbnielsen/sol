...
- **Dropped an unverifiable claim.** `framework/README.md` said "Sol hosts the TypeScript scaffold templates". `rg -il 'fastify|package\.json|@sol-fab' cli/sol/lib cli/sol/bin` matches only a comment in `sol_cli_workspace.mli`. The positive control, `rg -c 'dune-project|\(executable' cli/sol/lib/sol_cli_scaffold_templates.ml` → 6, shows the scaffold templates are visible to the search and are OCaml. So the new text doesn't repeat it. If TypeScript scaffolding lands (FEAT-082), link it from `framework/typescript/README.md`.
