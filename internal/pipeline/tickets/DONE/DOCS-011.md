...
**Description:** `sol open` (opens Grafana logs/metrics/dashboard views in a browser, with `--scope`/`--observability-backend`/`--base-domain` flags) is registered in `cli/sol/bin/main.ml`'s command group alongside every other top-level command, but `README.md`, `docs/guides/TUTORIAL.md`, and `docs/ROADMAP.md` have zero mentions of it while every other command has TUTORIAL.md coverage.
