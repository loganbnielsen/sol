...
**Remediation:** Update `internal/pipeline/audits/AUDIT.md`'s source-location references and prose to the current layout: `cli/sol/bin/` · `cli/sol/lib/sol_cli_scaffold.ml` · `cli/sol/lib/sol_cli_manifest.ml` + `sol_cli_manifest_yaml.ml`; note Kafka core/consumer/producer now live in the external `~/Code/kafka-eio` package (flat `lib/kafka_stubs.c`, `lib/kafka_consumer.ml`); `framework/sol-worker/lib/worker.ml`; `Sol.Service.Make`/`Sol.Worker.Make`/`Sol.Fn.Make`; and replace `sun.toml` references with the current `sol.yml`/target-file model. Keep the file as a reusable template — just point it at what exists today.

## Review — automated checks passed
docs-only refresh of AUDIT.md's stale sun->sol paths; every claimed path/module/label/table-name spot-checked against real current source and confirmed correct; diff scoped to the single file, project/tickets/ untouched in the branch, build clean
