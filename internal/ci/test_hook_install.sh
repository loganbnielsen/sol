...
scratch="$tmp/repo"
mkdir -p "$scratch/cli/platform/local/scripts" "$scratch/internal/tooling/hooks"
cp "$root/cli/platform/local/scripts/install-hooks.sh" "$scratch/cli/platform/local/scripts/"
...
(cd "$scratch" && bash cli/platform/local/scripts/install-hooks.sh >/dev/null) \
