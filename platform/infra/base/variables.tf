...
# job (platform/infra/aws installs the EBS CSI addon and its scoped IRSA
...
# ── self_hosted_durable (GCP) — from platform/infra/gcp's outputs ──────── #
...
variable "loki_gcs_bucket" {
  description = "GCS bucket for durable Loki storage. From platform/infra/gcp's loki_gcs_bucket output."
  type        = string
  default     = ""
}

variable "loki_workload_identity_sa_email" {
  description = "GCP service account email for Loki's GCS access (Workload Identity). From platform/infra/gcp's loki_workload_identity_sa_email output."
  type        = string
  default     = ""
}

variable "thanos_gcs_bucket" {
  description = "GCS bucket for durable Prometheus/Thanos storage. From platform/infra/gcp's thanos_gcs_bucket output."
  type        = string
  default     = ""
}

variable "thanos_workload_identity_sa_email" {
  description = "GCP service account email for Thanos's GCS access (Workload Identity). From platform/infra/gcp's thanos_workload_identity_sa_email output."
  type        = string
  default     = ""
}
